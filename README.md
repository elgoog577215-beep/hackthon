...
本仓库同时维护灵知和[启智](./qizhi/README.md)。灵知代码保留在根目录；启智代码位于 `qizhi/`，两者使用同一提交，不再通过子模块同步。联合启动与配置检查入口为 `./scripts/qizhi.sh`，灵知独立开发继续使用 `./dev.sh`。
...
| `qizhi-dev` | 浙江大学启智开发与联调服务器 | 启智开发环境使用本仓库同一提交中的启智与灵知代码 | 先连接浙江大学 aTrust / IDC；账号保存在本机密码管理器 |
| `qizhi-prod` | 浙江大学启智正式生产服务器 | 从本仓库经过验收的提交发布启智与灵知 | 先连接浙江大学 aTrust / IDC；账号保存在本机密码管理器，运行密钥保存在服务器 `.env` |
...
本仓库提交（qizhi/ + 根目录灵知） ─▶ qizhi-dev ─▶ qizhi-prod
                         │
                         └──────────▶ a800-qwen（按环境配置调用）
```

服务器地址、账号或密钥发生变化时，先更新私有凭据记录；只有服务器职责、发布关系或访问方式变化时才修改 README。启智联合构建、配置与发布步骤见[接入与部署说明](./qizhi/docs/灵知新版课程接入与部署.md)。
