  ...
  <section class="teacher-workbench">
    ...
    <main class="workbench-center">
      <header class="center-heading">
        <div><small>{{ activeStageDefinition.step }} / 05</small><h2>{{ activeStageDefinition.label }}</h2><p>{{ activeStageDefinition.description }}</p></div>
...
type StageId = 'foundation' | 'lesson' | 'question-bank' | 'script' | 'ppt'
...
const referencesByStage = reactive<Record<StageId, CourseReferenceItem[]>>({ foundation: [], lesson: [], 'question-bank': [], script: [], ppt: [] })
...
async function loadQuestionBankStatus() { if (!props.courseId) return; try { const response = await http.get(`/api/courses/${props.courseId}/question-bank`, { silentError: true }); questionBankReady.value = Number(response.data?.total || 0) > 0 } catch { questionBankReady.value = false } }
