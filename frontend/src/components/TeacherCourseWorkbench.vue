<template>
  <section class="teacher-workbench">
    <aside class="stage-rail" :aria-label="t('courseWorkbench.stageNavigation', '课程生产阶段')">
      <header><strong>{{ t('courseWorkbench.title', '课程工作台') }}</strong><small>{{ t('courseWorkbench.progress', '按依赖顺序逐步完善') }}</small></header>
      <nav>
    ...
    <main class="workbench-center">
      <header class="center-heading">
        <div><small>{{ activeStageDefinition.step }} / 04</small><h2>{{ activeStageDefinition.label }}</h2><p>{{ activeStageDefinition.description }}</p></div>
      ...
      <section v-if="showStreaming" class="generation-surface" aria-live="polite">
        <header>
          <div><LoaderCircle :size="18" class="spin" /><span><strong>{{ t('courseWorkbench.generating', '正在生成课程大纲') }}</strong><small>{{ currentGenerationLabel }}</small></span></div>
        ...
        <article class="stream-content">
          ...
          <div v-if="!visibleStreamNodes.length" class="stream-waiting"><LoaderCircle :size="20" class="spin" />{{ t('courseWorkbench.waitingForContent', 'AI 正在建立课程结构…') }}</div>
        </article>
      ...
      <section v-else class="lesson-stage">
        ...
        <template v-else-if="activeStage === 'script-ppt'">
          <div class="asset-switch"><button type="button" :class="{ active: pairedAsset === 'script' }" @click="pairedAsset = 'script'">{{ t('courseWorkbench.script', '讲稿') }}</button><button type="button" :class="{ active: pairedAsset === 'ppt' }" @click="pairedAsset = 'ppt'">PPT</button></div>
          <section v-if="pairedAsset === 'script'" class="formal-surface lesson-formal"><header><div><strong>{{ t('courseWorkbench.script', '讲稿') }}</strong><small>{{ t('courseWorkbench.scriptPptSameSource', '与 PPT 结构化同源') }}</small></div><button type="button" :disabled="!selectedLessonId" @click="emit('openScript', selectedLessonId)"><Pencil :size="15" />{{ t('courseWorkbench.openEditor', '打开编辑') }}</button></header><article v-if="scriptMarkdown"><MarkdownRenderer :content="scriptMarkdown" /></article><div v-else class="empty-asset">{{ t('courseWorkbench.scriptPending', '课程内容生成后，这里会形成可编辑的轻量讲稿。') }}</div></section>
          <section v-else class="formal-surface lesson-formal"><header><div><strong>PPT</strong><small>{{ pptAsset ? t('courseWorkbench.formalSaved', '已进入课程正式文件') : t('courseWorkbench.pptFromScript', '从教案与讲稿生成') }}</small></div><button type="button" :disabled="pptBusy || !workingLessonRevision" @click="generatePpt"><LoaderCircle v-if="pptBusy" :size="15" class="spin" /><Presentation v-else :size="15" />{{ pptAsset ? t('courseWorkbench.openPpt', '打开 PPT') : t('courseWorkbench.generatePpt', '生成 PPT') }}</button></header><article v-if="pptAsset"><h3>{{ pptAsset.revisions?.at(-1)?.deck?.title || selectedLesson?.title }}</h3><ol><li v-for="slide in pptAsset.revisions?.at(-1)?.deck?.slides || []" :key="slide.slide_id">{{ slide.title }}</li></ol></article><div v-else class="empty-asset">{{ workingLessonRevision ? t('courseWorkbench.pptReadyToGenerate', '教案已准备好，可以生成配套 PPT。') : t('courseWorkbench.planBeforePpt', '请先生成并确认本讲教案。') }}</div></section>
        </template>

        <template v-else>
          <form class="stage-form stage-form--lesson" @submit.prevent="openQuestionBank">
            <div class="form-grid"><label class="form-field"><span>{{ t('courseWorkbench.form.questionCount', '题目数量') }}</span><input v-model.number="questionCount" type="number" min="1" max="100" /></label><label class="form-field"><span>{{ t('courseWorkbench.form.difficulty', '难度') }}</span><select v-model="questionDifficulty"><option value="basic">{{ t('courseWorkbench.form.basic', '基础') }}</option><option value="mixed">{{ t('courseWorkbench.form.mixed', '混合') }}</option><option value="challenge">{{ t('courseWorkbench.form.challenge', '挑战') }}</option></select></label></div>
            <label class="form-field form-field--wide"><span>{{ t('courseWorkbench.form.questionRequirements', '命题要求') }}</span><textarea v-model.trim="questionRequirements" rows="4" :placeholder="t('courseWorkbench.form.questionRequirementsPlaceholder', '例如：覆盖核心知识点，包含应用题')" /></label>
            <footer><span>{{ t('courseWorkbench.form.questionSourceHint', '老师未提供题库时由 AI 生成') }}</span><button class="primary" type="submit" :disabled="!selectedLessonId"><ListChecks :size="16" />{{ t('courseWorkbench.openQuestionBank', '进入题库') }}</button></footer>
          </form>
        </template>
      </section>
    </main>
...
import { BookOpenText, Check, ClipboardList, FileText, Layers3, ListChecks, LoaderCircle, Pause, Pencil, Presentation, RefreshCw, Sparkles } from 'lucide-vue-next'
...
type StageId = 'foundation' | 'lesson' | 'script-ppt' | 'assessment'
const props = defineProps<{ courseId: string; courseTitle: string; generationOptions: CourseGenerationOptions & { subject?: string }; generationStarting?: boolean }>()
...
const activeStage = ref<StageId>('foundation'); const pairedAsset = ref<'script' | 'ppt'>('script'); const selectedLessonId = ref('')
const referencesByStage = reactive<Record<StageId, CourseReferenceItem[]>>({ foundation: [], lesson: [], 'script-ppt': [], assessment: [] })
...
const lessonRequirements = ref(''); const questionCount = ref(12); const questionDifficulty = ref('mixed'); const questionRequirements = ref('')
...
const stages = computed(() => [
  ...
  { id: 'script-ppt' as const, step: '03', label: t('courseWorkbench.stages.scriptPpt', '讲稿与 PPT'), description: t('courseWorkbench.stages.scriptPptHelp', '结构化同源'), icon: markRaw(BookOpenText) },
  { id: 'assessment' as const, step: '04', label: t('courseWorkbench.stages.assessment', '题库'), description: t('courseWorkbench.stages.assessmentHelp', '练习、作业与考试'), icon: markRaw(ListChecks) },
])
...
const generationError = computed(() => generationStore.generationStatus === 'error' ? String(generationStore.failureReport?.failed_nodes?.[0]?.error || t('courseWorkbench.generationFailed', '生成中断，可以从当前结果重试。')) : '')
...
function stageReady(stage: StageId) { if (stage === 'foundation') return hasOutline.value; if (stage === 'lesson') return lessonStore.lessons.some(item => Boolean(item.plan.working_revision_id)); if (stage === 'script-ppt') return courseStore.nodes.some(node => Boolean(node.node_content)); return false }
...
async function openQuestionBank() { if (!selectedLesson.value) return; await saveRelationships(`question-bank:${selectedLessonId.value}`, 'question_bank', `${selectedLesson.value.title}题库`); emit('openPractice', selectedLessonId.value) }
...
@media(max-width:760px){.teacher-workbench{height:auto;min-height:100%;grid-template-columns:1fr;overflow:auto}.stage-rail{display:block;border-right:0;border-bottom:1px solid #e4e9f1}.stage-rail>header,.stage-rail>footer{display:none}.stage-rail nav{grid-template-columns:repeat(4,minmax(0,1fr));overflow:auto;padding:8px}.stage-rail nav button{min-width:108px;min-height:50px;grid-template-columns:22px minmax(0,1fr);padding:6px 8px}.stage-rail nav small{display:none}.workbench-center{overflow:visible;padding:18px 12px 30px}.center-heading h2{font-size:21px}.center-heading>button{font-size:0;width:38px;padding:0;justify-content:center}.stage-form{padding:19px 16px}.form-grid{grid-template-columns:1fr}.stage-form>footer{align-items:stretch;flex-direction:column}.primary{justify-content:center}.lesson-selector{grid-template-columns:1fr}.stream-content,.formal-surface>article{max-height:none;padding-inline:18px}.reference-tray{border-left:0;border-top:1px solid #e4e9f1}}
