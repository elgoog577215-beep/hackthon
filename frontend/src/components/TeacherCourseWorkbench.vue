    ...
    >
      ...
      <section v-else class="lesson-stage" :class="{ 'has-lesson-outline': activeStage === 'lesson' && lessonStore.lessons.length && !outlineGatePending }">
        <div class="lesson-workspace" :class="{ 'is-outline-collapsed': lessonOutlineCollapsed }">
          ...
          >
            <nav v-show="!lessonOutlineCollapsed" id="lesson-outline-navigation">
            ...
            >
              <small class="lesson-outline-handle-count">{{ Math.max(1, selectedLessonIndex + 1) }}/{{ lessonStore.lessons.length }}</small>
              <PanelLeftOpen v-if="lessonOutlineCollapsed" :size="16" />
              <PanelLeftClose v-else :size="16" />
              <span>{{ lessonOutlineCollapsed
                ? t('courseWorkbench.lessonOutline.showProgress', '展示总进度')
                : t('courseWorkbench.lessonOutline.hideProgress', '收起总进度') }}</span>
            </button>
          </aside>
...
.has-lesson-outline .lesson-workspace{grid-template-columns:208px minmax(0,1fr);gap:20px}.has-lesson-outline .lesson-workspace.is-outline-collapsed{grid-template-columns:40px minmax(0,1fr);gap:10px}.lesson-outline,.is-outline-collapsed .lesson-outline{position:relative;display:block;min-height:156px}.lesson-outline>nav{position:relative;padding:3px 0 3px 2px}.lesson-outline>nav::before{position:absolute;top:18px;bottom:18px;left:12px;width:1px;background:#dde3ec;content:""}.lesson-outline-chapter-button{position:relative;min-height:46px;grid-template-columns:20px minmax(0,1fr);gap:7px;padding:5px 7px 5px 2px;border-radius:8px}.lesson-outline-chapter-marker{position:relative;z-index:1;width:6px;height:6px;border-color:#c4cedb;background:#f3f5f9}.lesson-outline-chapter-marker[data-state="generating"]{border-color:#6661dc;background:#6661dc}.lesson-outline-chapter-marker[data-state="review"]{border-color:#8884d8;background:#f3f2ff}.lesson-outline-chapter-marker[data-state="confirmed"]{border-color:#6661dc;background:#6661dc}.lesson-outline-chapter-marker[data-state="failed"]{border-color:#d75563;background:#d75563}.lesson-outline-chapter-copy{gap:1px}.lesson-outline-chapter-copy strong{color:#5e6b7e;font-size:11.5px;font-weight:620;line-height:1.4}.lesson-outline-chapter-copy small{color:#8a96a8;font-size:9.5px}.lesson-outline-chapter-copy small[data-state="review"]{color:#7773bd}.lesson-outline-chapter-copy small[data-state="failed"]{color:#b94b57}.lesson-outline-chapter-button:hover{background:rgba(255,255,255,.52)}.lesson-outline-chapter-button.active{background:rgba(239,240,255,.62)}.lesson-outline-chapter-button.active .lesson-outline-chapter-marker{box-shadow:none}.lesson-outline-chapter-button.active strong{color:#34316f}.lesson-outline-chapter-button.active small{color:#6965b9}.lesson-outline-handle{position:absolute;z-index:3;top:8px;right:-18px;width:38px;min-height:140px;justify-content:center;gap:10px;padding:11px 0;border:1px solid #d6d7f4;border-radius:11px;color:#514bdc;background:#f5f4ff;box-shadow:0 8px 22px rgba(63,58,144,.13);transition:color .16s ease,border-color .16s ease,background .16s ease,box-shadow .16s ease,transform .16s cubic-bezier(.2,.8,.2,1)}.is-outline-collapsed .lesson-outline-handle{right:1px}.lesson-outline-handle-count{color:#706cc7;font-size:9px;font-weight:800;font-variant-numeric:tabular-nums;line-height:1}.lesson-outline-handle span{color:inherit;font-size:10.5px;font-weight:700;letter-spacing:.06em;line-height:1.15}.lesson-outline-handle:hover{transform:translateY(-1px);border-color:#bfc0ed;color:#3730a3;background:#fff;box-shadow:0 10px 26px rgba(63,58,144,.17)}.lesson-outline-handle:focus-visible{outline:3px solid rgba(91,87,232,.2);outline-offset:2px}
@media(max-width:1320px){.has-lesson-outline .lesson-workspace{grid-template-columns:204px minmax(0,1fr);gap:18px}.has-lesson-outline .lesson-workspace.is-outline-collapsed{grid-template-columns:40px minmax(0,1fr);gap:10px}}
.lesson-outline-handle:focus-visible{outline:0;border-color:#aaa7ea;box-shadow:0 8px 22px rgba(63,58,144,.16)}
