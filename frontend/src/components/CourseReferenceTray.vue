<template>
  <aside class="reference-tray" :class="{ 'is-compact': compact, 'is-question-bank': variant === 'question-bank', 'is-ppt': stage === 'ppt' }" :aria-label="trayTitle">
    ...
    <section v-if="variant === 'question-bank'" class="source-group source-group--question-bank">
      <div class="group-heading"><strong>{{ t('courseWorkbench.references.questionSources', '真题资料') }}</strong><small>{{ questionSources.length }}</small></div>
      <p class="question-source-help">{{ t('courseWorkbench.references.questionSourcesHelp', '上传历年试卷或已有题库，作为导入与智能出题依据') }}</p>
      <div class="reference-list">
        ...
        ><Plus :size="16" />{{ t('courseWorkbench.references.addQuestionSources', '添加或拖入真题') }}</button>
      </div>
...
.reference-tray.is-question-bank{height:100%;overflow:auto;border-left:0;background:#fbfcfe}.source-group--question-bank{padding-bottom:18px}.question-source-help{margin:0;color:#7a8699;font-size:11px;line-height:1.55}.reference-tray.is-question-bank .reference-add{min-height:48px}
</style>
