  ...
  <aside class="reference-tray" :class="{ 'is-compact': compact, 'is-question-bank': variant === 'question-bank', 'is-ppt': stage === 'ppt' }" :aria-label="trayTitle">
    ...
    <section v-if="variant === 'question-bank'" class="source-group source-group--question-bank">
      <div class="reference-list">
        ...
        <p v-if="!questionSources.length" class="question-source-empty">
          {{ t('courseWorkbench.references.questionSourcesEmpty', '暂无真题资料') }}
        </p>
        <button
          type="button"
          class="reference-add"
...
.reference-tray.is-question-bank{height:100%;overflow:auto;border-left:0;background:#fbfcfe}.reference-tray.is-question-bank .reference-tray__header{background:#fbfcfe}.source-group--question-bank{gap:10px;padding:12px 14px 18px}.source-group--question-bank .reference-list{gap:0}.source-group--question-bank .reference-item{min-height:48px;padding:7px 2px;border:0;border-bottom:1px solid #e7ebf2;border-radius:0;background:transparent}.question-source-empty{margin:22px 0 12px;color:#8a94a5;font-size:11px;text-align:center}.reference-tray.is-question-bank .reference-add{min-height:36px;margin-top:8px;border:0;border-radius:8px;color:#5552c8;background:#f0f1ff}.reference-tray.is-question-bank .reference-add:hover{color:#4338ca;background:#e7e8ff}
</style>
