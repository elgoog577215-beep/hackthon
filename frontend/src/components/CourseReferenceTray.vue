<template>
  <aside class="reference-tray" :class="{ 'is-compact': compact, 'is-question-bank': variant === 'question-bank', 'is-ppt': stage === 'ppt' }" :data-workflow-state="effectiveWorkflowState" :aria-label="trayTitle">
    ...
    <Transition name="tray-mode" mode="out-in">
      ...
      <section v-else-if="variant === 'default' && workflowLocked" key="workflow" class="workflow-state" :class="`workflow-state--${effectiveWorkflowState}`" aria-live="polite">
        <header>
      ...
      <div v-else-if="variant === 'default'" key="sources" class="reference-interactive">
        <section v-if="effectiveWorkflowState !== 'collecting' && (!completedWorkflow || completedSourceEditing)" class="source-status" :class="[`source-status--${effectiveWorkflowState}`, `source-status--sources-${sourceReadiness.kind}`, { 'is-editing': completedSourceEditing, 'is-dirty': sourceSelectionDirty }]" aria-live="polite">
