  ...
  <aside class="reference-tray" :class="{ 'is-compact': compact, 'is-question-bank': variant === 'question-bank', 'is-ppt': stage === 'ppt' }" :data-workflow-state="effectiveWorkflowState" :aria-label="trayTitle">
    ...
    <Transition name="tray-mode" mode="out-in">
      ...
      <div v-else-if="variant === 'default'" key="sources" class="reference-interactive">
        <section v-if="effectiveWorkflowState !== 'collecting'" class="source-status" :class="[`source-status--${effectiveWorkflowState}`, `source-status--sources-${sourceReadiness.kind}`, { 'is-editing': completedSourceEditing, 'is-dirty': sourceSelectionDirty }]" aria-live="polite">
        ...
        <section v-if="completedSnapshotVisible" class="confirmed-source-summary">
          <div class="group-heading">
            ...
            <small>{{ displayedSourceSnapshot.length }}</small>
          </div>
          ...
          <p v-else class="confirmed-source-empty">{{ t('courseWorkbench.references.confirmedWithoutSources', '本阶段未使用补充资料，系统依据课程信息和前序已生成内容生成。') }}</p>
          ...
          <small class="confirmed-source-hint">{{ sourceSelectionDirty
            ? t('courseWorkbench.references.pendingAdjustedSourcesHint', '以上仍是当前内容实际使用的资料；本次调整将在重新生成后生效。')
            : t('courseWorkbench.references.adjustSourcesHint', '只有主动进入调整后才会显示上传入口；已生成内容不会被直接改动。') }}</small>
        </section>
