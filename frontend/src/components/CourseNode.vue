...
.course-node[data-level="2"] { position:relative; margin:30px auto 38px; padding:0 clamp(22px,4vw,42px) 34px; overflow:visible; border:1px solid rgba(241,245,249,.96); border-radius:var(--lz-radius-surface); background:var(--bg-secondary); }
...
.course-opening > span { padding:6px 12px; border:1px solid rgba(203,213,225,.68); border-radius:999px; color:var(--lz-brand-strong); background:rgba(255,255,255,.7); font-size:var(--text-xs); font-weight:600; text-transform:uppercase; }
.course-opening h1 { max-width:760px; margin:22px 0 14px; color:#312e81; font-size:clamp(30px,4vw,46px); line-height:1.16; }
.course-opening p,.chapter-copy p { max-width:720px; margin:0; color:var(--lz-text-secondary); font-size:var(--text-xs); line-height:1.7; }
...
.chapter-badge { display:inline-flex; align-items:center; gap:7px; padding:5px 8px; border-radius:var(--radius-sm); color:var(--lz-brand-strong); background:var(--lz-brand-soft); font-size:var(--text-xs); font-weight:600; letter-spacing:0; }
.chapter-badge svg { color:inherit; }
.chapter-index { color:var(--lz-text-secondary); font-size:22px; font-weight:500; font-variant-numeric:tabular-nums; line-height:1; }
.chapter-copy { min-width:0; grid-column:1/-1; }
.chapter-copy h2 { margin:0 0 16px; color:var(--lz-text-strong); font-size:clamp(28px,2.4vw,36px); font-weight:700; line-height:1.4; overflow-wrap:anywhere; }
.chapter-status { display:flex; flex-wrap:wrap; justify-content:flex-end; gap:5px; }
.chapter-status span { padding:4px 8px; border:1px solid rgba(203,213,225,.68); border-radius:999px; color:var(--lz-text-secondary); background:rgba(255,255,255,.72); font-size:var(--text-xs); }
...
.generation-status[data-state="generating"],.section-generation-status[data-state="generating"] { border-color:#c7d2fe; color:var(--lz-brand-strong); background:var(--lz-brand-soft); }
...
.generation-placeholder { min-height:52px; display:flex; align-items:center; gap:9px; margin:8px 0 0; padding:12px 14px; border:1px dashed #dbe2ee; border-radius:10px; color:#64748b; background:#f8fafc; font-size:var(--text-xs); font-weight:650; }
.generation-placeholder[data-state="generating"] { border-color:#c7d2fe; color:var(--lz-brand-strong); background:#f5f7ff; }
.generation-placeholder[data-state="failed"] { border-color:#fecaca; color:#b91c1c; background:#fef2f2; }
.task-launcher { position:relative; width:100%; min-height:112px; margin:30px 0 0; display:grid; grid-template-columns:44px minmax(0,1fr) auto; align-items:center; gap:16px; overflow:hidden; padding:18px 18px 18px 20px; border:1px solid #dbe2ee; border-radius:16px; color:var(--lz-text); background:#f8fafc; text-align:left; cursor:pointer; transition:border-color var(--duration-fast),background var(--duration-fast); }
.task-launcher:hover { border-color:var(--lz-brand); background:var(--lz-brand-soft); }
.task-launcher:focus-visible { outline:3px solid rgba(99,102,241,.28); outline-offset:3px; }
.task-icon { width:44px; height:44px; display:grid; place-items:center; border:1px solid #c7d2fe; border-radius:13px; color:var(--lz-brand-strong); background:var(--lz-brand-soft); }
.task-copy { min-width:0; display:flex; flex-direction:column; }
.task-meta { margin-bottom:7px; color:var(--lz-brand-strong); font-size:var(--text-xs); font-weight:600; letter-spacing:.02em; }
.task-copy strong { display:block; overflow-wrap:anywhere; color:var(--lz-text-strong); font-size:var(--text-base); font-weight:600; line-height:1.55; }
.task-copy small { margin-top:6px; color:var(--lz-text-secondary); font-size:var(--text-xs); line-height:1.45; }
.task-action { display:inline-flex; align-items:center; gap:6px; padding:8px 10px; border-radius:9px; color:var(--lz-brand-strong); background:var(--lz-brand-soft); font-size:var(--text-xs); font-weight:750; white-space:nowrap; }
...
.section-generation-status { width:max-content; display:inline-flex; align-items:center; gap:4px; margin-top:2px; padding:3px 7px; border:1px solid #dbe2ee; border-radius:999px; color:#64748b; background:#f8fafc; font-size:var(--text-xs); font-weight:700; }
...
.task-launcher:active { background:var(--lz-brand-soft); }
.task-action { font-size:var(--text-sm); }
@media (prefers-reduced-motion:reduce) { .course-node,.task-launcher,.section-content { transition:none; } }
