<template>
  <section class="companion-studio">
    <div v-if="loading" class="studio-state" role="status"><LoaderCircle :size="20" class="spin" />{{ t('companionDocuments.loading', '正在读取配套文档模板…') }}</div>
    <div v-else-if="loadError" class="studio-state studio-state--error" role="alert"><TriangleAlert :size="21" /><strong>{{ t('companionDocuments.loadFailed', '配套文档读取失败') }}</strong><span>{{ loadError }}</span><button type="button" @click="loadStudio">{{ t('common.retry', '重试') }}</button></div>
    ...
    <template v-else-if="!activeTemplate">
      ...
      <div class="template-grid">
        <button v-for="template in templates" :key="template.template_id" type="button" class="template-card" @click="openTemplate(template)">
          ...
          <span class="template-copy"><strong>{{ templateName(template) }}</strong><small>{{ templateDescription(template) }}</small></span>
    ...
    <form v-else class="document-form" @submit.prevent="generateDocument">
      ...
      <div class="form-body">
        ...
        <template v-if="activeTemplate.form_kind === 'grading_rubric'">
          <section class="form-section">
            <header><div><strong>{{ t('companionDocuments.rubric.components', '考核项目') }}</strong><small>{{ t('companionDocuments.rubric.componentsHelp', '各项比例合计必须为 100%') }}</small></div><span class="weight-total" :data-valid="rubricTotal === 100">{{ rubricTotal }}%</span></header>
        ...
        <template v-else-if="activeTemplate.form_kind === 'material_checklist'">
          ...
          <section class="form-section checklist-section">
            <header><div><strong>{{ t('companionDocuments.checklist.items', '材料清单') }}</strong><small>{{ t('companionDocuments.checklist.itemsHelp', '只勾选已经确认齐全的材料') }}</small></div><span>{{ checklistCompleted }}/{{ checklistItems.length }}</span></header>
      ...
      <footer class="form-footer"><span>{{ t('companionDocuments.sourceHint', '右侧资料会作为引用依据，并保留来源关系') }}</span><button class="primary-action" type="submit" :disabled="saving || !canSubmit"><LoaderCircle v-if="saving" :size="16" class="spin" /><Sparkles v-else :size="16" />{{ saving ? t('companionDocuments.saving', '正在生成…') : t('companionDocuments.generateAndSave', '生成并保存') }}</button></footer>
    </form>
  </section>
</template>
...
function templateDescription(template: Template) { return activeLocale.value === 'en' ? template.description_en : template.description }
...
async function loadStudio() {
  ...
  try {
    const response = await http.get(`/api/courses/${props.courseId}/companion-documents`, teacherRequestConfig({ silentError: true }))
