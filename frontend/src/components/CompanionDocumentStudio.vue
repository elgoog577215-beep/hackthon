<template>
  <section class="companion-studio">
    <div v-if="loading && !templates.length && !documents.length" class="studio-state" role="status"><LoaderCircle :size="20" class="spin" />{{ loadingLabel }}</div>
    <div v-else-if="loadError && !templates.length && !documents.length" class="studio-state studio-state--error" role="alert"><TriangleAlert :size="21" /><strong>{{ loadFailedLabel }}</strong><span>{{ loadError }}</span><button type="button" @click="loadStudio">{{ t('common.retry', '重试') }}</button></div>
    ...
    <template v-else-if="!activeTemplate">
      <div v-if="!showTemplatePicker" class="studio-state studio-state--error" role="alert">
        <TriangleAlert :size="21" />
        <strong>{{ t('companionDocuments.templateUnavailable', '所选模板暂不可用') }}</strong>
        <button type="button" @click="loadStudio">{{ t('common.retry', '重试') }}</button>
      </div>
      <template v-else>
...
const props = withDefaults(defineProps<{ courseId: string; templateId?: string; showTemplatePicker?: boolean }>(), {
  templateId: '',
...
const loadingLabel = computed(() => props.showTemplatePicker
  ? t('companionDocuments.loading', '正在读取配套文档模板…')
  : t('companionDocuments.loadingFile', '正在读取课程文件…'))
const loadFailedLabel = computed(() => props.showTemplatePicker
  ? t('companionDocuments.loadFailed', '配套文档读取失败')
  : t('companionDocuments.loadFileFailed', '课程文件读取失败'))
...
function openRequestedTemplate() {
  const template = templates.value.find(item => item.template_id === props.templateId)
  if (template) {
...
async function loadStudio() {
  ...
  try {
    ...
    if (props.templateId) openRequestedTemplate()
    else if (activeTemplate.value) openTemplate(activeTemplate.value)
  } catch (error: any) { loadError.value = String(error?.response?.data?.detail || error?.message || loadFailedLabel.value) }
...
watch(() => props.templateId, () => { if (props.templateId) openRequestedTemplate(); else if (props.showTemplatePicker) backToTemplates() })
</script>
...
.document-toolbar.is-direct-template{grid-template-columns:minmax(0,1fr) auto}
