        ...
        <div class="research-body">
          <aside class="research-brief">
            ...
            <button class="search-action" type="button" :disabled="searching || brief.trim().length < 2" @click="runSearch">
              ...
              {{ searching ? t('courseWorkbench.webResearch.searching', '正在检索、深读与整理…') : t('courseWorkbench.webResearch.search', '开始检索') }}
            </button>
            ...
            <div v-else class="source-results">
              <label v-for="source in session.results" :key="source.source_id" class="source-result" :class="{ selected: selectedIds.has(source.source_id) }">
                ...
                <div>
                  ...
                  <p>{{ source.text }}</p>
                  <footer>
                    <span v-if="source.content_status === 'full_text'" class="deep-read"><BookOpenCheck :size="12" />{{ t('courseWorkbench.webResearch.deepRead', '正文已深读') }}</span>
...
type WebResearchSource = {
  ...
  license?: string
  source_type?: string
  content_status?: 'full_text' | 'excerpt_fallback' | 'not_read'
...
const error = ref('')
const resultSummary = computed(() => t('courseWorkbench.webResearch.resultSummary', '{count} 个来源 · {full} 篇已深读 · {domains} 个网域 · {rejected} 个低相关结果已过滤')
  .replace('{count}', String(session.value?.results.length || 0))
  .replace('{full}', String(session.value?.research_summary?.full_text_count || 0))
  .replace('{domains}', String(session.value?.research_summary?.domain_count || 0))
  .replace('{rejected}', String(session.value?.rejected_count || 0)))
