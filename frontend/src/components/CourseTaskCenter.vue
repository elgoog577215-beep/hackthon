          ...
          <main v-if="selectedTask" class="task-detail">
            ...
            >
              ...
              <ul v-if="webSearchSummary.sources.length" class="web-search-summary__sources">
                <li v-for="source in webSearchSummary.sources" :key="source.url">
                  <a :href="source.url" target="_blank" rel="noopener noreferrer nofollow">{{ source.title }}</a>
                  <small>
                    {{ t('courseGeneration.materials.webSearch.trust', '可信度') }}：{{ t(`courseGeneration.materials.webSearch.trustLevel.${source.credibility}`, source.credibility) }}
                    · {{ t('courseGeneration.materials.webSearch.fetchedAt', '抓取时间') }}：{{ source.retrievedAt }}
                  </small>
                </li>
              </ul>
...
const webSearchSummary = computed(() => {
  ...
  return {
    ...
    sources: sources.map((item: Record<string, any>) => ({
      url: String(item.url || ''),
      title: String(item.title || item.domain || item.url || ''),
...
.web-search-summary__rejected li { display:grid; gap:2px; font-size:11px; overflow-wrap:anywhere; }
