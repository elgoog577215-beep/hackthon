      ...
      <div v-if="modelValue" class="course-adjustment-layer" @keydown="handleKeydown">
        ...
        <section ref="workspaceRef" class="course-adjustment-workspace" role="dialog" aria-modal="true" :aria-labelledby="titleId" tabindex="-1">
          <header class="course-adjustment-header">
            ...
            <span class="course-adjustment-state" :data-state="workspaceState">{{ workspaceStateLabel }}</span>
          ...
          <div :class="['course-adjustment-body', `course-adjustment-body--${workspaceState}`]">
            ...
            <main v-else-if="workspaceState === 'interpreting'" class="workspace-single workspace-state-interpreting">
              <section class="intent-review-card">
                <header class="state-heading">
                  ...
                  <div><small>{{ t('courseEvolution.workspace.interpretingKicker', 'AI 正在理解') }}</small><h3>{{ t('courseEvolution.workspace.interpretingTitle', '先确认目标有没有理解偏') }}</h3></div>
                </header>
                <div class="intent-comparison">
                  <article><small>{{ t('courseEvolution.workspace.teacherRequest', '老师原话') }}</small><p>{{ rawRequest }}</p></article>
                  <ArrowRight :size="18" />
                  <article><small>{{ t('courseEvolution.workspace.aiInterpretation', 'AI 当前的理解') }}</small><p>{{ interpretedGoal }}</p></article>
                </div>
                <ul v-if="protectedRequirements.length" class="protected-list"><li v-for="item in protectedRequirements" :key="item"><ShieldCheck :size="14" />{{ item }}</li></ul>
                ...
                <button v-if="!correctionOpen" type="button" class="secondary-action" @click="openCorrection"><PencilLine :size="15" />{{ t('courseEvolution.workspace.correctUnderstanding', '修正 AI 的理解') }}</button>
                <form v-else class="understanding-correction" @submit.prevent="submitCorrection">
                  <label><span>{{ t('courseEvolution.workspace.correctionLabel', '请直接说哪里理解错了') }}</span><textarea v-model="correctionText" rows="3" :placeholder="t('courseEvolution.workspace.correctionPlaceholder', '例如：不是删除案例，而是把案例移到新章节并保留原始资料。')" /></label>
            ...
            <main v-else-if="workspaceState === 'scanning'" class="workspace-single workspace-state-scanning">
              <section class="scan-progress-card" aria-live="polite">
                ...
                <section v-if="rawRequest || interpretedGoal" class="compact-understanding">
                  <p><b>{{ t('courseEvolution.workspace.teacherRequest', '老师原话') }}</b>{{ rawRequest }}</p>
                  <p><b>{{ t('courseEvolution.workspace.aiInterpretation', 'AI 当前的理解') }}</b>{{ interpretedGoal }}</p>
                </section>
            ...
            <div v-else-if="workspaceState === 'content'" class="workspace-two-column workspace-state-content">
              ...
              <main class="content-diff-review">
                <section class="understanding-bar"><div><small>{{ t('courseEvolution.workspace.aiInterpretation', 'AI 当前的理解') }}</small><p>{{ interpretedGoal }}</p></div><button type="button" class="text-action" @click="openCorrection">{{ t('courseEvolution.workspace.adjustUnderstanding', '调整理解') }}</button></section>
                <form v-if="correctionOpen" class="understanding-correction inline-correction" @submit.prevent="submitCorrection"><label><span>{{ t('courseEvolution.workspace.correctionLabel', '请直接说哪里理解错了') }}</span><textarea v-model="correctionText" rows="3" :placeholder="t('courseEvolution.workspace.correctionPlaceholder', '例如：不是删除案例，而是把案例移到新章节并保留原始资料。')" /></label><div><button type="button" class="text-action" @click="correctionOpen = false">{{ t('common.cancel', '取消') }}</button><button type="submit" class="primary-action" :disabled="store.generating || !correctionText.trim()"><LoaderCircle v-if="store.generating" :size="15" class="spinning" /><Check v-else :size="15" />{{ t('courseEvolution.workspace.submitCorrection', '按修正重新理解') }}</button></div></form>
            ...
            <main v-else class="workspace-single workspace-state-applied">
              <section class="application-receipt" :data-status="focusedPlan?.status">
                ...
                <p class="receipt-request">{{ rawRequest || focusedPlan?.request_text }}</p>
...
const workspaceStateLabel = computed(() => ({ request: t('courseEvolution.workspace.stateRequest', '等待要求'), interpreting: t('courseEvolution.workspace.stateInterpreting', '正在理解'), scanning: t('courseEvolution.workspace.stateScanning', '正在扫描'), content: t('courseEvolution.workspace.stateContent', '内容审阅'), structure: t('courseEvolution.workspace.stateStructure', '结构审阅'), applied: t('courseEvolution.workspace.stateApplied', '执行回执') })[workspaceState.value])
