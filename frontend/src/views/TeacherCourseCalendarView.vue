          ...
          <div v-else-if="editable" class="calendar-board" :class="{ 'is-table': view === 'table' }">
            ...
            <section class="calendar-canvas">
              ...
              <div v-else class="table-wrap">
                ...
                <button v-if="!editable.sessions.length" type="button" class="empty-action" @click="deriveFromOutline"><Sparkles :size="18" /><strong>还没有课次</strong><span>从教学大纲生成候选，或手动新增一条。</span></button>
              </div>
            </section>
            ...
            <aside v-if="view === 'month'" class="session-inspector" aria-label="当前课次编辑">
              <template v-if="selectedSession">
                ...
                <footer><span>{{ selectedSession.source === 'outline' ? '来源：教学大纲' : '来源：手动创建' }}</span><button type="button" class="danger-button" @click="removeSession(selectedIndex!)"><Trash2 :size="14" />删除</button></footer>
              </template>
...
import { CalendarDays, CalendarRange, ChevronLeft, ChevronRight, LoaderCircle, Plus, RefreshCw, Save, Sparkles, Table2, Trash2, TriangleAlert } from 'lucide-vue-next'
...
async function deriveFromOutline() {
  ...
  try {
    ...
    const action = await ElMessageBox.confirm(`将形成 ${result.candidate_count} 条课次候选；保留人工课次 ${result.retained_count} 条；新增 ${result.new_count} 条。`, '采用大纲候选', { type: 'info', confirmButtonText: '采用候选', cancelButtonText: '取消' })
