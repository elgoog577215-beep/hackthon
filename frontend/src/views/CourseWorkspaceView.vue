  ...
  <main class="course-workspace-page">
    <Teleport to="#app-header-route-actions">
      <div class="workspace-route-actions">
        <span class="workspace-state" :data-state="courseState">{{ courseStateLabel }}</span>
        <button type="button" @click="workbenchOpen = true"><ListTodo :size="16" />{{ t('courseFiles.taskCenter') }}</button>
      </div>
    </Teleport>
...
import { ArrowLeft, ListTodo, LoaderCircle, TriangleAlert } from 'lucide-vue-next'
...
.back-button,.workspace-route-actions button { display:inline-flex; align-items:center; gap:7px; border:0; background:transparent; color:var(--lz-text-secondary); font-size:12px; font-weight:700; cursor:pointer; }
...
.workspace-route-actions { display:flex; align-items:center; gap:10px; }
.workspace-route-actions button { min-height:34px; padding:0 10px; border:1px solid var(--lz-border); border-radius:9px; background:#fff; }
