    ...
    <div class="production-shell">
      ...
      <main class="production-main">
        ...
        <div v-if="pageMode === 'production'" class="production-tabs" aria-label="课程生产分类">
          ...
          <span class="production-tabs__summary">已就绪 {{ confirmedStageCount }}/{{ stages.length }}</span>
          ...
          <section class="stage-workspace" :aria-label="activeStageLabel">
            ...
            <template v-else-if="activeStage === 'overview'">
              ...
              <div class="lesson-overview">
                <table data-testid="production-lesson-table">
                  ...
                  <tbody>
                    <tr v-for="(lesson, index) in lessons" :key="lesson.node_id" :class="{ selected: previewLesson?.node_id === lesson.node_id }">
                      <td><strong>{{ String(index + 1).padStart(2, '0') }}</strong></td>
                      <td><button type="button" class="lesson-link" @click="previewLesson = lesson">{{ lesson.node_name }}</button></td>
                      <td><span :data-state="lessonState(lesson)">{{ lessonState(lesson) }}</span></td>
                      <td><span :data-state="pptAvailable ? 'ready' : 'locked'">{{ pptAvailable ? '可制作' : '等待正式课程' }}</span></td>
                      <td>{{ isPublished ? '已发布' : '未发布' }}</td>
                      <td><button type="button" class="row-action" @click="previewLesson = lesson">快速预览</button></td>
                    </tr>
                  </tbody>
                </table>
                <section v-if="previewLesson" class="lesson-preview" data-testid="production-lesson-preview">
                  <header><div><small>第 {{ lessonNumber(previewLesson) }} 讲</small><h2>{{ previewLesson.node_name }}</h2></div><button type="button" aria-label="关闭预览" @click="previewLesson = null">×</button></header>
                  <div class="preview-body"><strong>教学内容预览</strong><p>{{ nodeSummary(previewLesson) }}</p></div>
                  <footer><button type="button" @click="continueTeaching(previewLesson)">继续制作教案</button><button type="button" :disabled="!pptAvailable" @click="continuePpt(previewLesson)">进入 PPT 工作台</button></footer>
                </section>
              </div>
            </template>
            ...
            <template v-else-if="activeStage === 'teaching'">
              ...
              <GenerationLessonPlan
                ...
                @ai-open-change="aiPanelOpen = $event"
              />
            </template>

            <template v-else-if="activeStage === 'ppt'">
              <header class="workspace-header"><div><small>下游表达</small><h1>课堂课件</h1></div><span :data-state="pptAvailable ? 'confirmed' : 'locked'">{{ pptAvailable ? '可以进入工作台' : '等待课程发布' }}</span></header>
              <div class="ppt-entry">
...
const nextActionLabel = computed(() => {
  if (isPublished.value) return '课程已发布，可继续维护'
...
const confirmedStageCount = computed(() => {
  if (isPublished.value && courseStore.currentDocumentRevision) return stages.length
  return Number(lessons.value.length > 0) + Number(stageStatus('teaching') === 'confirmed') + Number(pptAvailable.value)
})
...
const releaseRows = computed(() => [
  ...
  },
  {
    ...
    teacherState: pptAvailable.value ? 'confirmed' : 'pending', teacherLabel: pptAvailable.value ? '可进入工作台' : '尚未构建',
...
function workflowStatusLabel(value: string) {
  return ({ locked: '尚未开放', pending: '等待处理', in_progress: '进行中', waiting_for_confirmation: '等待确认', confirmed: '已确认', needs_regeneration: '需要重建', failed: '失败', auto_resuming: '自动恢复中', manual_resume: '等待手动恢复', quality_blocked: '质量检查阻断', conflict: '版本冲突', unavailable: '不可恢复', completed: '已完成' } as Record<string, string>)[value] || value
}
function stageStatus(key: StageKey) {
  if (key === 'overview') return lessons.value.length ? 'confirmed' : 'pending'
  if (key === 'ppt') return pptAvailable.value ? 'confirmed' : 'locked'
...
function nodeSummary(node: Node) {
  const firstBlock = node.content_blocks?.find(block => block.summary || block.content)
  return String(node.node_content || node.learning_objective || firstBlock?.summary || firstBlock?.content || '本讲已有教学结构，可进入教案工作区查看目标、策略、评价与课堂活动。').replace(/[#*_`]/g, '').slice(0, 420)
}
function continueTeaching(node: Node) { selectLesson(node); activeStage.value = 'teaching'; previewLesson.value = null }
function openTeachingAi() { void lessonPlanRef.value?.openAiAssistant('section') }
function continuePpt(node: Node) { selectLesson(node); openPpt() }
function openPpt() { if (pptAvailable.value) void router.push({ name: 'ppt-workspace', params: { courseId: courseId.value }, query: { returnTo: route.fullPath, node: selectedNodeId.value } }) }
...
async function loadCourse() {
  ...
  try {
    ...
    if (lessons.value[0] && !lessons.value.some(node => node.node_id === courseStore.currentNode?.node_id)) courseStore.selectNode(lessons.value[0])
  } catch (error) {
...
.exit-immersive { margin-left:auto; height:30px; padding:0 10px; border:1px solid var(--lz-border); border-radius:7px; color:var(--lz-text-secondary); background:var(--lz-surface); cursor:pointer; }
...
.lesson-overview tr.selected td { background:var(--lz-brand-soft); }.lesson-overview td:first-child strong { color:var(--lz-brand); }
...
@media (max-width:1180px) { .workspace-grid.immersive.with-ai-dock { grid-template-columns:176px minmax(0,1fr) 286px; } }
