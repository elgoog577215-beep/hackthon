<template>
  <section class="teacher-total-calendar">
    <header class="product-bar">
      ...
      <div class="product-actions"><button type="button" @click="load"><RefreshCw :size="17" :class="{ spin: store.loading }" />{{ t('common.refresh', '刷新') }}</button></div>
    </header>
    ...
    <main class="calendar-main">
      <div class="status-bar" role="status">
        <strong>{{ monthLabel }}</strong><span>{{ t('teacherCalendar.courseCount', '课程') }} {{ courseCount }}</span><span>{{ t('teacherCalendar.sessions', '课次') }} {{ store.totalSessions.length }}</span><span>{{ t('teacherCalendar.thisWeek', '本周') }} {{ weekSessions.length }}</span><span class="spacer"></span><span>{{ t('teacherCalendar.totalReadOnly', '在单课程日历中修改') }}</span>
      </div>
      ...
      <section class="workspace">
        <header class="toolbar">
          ...
          <button type="button" @click="movePeriod(-1)"><ChevronLeft :size="16" /></button><strong>{{ periodLabel }}</strong><button type="button" @click="movePeriod(1)"><ChevronRight :size="16" /></button><button type="button" @click="goToday">{{ t('teacherCalendar.today', '今天') }}</button>
        </header>
        ...
        <div v-else-if="view === 'month'" class="month-view">
          <TeachingCalendarMonthGrid :month="monthCursor" :sessions="store.totalSessions" show-course @select="openSession" />
          <div v-if="!store.totalSessions.length" class="calendar-empty-hint">
        ...
        <div v-else-if="view === 'week'" class="week-view">
          <section v-for="day in weekDays" :key="day.date">
            ...
            <button v-for="session in day.sessions" :key="session.session_id" type="button" :data-color="session.course_color_key ?? 0" @click="openSession(session)">
        ...
        <div v-else class="list-view">
          <table><thead><tr><th>{{ t('teacherCalendar.columns.date', '日期') }}</th><th>{{ t('teacherCalendar.course', '课程') }}</th><th>{{ t('teacherCalendar.columns.content', '教学内容') }}</th><th>{{ t('teacherCalendar.columns.location', '上课地点') }}</th><th>{{ t('teacherCalendar.columns.teacher', '教师') }}</th><th>{{ t('teacherCalendar.columns.type', '类型') }}</th></tr></thead>
            <tbody><tr v-for="session in store.totalSessions" :key="session.session_id" data-testid="total-calendar-row" tabindex="0" @click="openSession(session)" @keydown.enter="openSession(session)"><td><strong>{{ session.date }}</strong><small>{{ session.start_time?.slice(0, 5) }}<template v-if="session.end_time">—{{ session.end_time.slice(0, 5) }}</template></small></td><td><i :data-color="session.course_color_key ?? 0"></i>{{ session.course_title }}</td><td>{{ session.content_summary }}</td><td>{{ session.location || '—' }}</td><td>{{ session.teacher_name || '—' }}</td><td>{{ session.teaching_type }}</td></tr></tbody>
          </table>
          <div v-if="!store.totalSessions.length" class="empty-state">{{ t('teacherCalendar.totalEmpty', '当前范围还没有已排期课次') }}</div>
        </div>
      </section>
    </main>
  </section>
...
const weekDays = computed(() => Array.from({ length: 7 }, (_, index) => { const value = new Date(weekStart.value); value.setDate(value.getDate() + index); const date = iso(value); return { date, label: weekdayNames[index], sessions: store.totalSessions.filter(item => item.date === date) } }))
const weekSessions = computed(() => weekDays.value.flatMap(day => day.sessions))
...
async function load() { const range = loadRange(); try { await store.loadTotal(range.from, range.to) } catch { /* exact error remains visible */ } }
