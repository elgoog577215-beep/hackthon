...
.week-session{--course-color:#5147d9;--course-surface:color-mix(in srgb,var(--course-color) 7%,#fff);--course-border:color-mix(in srgb,var(--course-color) 18%,#e5e8ef);z-index:2;min-width:0;container-type:inline-size;display:grid;align-content:start;gap:4px;margin:5px;padding:8px 9px;overflow:hidden;border:1px solid var(--course-border);border-radius:12px;color:var(--lz-text-secondary);background:var(--course-surface);text-align:left;cursor:pointer;transition:transform .16s ease,border-color .16s ease,background-color .16s ease,box-shadow .16s ease}
...
.week-session:hover{z-index:4;transform:translateY(-1px);border-color:color-mix(in srgb,var(--course-color) 34%,#dfe3ea);background:#fff;box-shadow:0 8px 20px rgb(15 23 42 / 10%)}
.week-session.active{z-index:4;border-color:color-mix(in srgb,var(--course-color) 48%,#dfe3ea);background:#fff;box-shadow:0 9px 24px rgb(15 23 42 / 12%)}
...
.week-session strong,.week-session small{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.week-session strong{color:var(--lz-text-strong);font-size:11px;font-weight:740;line-height:1.45}.week-session small{color:var(--lz-text-secondary);font-size:10px;line-height:1.4}.week-session:focus-visible{z-index:4;outline:3px solid color-mix(in srgb,var(--course-color) 28%,transparent);outline-offset:1px}
@container(max-width:135px){.week-session time{grid-template-columns:auto minmax(0,1fr)}.week-session time span{display:none}}
