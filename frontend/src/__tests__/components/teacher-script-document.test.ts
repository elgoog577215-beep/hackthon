...
describe('统一讲义页面', () => {
  ...
  it('无修改不写新版本，保存失败保留草稿并返回失败', async () => {
    const store = useTeacherLessonAuthoringStore()
    const save = vi.spyOn(store, 'saveScriptDraft').mockRejectedValue(new Error('保存失败'))
    const wrapper = mount(TeacherScriptDocument, { props: { courseId: 'course-1', lesson: structuredClone(lesson) } })
