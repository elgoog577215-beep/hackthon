...
describe('course library density contract', () => {
  it('preserves the original course library shell measurements', () => {
    ...
    expect(librarySource).toMatch(/\.library-toolbar__summary\s*\{[^}]*justify-self:\s*end/s)
  ...
  it('uses two readable teacher-workbench cards and gives a single course enough width', () => {
    ...
    expect(librarySource).toMatch(/\.course-collection\s*\{[^}]*max-width:\s*1040px[^}]*margin:\s*0\s+auto/s)
    expect(librarySource).toMatch(/\.course-grid\s*\{[^}]*margin:\s*14px\s+0\s+0[^}]*justify-content:\s*start/s)
    expect(librarySource).toMatch(/\.course-grid\s*\{[^}]*grid-template-columns:\s*repeat\(2,minmax\(0,1fr\)\)/s)
    expect(librarySource).toMatch(/\.course-grid:has\(\.course-item:only-child\)\s*\{[^}]*grid-template-columns:\s*minmax\(0,1fr\)/s)
    ...
    expect(librarySource).toMatch(/@media\s*\(max-width:860px\)\s*\{[^}]*\.course-collection\s*\{[^}]*max-width:\s*620px/s)
    expect(librarySource).toMatch(/@media\s*\(max-width:860px\)[\s\S]*?\.course-grid\s*\{[^}]*grid-template-columns:\s*minmax\(0,1fr\)/s)
