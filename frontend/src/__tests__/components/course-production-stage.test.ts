...
describe('CourseProductionStage', () => {
  ...
  it('在课程现场解释中断、保存边界和恢复动作', async () => {
    ...
    const journey = wrapper.findAll('.formation-journey li')
    expect(journey).toHaveLength(3)
    expect(journey[0]!.text()).toContain('课程结构')
    expect(journey[0]!.attributes('data-state')).toBe('error')
    expect(journey[1]!.text()).toContain('教案与知识')
    expect(journey[2]!.text()).toContain('课程内容')
  ...
  it('标题下方用可播报的实时阶段摘要取代静态说明', async () => {
    ...
    expect(wrapper.get('.formation-outline').attributes()).not.toHaveProperty('open')
    expect(wrapper.findAll('.formation-journey li')[1]!.attributes('data-state')).toBe('active')
