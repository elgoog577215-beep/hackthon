export type TeacherLessonAiPhase =
  | 'ready'
  | 'clarifying'
  | 'generating'
  | 'review'
...
export type TeacherLessonAiEvent =
...
const GENERATION_START_PHASES = new Set<TeacherLessonAiPhase>([
  'ready',
  'clarifying',
  'review',
  'success',
...
export function transitionTeacherLessonAiPhase(
  phase: TeacherLessonAiPhase,
  event: TeacherLessonAiEvent,
...
export function teacherLessonAiBusy(phase: TeacherLessonAiPhase): boolean {
...
export function assessTeacherLessonRequest(value: string): 'clarify' | 'generate' {
  ...
  const target = /(目标|重点|难点|知识|概念|案例|互动|活动|检查|评价|提问|练习|节奏|时间|讲授|作业|备注|教师|学生|导入|总结|迁移)/
  const concreteAction = /(增加|减少|删除|压缩|延长|改成|改为|突出|补充|替换|重写|调整|优化|细化|合并|拆分|保留|对齐|强化|弱化)/
...
function compactTeacherTurns(messages: TeacherLessonAiMessage[], budget = 1080): string[] {
...
export function buildTeacherLessonAiInstruction(
  ...
  return [
    ...
    `范围：课程“${scope.courseTitle}”；讲次“${scope.lessonTitle}”；小节“${scope.sectionTitle || '整讲教案'}”；当前纳入 ${scope.referenceCount} 份资料。`,
    `教师对话（越靠后优先级越高）：\n${requirements}`,
    '修改原则：只修改实现教师要求所必需的字段，未涉及内容保持原样；不得改写知识事实、稳定 ID、小节数量、教学环节身份和顺序。',
    '教学原则：目标应可观察；活动和课堂检查应与目标对应；除非教师明确要求调整节奏，否则保持原有总时长。',
    '输出边界：只生成候选，不确认、不发布、不生成学生课程正文。',
  ].join('\n')
}
