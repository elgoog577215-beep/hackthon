...
## 15. 当前实施证据（2026-09-06，仍在推进）

- 已完成项仅表示对应合同或局部机制有证据；不代表第 14 节通过。新建开关默认关闭，未部署。
- 新内容合同与比较身份、分支端点、精确引用、已采用素材摘要、确认后零模型调用已有针对性测试；抽取共享类型后 196 项合同及新链测试通过，相关原链路 373 项通过。后端依赖检查覆盖 304 个模块，循环引用为 0。前端构建通过。
- `academic-editorial` 的十个组件版式共 28 张物理合成测试页已通过原生对象回读、LibreOffice/PDF/PNG 检查，教师评阅仍 pending。源字体及 Python 包版本已固定，Linux 部署工具依赖与跨平台认证尚未完成，因此 9.4 保持未勾选。
- 真实线性代数讲义在隔离副本中通过来源准入，整讲规划为 21 个初始页任务；逐页阶段仍在容量修复。此前单块摘录比较样板已导出并检查三张静态状态页，但它不代替 12.3 的完整教师路径。
- 中文真实组件浏览器检查已证明比较维度修改进入保存事件、修改后禁止确认以及三张已保存状态预览；保存事件未写入正式教师 API，因此 14.3 保持未勾选。
- 当前真实讲义有将矛盾行与非阶梯形混淆的来源问题；原讲义未修改。来源忠实与知识正确分开验收，不能把该课件标为教师评阅通过。
- 多页任务组已接入初始规划与选定页重生，仍需真实模型覆盖；原生模板的静态背景、连线/图片替换、个人模板注册发布，数据图表/第二主题，以及故障矩阵仍在待办中。
