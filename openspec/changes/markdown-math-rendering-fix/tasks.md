...
## 4. Extend tolerance for mixed legacy content

- [x] 4.1 Normalize exactly doubled LaTeX command escapes without corrupting matrix row separators.
- [x] 4.2 Split delimiter-free equations from trailing Chinese prose so only the equation enters KaTeX.
- [x] 4.3 Merge display-matrix environments nested inside legacy inline `\\left...\\right` formulas into one valid display expression.
- [x] 4.4 Route inline AI learning-record summaries through the shared Markdown/KaTeX renderer.
- [x] 4.5 Verify the real 383-node linear-algebra course has zero formula fallbacks and zero leaked raw math markers.
