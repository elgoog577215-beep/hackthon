...
任何知识结构变化 MUST 计算对正文块、课件、学习目标、正式题目、能力、易错、掌握标准和绑定的影响；任何改变当前课程语义覆盖的正文变化 MUST 检查课程知识库是否需要补写、细化或调整关系。知识语义变化 MUST 进入维护者 `CourseAuthoringChange`；`CourseEvolutionPlan` MAY 在确认后修改当前课程正文，但 MUST NOT 直接写入课程知识库。
...
### Requirement: 当前课程维护联动必须具有单一因果链和循环抑制

同一课程维护变化 MUST 使用一个 `authoring_change_id`、因果来源和依赖图。系统 MUST 对重复请求幂等处理，MUST NOT 将知识变化引发的课程变化再次识别为一条全新的相同知识变化。课程生长若触及知识语义，只可读取影响结果或生成当前课程维护者建议。

#### Scenario: 知识拆分引发课程补写

- **WHEN** 课程补写由同一当前课程维护候选中的知识拆分操作产生
