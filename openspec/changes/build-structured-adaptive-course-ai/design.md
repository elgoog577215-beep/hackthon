...
- 跨课程术语复用可以通过可选参考和显式转换完成，不需要让学生运行时依赖第二套知识身份。
- 外部参考缺失、冲突或版本落后时，当前课程仍必须独立通过质量门并正常工作。
...
- **[风险] 旧跨课程设施重新成为隐藏真源** → 外部参考只输出建议，不参与当前课程 ID、发布、AI 使用和掌握门禁；契约测试覆盖参考缺失与冲突。
- **[风险] 知识库再次退化成章节标题复制** → 首次生成先产出知识蓝图，原子性与模板内容作为硬门；历史课程无法确认时显式标记 `degraded`。
