...
- [x] 14.2 Reconcile teacher-outline completion through the teacher projection and expose the first lesson as the next explicit action without auto-advancing the page.
...
- [x] 14.4 Show lesson-plan job progress and failures, prevent duplicate submission, make a valid result the current revision, and bind script generation to that revision without an artificial confirmation state.
...
- [x] 15.1 Keep lesson-plan viewing, editing and AI candidates in one document surface; use current-revision save and explicit candidate adoption instead of a lesson-plan confirmation action.
- [x] 15.2 Add one shared previous/current/next lesson navigator for lesson plans, question bank, scripts and PPT handoff.
- [x] 15.3 Store one formal script asset per lesson, generate it from the current valid lesson plan and teacher requirements, support inline editing and AI candidates, and make a structurally valid result the current script revision.
...
- [x] 15.5 Bind each V6 PPT revision to the current valid script revision and its transitive lesson-plan source; mark it stale when either source changes.
