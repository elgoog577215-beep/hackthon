<template>
  <section ref="documentRoot" class="lesson-document" :class="{ 'is-ai-candidate': pendingCandidate }">
    ...
    <span v-if="pendingCandidate && !inlineCandidateInPlace" class="sr-only" role="status">{{ tr('courseWorkbench.lessonDocument.candidateCanvasTitle') }}</span>
    <div
      v-if="pendingCandidate && !inlineCandidateInPlace"
      class="candidate-canvas-notice"
      role="region"
      :aria-label="tr('courseWorkbench.lessonDocument.candidateCanvasTitle')"
    >
      <div>
        <Sparkles :size="16" />
        <div class="candidate-canvas-notice__copy">
          ...
          <details class="candidate-provenance">
            <summary>{{ candidateOriginLabel }} · {{ candidateCreatedAt }}</summary>
            <p class="candidate-provenance__instruction">{{ pendingCandidate.instruction }}</p>
          </details>
        </div>
      </div>
    ...
    <TextSelectionAiAction
      ...
      group-selector="[data-ai-wide-anchor]"
    ...
    <article v-if="planSections.length" class="document-body">
      ...
      >
        ...
        <section class="document-section objective-section">
          ...
          <div class="objective-grid" :class="{ 'is-ai-reviewing': candidateObjectivesChanged(section) }" data-ai-inline-anchor data-ai-wide-anchor>
            <div data-ai-block data-ai-field="knowledge_objectives" :data-ai-label="tr('courseWorkbench.lessonDocument.knowledgeObjective')" :class="{ 'ai-change-target': candidateChanged(section, 'knowledge_objectives') }"><i v-if="candidateChanged(section, 'knowledge_objectives')" class="ai-change-marker">{{ tr('courseWorkbench.lessonDocument.changeMarker') }}</i><h5>{{ tr('courseWorkbench.lessonDocument.knowledgeObjective') }}</h5><textarea v-if="editing" :value="listText(section.knowledge_objectives)" rows="3" @input="updateList(section, 'knowledge_objectives', $event)" /><ul v-else-if="sectionKnowledgeObjectives(section).length"><li v-for="(item, itemIndex) in sectionKnowledgeObjectives(section)" :key="`${itemIndex}-${item}`" data-ai-field="knowledge_objectives" :data-ai-item-id="String(itemIndex)" :data-ai-label="tr('courseWorkbench.lessonDocument.knowledgeObjective')" :class="{ 'ai-change-target': candidateListItemChanged(section, 'knowledge_objectives', itemIndex) }"><MathText :content="item" /></li></ul><p v-else>{{ emptyValue }}</p></div>
            <div data-ai-block data-ai-field="ability_objectives" :data-ai-label="tr('courseWorkbench.lessonDocument.abilityObjective')" :class="{ 'ai-change-target': candidateChanged(section, 'ability_objectives') }"><i v-if="candidateChanged(section, 'ability_objectives')" class="ai-change-marker">{{ tr('courseWorkbench.lessonDocument.changeMarker') }}</i><h5>{{ tr('courseWorkbench.lessonDocument.abilityObjective') }}</h5><textarea v-if="editing" :value="listText(section.ability_objectives)" rows="3" @input="updateList(section, 'ability_objectives', $event)" /><ul v-else-if="sectionAbilityObjectives(section).length"><li v-for="(item, itemIndex) in sectionAbilityObjectives(section)" :key="`${itemIndex}-${item}`" data-ai-field="ability_objectives" :data-ai-item-id="String(itemIndex)" :data-ai-label="tr('courseWorkbench.lessonDocument.abilityObjective')" :class="{ 'ai-change-target': candidateListItemChanged(section, 'ability_objectives', itemIndex) }"><MathText :content="item" /></li></ul><p v-else>{{ emptyValue }}</p></div>
            <div data-ai-block data-ai-field="education_objectives" :data-ai-label="tr('courseWorkbench.lessonDocument.educationObjective')" :class="{ 'ai-change-target': candidateChanged(section, 'education_objectives') }"><i v-if="candidateChanged(section, 'education_objectives')" class="ai-change-marker">{{ tr('courseWorkbench.lessonDocument.changeMarker') }}</i><h5>{{ tr('courseWorkbench.lessonDocument.educationObjective') }}</h5><textarea v-if="editing" :value="listText(section.education_objectives)" rows="3" @input="updateList(section, 'education_objectives', $event)" /><ul v-else-if="stringList(section.education_objectives).length"><li v-for="(item, itemIndex) in stringList(section.education_objectives)" :key="`${itemIndex}-${item}`" data-ai-field="education_objectives" :data-ai-item-id="String(itemIndex)" :data-ai-label="tr('courseWorkbench.lessonDocument.educationObjective')" :class="{ 'ai-change-target': candidateListItemChanged(section, 'education_objectives', itemIndex) }"><MathText :content="item" /></li></ul><p v-else>{{ emptyValue }}</p></div>
          </div>
        </section>
        ...
        <section class="document-section">
          ...
          <div class="focus-grid" data-ai-inline-anchor data-ai-wide-anchor>
            <div data-ai-block data-ai-field="key_points" :data-ai-label="tr('courseWorkbench.lessonDocument.keyPoints')" :class="{ 'ai-change-target': candidateChanged(section, 'key_points') }"><i v-if="candidateChanged(section, 'key_points')" class="ai-change-marker">{{ tr('courseWorkbench.lessonDocument.changeMarker') }}</i><h4>{{ tr('courseWorkbench.lessonDocument.keyPoints') }}</h4><textarea v-if="editing" :value="listText(section.key_points)" rows="3" @input="updateList(section, 'key_points', $event)" /><ul v-else-if="stringList(section.key_points).length"><li v-for="(item, itemIndex) in stringList(section.key_points)" :key="`${itemIndex}-${item}`" data-ai-field="key_points" :data-ai-item-id="String(itemIndex)" :data-ai-label="tr('courseWorkbench.lessonDocument.keyPoints')" :class="{ 'ai-change-target': candidateListItemChanged(section, 'key_points', itemIndex) }"><MathText :content="item" /></li></ul><p v-else>{{ emptyValue }}</p></div>
            <div data-ai-block data-ai-field="key_difficulties" :data-ai-label="tr('courseWorkbench.lessonDocument.difficulties')" :class="{ 'ai-change-target': candidateChanged(section, 'key_difficulties') }"><i v-if="candidateChanged(section, 'key_difficulties')" class="ai-change-marker">{{ tr('courseWorkbench.lessonDocument.changeMarker') }}</i><h4>{{ tr('courseWorkbench.lessonDocument.difficulties') }}</h4><textarea v-if="editing" :value="listText(section.key_difficulties)" rows="3" @input="updateList(section, 'key_difficulties', $event)" /><ul v-else-if="stringList(section.key_difficulties).length"><li v-for="(item, itemIndex) in stringList(section.key_difficulties)" :key="`${itemIndex}-${item}`" data-ai-field="key_difficulties" :data-ai-item-id="String(itemIndex)" :data-ai-label="tr('courseWorkbench.lessonDocument.difficulties')" :class="{ 'ai-change-target': candidateListItemChanged(section, 'key_difficulties', itemIndex) }"><MathText :content="item" /></li></ul><p v-else>{{ emptyValue }}</p></div>
          </div>
        </section>
        ...
        <section class="document-section flow-section">
          ...
          <div class="teaching-block-list">
            <article v-for="(module, index) in sectionModules(section)" :key="module.arrangement_block_id || module.module_id || index" class="teaching-block" data-ai-inline-anchor :class="{ 'is-overtime': isOvertimeModule(section, module, index) }">
              ...
              <div v-else class="lesson-block-summary" data-ai-inline-anchor data-ai-wide-anchor>
        ...
        <section class="document-section lesson-time-section" data-period="after" data-ai-field="homework" data-ai-inline-anchor :data-ai-label="tr('courseWorkbench.lessonDocument.homework')" :class="{ 'ai-change-target': candidateFieldChanged(section, 'homework') }">
          ...
          <div class="assignment-contract" data-ai-inline-anchor data-ai-wide-anchor>
        ...
        <section class="document-section materials-record"><h4>{{ tr('courseWorkbench.lessonDocument.materialsAndRecords') }}</h4><div class="closing-grid" data-ai-inline-anchor data-ai-wide-anchor><div data-ai-field="resource_refs" :data-ai-label="tr('courseWorkbench.lessonDocument.extensionReading')" :class="{ 'ai-change-target': candidateFieldChanged(section, 'resource_refs') }"><i v-if="candidateFieldChanged(section, 'resource_refs')" class="ai-change-marker">{{ tr('courseWorkbench.lessonDocument.changeMarker') }}</i><h4>{{ tr('courseWorkbench.lessonDocument.extensionReading') }}</h4><textarea v-if="editing" :value="listText(section.resource_refs)" rows="4" @input="updateList(section, 'resource_refs', $event)" /><ul v-else-if="stringList(section.resource_refs).length"><li v-for="(item, itemIndex) in stringList(section.resource_refs)" :key="`${itemIndex}-${item}`" data-ai-field="resource_refs" :data-ai-item-id="String(itemIndex)" :data-ai-label="tr('courseWorkbench.lessonDocument.extensionReading')" :class="{ 'ai-change-target': candidateListItemChanged(section, 'resource_refs', itemIndex) }"><MathText :content="item" /></li></ul><p v-else>{{ emptyValue }}</p></div><div><h4>{{ tr('courseWorkbench.lessonDocument.activityPhotos') }}</h4><p>{{ tr('courseWorkbench.lessonDocument.activityPhotosPending') }}</p></div></div></section>
...
const fallbackMessages: Record<string, string> = {
  ...
  'courseWorkbench.lessonDocument.candidateOriginInline': '文中修改',
  'courseWorkbench.lessonDocument.candidateOriginAssistant': '教案助手',
  'courseWorkbench.lessonDocument.candidateOriginCourseChange': '课程调整',
  'courseWorkbench.lessonDocument.candidateOriginLegacy': '历史候选（来源未记录）',
  'courseWorkbench.lessonDocument.candidateCreatedUnknown': '时间未记录',
...
const candidateOriginLabel = computed(() => {
  const labels: Record<string, string> = {
    inline_edit: tr('courseWorkbench.lessonDocument.candidateOriginInline'),
    assistant: tr('courseWorkbench.lessonDocument.candidateOriginAssistant'),
    course_change: tr('courseWorkbench.lessonDocument.candidateOriginCourseChange'),
  }
  return labels[pendingCandidate.value?.origin || '']
    || tr('courseWorkbench.lessonDocument.candidateOriginLegacy')
})
const candidateCreatedAt = computed(() => {
  const value = String(pendingCandidate.value?.created_at || '').trim()
  if (!value) return tr('courseWorkbench.lessonDocument.candidateCreatedUnknown')
...
function candidateObjectivesChanged(section: Record<string, any>): boolean {
  return ['knowledge_objectives', 'ability_objectives', 'education_objectives']
    .some(key => rawCandidateChanged(section, key))
}
...
.candidate-canvas-notice{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;padding:12px 28px;border-bottom:1px solid #d9ddf5;color:#4338ca;background:#f5f5ff}.candidate-canvas-notice>div{min-width:0;display:flex;align-items:flex-start;gap:9px}.candidate-canvas-notice>div>.candidate-canvas-notice__copy{min-width:0;display:grid;gap:2px}.candidate-canvas-notice strong{font-size:15px}.candidate-canvas-notice small{color:#676aa0;font-size:12px}.candidate-provenance{margin-top:4px;color:#555a89;font-size:12px}.candidate-provenance summary{width:max-content;max-width:100%;cursor:pointer;font-weight:700}.candidate-provenance summary:focus-visible{outline:2px solid #5148dc;outline-offset:2px}.candidate-provenance__instruction{max-width:680px;margin:7px 0 0;padding:8px 10px;border-left:2px solid #8b88df;border-radius:0 6px 6px 0;color:#414866;background:rgba(255,255,255,.72);font-size:13px;line-height:1.55;overflow-wrap:anywhere}.candidate-canvas-notice nav{flex:none;display:flex;align-items:center;gap:6px}.candidate-canvas-notice button{min-width:24px;min-height:32px;display:inline-flex;align-items:center;gap:5px;padding:0 9px;border:1px solid #d0d1ee;border-radius:7px;color:#4f55a9;background:#fff;font-size:11px;font-weight:750;cursor:pointer}.candidate-canvas-notice button.primary{border-color:#5148dc;color:#fff;background:#5148dc}.candidate-canvas-notice button:hover:not(:disabled){border-color:#9692e8;color:#4338ca;background:#f8f7ff}.candidate-canvas-notice button.primary:hover:not(:disabled){border-color:#433bc4;color:#fff;background:#433bc4}.candidate-canvas-notice button:focus-visible{outline:3px solid rgba(91,84,232,.22);outline-offset:2px}.candidate-canvas-notice button:disabled{opacity:.5;cursor:not-allowed}.lesson-document>:deep(.app-error-notice){margin:12px 28px 0}
...
.objective-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:24px}.objective-grid>div{min-width:0}.objective-grid h5{margin:0 0 9px;color:#4a5568;font-size:15px}.objective-grid.is-ai-reviewing{grid-template-columns:minmax(0,1fr);gap:12px}.objective-grid.is-ai-reviewing>div{padding:13px 15px;border:1px solid #e3e7ef;border-radius:8px}.objective-grid.is-ai-reviewing h5{padding-right:74px}.objective-grid.is-ai-reviewing :is(li,p){overflow-wrap:anywhere}.objective-section p{font-size:16px}.focus-grid,.closing-grid{display:grid;grid-template-columns:1fr 1fr;gap:0}.focus-grid>div,.closing-grid>div{min-width:0;padding-right:26px}.focus-grid>div+div,.closing-grid>div+div{padding-right:0;padding-left:26px;border-left:1px solid #e8ecf2}.section-heading{display:flex;align-items:center;justify-content:space-between;gap:16px}.section-heading span{color:#7a8699;font-size:15px}
...
.ai-change-target{position:relative}.is-ai-candidate .ai-change-target{margin-inline:-10px;padding-inline:10px;border-radius:9px;background:linear-gradient(90deg,rgba(238,242,255,.92),rgba(248,250,255,.42))}.is-ai-candidate .objective-grid.is-ai-reviewing>.ai-change-target{margin-inline:0;padding-inline:15px}.ai-change-target::before{position:absolute;top:8px;bottom:8px;left:0;width:2px;border-radius:2px;background:#6366f1;content:""}.ai-change-marker{position:absolute;top:9px;right:10px;padding:2px 6px;border-radius:999px;color:#4338ca;background:#e0e7ff;font-size:11px;font-style:normal;font-weight:800;line-height:1.5}.flow-section.ai-change-target{padding-inline:10px}.focus-grid>div.ai-change-target,.closing-grid>div.ai-change-target{padding-top:12px;padding-bottom:12px}.focus-grid>div+div.ai-change-target,.closing-grid>div+div.ai-change-target{padding-left:36px}
