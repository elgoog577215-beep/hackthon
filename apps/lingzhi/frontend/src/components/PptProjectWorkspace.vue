...
OWN-WORLD: macOS/Codex utility surface; neutral paper, graphite controls, quiet separators.
...
<template>
  <section class="ppt-project-workspace">
    <header class="project-toolbar">
      <div class="project-title"><Presentation :size="18" /><strong>PPT</strong></div>
      <nav :aria-label="t('pptProject.steps')">
        <button v-for="(label, index) in stepLabels" :key="label" type="button" :class="{ active: step === index + 1 }"
          :aria-current="step === index + 1 ? 'step' : undefined" :disabled="!canStep(index + 1)" @click="selectStep(index + 1)"><span>{{ index + 1 }}</span>{{ label }}</button>
      </nav>
    ...
    <section v-if="step === 1" class="project-selection">
      <div class="selection-heading"><h1>{{ t('pptProject.selectTitle') }}</h1><p>{{ t('pptProject.selectHint') }}</p></div>
      ...
      <footer class="selection-footer"><span>{{ t('pptProject.selected').replace('{lessons}', String(lessonIds.length)).replace('{files}', String(assetIds.length)) }}</span><button type="button" class="primary" :disabled="busy || (!lessonIds.length && !assetIds.length)" @click="prepare">{{ busy ? t('pptProject.working') : (project && !selectionMatches ? t('pptProject.newFromSelection') : t('pptProject.next')) }}<ArrowRight :size="16" /></button></footer>
    </section>
    <template v-else>
      ...
      <template v-if="step === 2">
        <PptManuscriptWorkflow v-if="project?.manuscript" ref="editor" embedded :allow-page-regeneration="false" :title="project.title" :state="manuscriptState" :busy="busy || running || project.source_state === 'stale'"
        ...
        <div v-else-if="!running" class="project-empty">{{ t('pptProject.noDraft') }}</div>
      </template>
      <section v-else class="project-render">
        <header><div><h1>{{ project?.title }}</h1><p>{{ renderedCurrent ? t('pptProject.ready') : t('pptProject.renderHint') }}</p></div>
          <button v-if="project?.last_good_render" type="button" :disabled="busy" @click="download"><Download :size="16" />{{ t('pptProject.export') }}</button>
          <button v-if="!renderedCurrent" type="button" class="primary" :disabled="busy || running || !canRender" @click="render"><Presentation :size="16" />{{ t('pptProject.render') }}</button>
        </header>
...
function failure(e:any) { if (e?.code === 'ERR_CANCELED') return; error.value = e?.response?.status === 409 ? t('pptProject.conflict') : t('pptProject.failed') }
...
async function prepare(){if(selectionMatches.value && project.value?.source_state !== 'stale'){step.value=2;return}busy.value=true;error.value='';try{const {data}=await http.post(base(),{lesson_ids:lessonIds.value,asset_ids:assetIds.value,expected_revision:catalog.value.document_revision},config());project.value=data;await http.post(url('prepare'),{expected_revision:data.revision},config());step.value=2;await poll();await loadCatalog()}catch(e){failure(e)}finally{busy.value=false}}
...
async function confirm(){confirming.value=true;await action('confirm');confirming.value=false;if(canRender.value)step.value=3}
...
<style scoped>
.ppt-project-workspace{height:100%;min-height:540px;display:flex;flex-direction:column;overflow:auto;background:#fff;color:#252525;font:15px/1.5 -apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC",sans-serif}.project-toolbar{min-height:58px;display:flex;align-items:center;gap:24px;padding:10px 24px;border-bottom:1px solid #e6e6e6;flex-shrink:0}.project-title{display:flex;align-items:center;gap:8px}.project-toolbar nav{display:flex;gap:8px;flex:1;justify-content:center}.ppt-project-workspace button{font:inherit;color:inherit;cursor:pointer;background:transparent;border:0;border-radius:7px;display:inline-flex;align-items:center;gap:8px;padding:8px 12px}.ppt-project-workspace button:hover:not(:disabled){background:#eee}.ppt-project-workspace button:focus-visible,.ppt-project-workspace input:focus-visible{outline:2px solid #3673d8;outline-offset:3px}.ppt-project-workspace button:disabled{opacity:.45;cursor:default}.project-toolbar nav button{font-size:15px;color:#656565}.project-toolbar nav button span{font-size:11px;display:grid;place-items:center;width:19px;height:19px;border:1px solid #ccc;border-radius:50%}.project-toolbar nav button.active{background:#efefef;color:#222}.project-toolbar nav button.active span{background:#333;color:#fff;border-color:#333}.project-legacy{font-size:15px!important;color:#666!important}.project-selection{width:min(100%,1000px);margin:auto;box-sizing:border-box;padding:44px 48px 24px;flex:1}.selection-heading h1{color:#252525;background:none;-webkit-text-fill-color:currentColor;font-size:26px;font-weight:600;letter-spacing:-.02em;margin:0 0 8px}.selection-heading p,.empty-copy{color:#666;font-size:15px;margin:0}.selection-columns{display:grid;grid-template-columns:1fr 1fr;gap:48px;margin:38px 0}.selection-columns h2,.existing-projects h2{color:#252525;font-size:15px;font-weight:600;margin:0 0 16px}.source-heading{display:flex;align-items:center;justify-content:space-between;margin-top:-8px}.source-heading h2{margin:0}.source-heading button{font-size:15px}.source-row{display:flex;align-items:center;gap:12px;min-height:49px;border-bottom:1px solid #ededed;padding:8px 0;cursor:pointer}.source-row input{width:15px;height:15px;accent-color:#353535;flex-shrink:0}.source-row span{flex:1;overflow-wrap:anywhere}.source-row small{color:#666;font-size:15px}.source-row.unavailable{color:#777;cursor:default}.file-input{display:none}.upload-area{display:flex!important;flex-direction:column;width:100%;min-height:162px;border:1px dashed #ccc!important;margin-top:12px;color:#686868!important;justify-content:center}.upload-area strong{font-size:15px;font-weight:500}.upload-area span{font-size:11px}.selection-footer{display:flex;align-items:center;justify-content:space-between;border-top:1px solid #e6e6e6;padding-top:20px;margin-top:36px;gap:24px}.selection-footer>span{font-size:15px;color:#666}.ppt-project-workspace button.primary{background:#292929;color:#fff;padding:9px 18px}.ppt-project-workspace button.primary:hover:not(:disabled){background:#424242}.existing-projects{margin-top:32px}.existing-projects>button{display:flex;width:100%;border-bottom:1px solid #ededed;border-radius:0;padding:12px 0;text-align:left}.existing-projects>button span{flex:1}.project-message{display:flex;align-items:center;gap:16px;padding:12px 24px;background:#f6f6f6;color:#555;font-size:15px}.project-message.is-error{color:#922e2e;background:#fff4f1}.project-progress{display:flex;align-items:center;gap:14px;padding:18px 24px;background:#fafafa}.project-progress progress{height:4px;width:140px;accent-color:#555}.project-progress button{margin-left:auto}.project-empty{padding:72px 32px;color:#666;text-align:center}.project-render{padding:24px;flex:1;min-height:0;display:flex;flex-direction:column}.project-render>header{display:flex;align-items:center;gap:12px;margin-bottom:24px}.project-render>header>div{flex:1}.project-render h1{color:#252525;font-size:20px;font-weight:600;margin:0 0 6px}.project-render p{font-size:15px;color:#666;margin:0}.project-deck{display:grid;grid-template-columns:170px minmax(0,1fr);min-height:350px;gap:24px;margin-top:20px;flex:1}.project-deck>nav{overflow:auto}.project-deck>nav button{display:flex;width:100%;font-size:15px;text-align:left;align-items:flex-start;margin-bottom:6px}.project-deck>nav button span{color:#777}.project-deck>nav button.active{background:#ededed}.project-canvas{align-self:center;min-width:0;background:#f5f5f5;padding:20px}.spinning{animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}@media(prefers-reduced-motion:reduce){.spinning{animation:none}}.ppt-project-workspace :deep(.ppt-manuscript-workflow){border:0;border-radius:0;box-shadow:none;flex:1;min-height:440px}.ppt-project-workspace :deep(.ppt-manuscript-workflow__header){background:#fafafa}.ppt-project-workspace :deep(.ppt-manuscript-workflow__page-list){background:#f7f7f7}.ppt-project-workspace :deep(.ppt-manuscript-workflow__pages){min-height:420px}@media(max-width:850px){.project-toolbar{gap:10px;padding-inline:12px}.project-title{display:none}.project-selection{padding:28px 24px}.selection-columns{gap:24px}.project-legacy{max-width:110px}.project-deck{grid-template-columns:120px minmax(0,1fr);gap:12px}.project-canvas{padding:8px}}
.ppt-project-workspace{--lz-brand-strong:#252525;--lz-bg-page:#fff;--lz-text-primary:#252525}
.ppt-project-workspace :deep(.teacher-document-command-bar__actions .primary-action){background:#292929;border-color:#292929;color:#fff}
.ppt-project-workspace :deep(.teacher-document-command-bar__actions .primary-action:hover:not(:disabled)){background:#424242;border-color:#424242}
.ppt-project-workspace :deep(.ppt-manuscript-workflow__page-rail.is-active){background:#ededed}
.ppt-project-workspace :deep(.ppt-manuscript-workflow__page-rail.is-active button){color:#252525}
.ppt-project-workspace :deep(.ppt-manuscript-workflow__pages){border-color:#e6e6e6;border-radius:0}
.ppt-project-workspace :deep(.ppt-manuscript-workflow__reading h2){color:#252525}
</style>
