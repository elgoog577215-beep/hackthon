  ...
  >
    ...
    <div v-else-if="embedded && activeStep === 1 && (generatorOpen || !pptManuscriptState?.manuscript)" class="ppt-workspace-preparation is-generator">
      ...
      <SlideDeckGeneratorDialog inline open :mode="selectedMode" :theme="selectedTheme" :web-image-retrieval="selectedWebImageRetrieval"
        ...
        @close="closeGenerator" @confirm="generateVariant" @create-template="openTemplateCreator" />
      <p v-if="!canGenerate && !store.building" class="ppt-workspace-preparation__hint">{{ t('pptWorkspace.flow.generationUnavailable') }}</p>
    </div>
    ...
    <PptManuscriptWorkflow
      ...
      @generate-ppt="embedded ? selectFlowStep(2) : generatePptFromConfirmedManuscript()"
      @dirty-change="manuscriptDirty = $event"
    />
...
}>(), { embedded: false, canGenerate: true, sourceReady: true })
const emit = defineEmits<{ changed: []; 'open-script': []; 'open-source': [sectionId: string] }>()
...
const courseId = computed(() => props.courseId || String(route.params.courseId || ''))
const teacherLessonId = computed(() => props.lessonId || String(route.query?.lesson || route.query?.node || ''))
const isTeacherSurface = computed(() => (
  props.embedded || Boolean(teacherLessonId.value)
...
const renderedCurrentManuscript = computed(() => Boolean(slideRepresentation.value && (
  !pptManuscriptState.value?.manuscript
  || pptManuscriptState.value.generated_representation_id === slideRepresentation.value.representation_id
)))
...
async function prepareToLeave(): Promise<boolean> {
  leaveError.value = ''
  if (props.externalBusy || pptManuscriptSaving.value || pptManuscriptConfirming.value || pptManuscriptRegenerating.value || pptAiCandidate.value?.status === 'pending' || deckWorkbench.value?.prepareToLeave() === false) {
    leaveError.value = t('pptWorkspace.flow.finishEditing')
    if (pptAiCandidate.value?.status === 'pending') { activeStep.value = 3; aiVisible.value = true }
    return false
  }
  if (!manuscriptDirty.value) return true
  const changes = manuscriptEditor.value?.pendingChanges()
  if (!changes) return false
  return await savePptManuscript(changes.updates, changes.pacing)
}
async function requestGeneration(action: string, resumeTaskId = '') {
  const attempt = workspaceAttempt
  const id = courseId.value
  if (!await prepareToLeave() || !isCurrentAttempt(id, attempt)) return
  if (action !== 'retry_generation') { openGenerator(action === 'regenerate_from_latest_source'); return }
  if (!resumeTaskId) return
  const recovered = await store.recoverDurableBuild(id, resumeTaskId)
  if (!isCurrentAttempt(id, attempt)) return
  if (recovered?.id !== resumeTaskId || store.buildTaskId !== resumeTaskId || !store.buildPaused) {
    pptManuscriptConfirmError.value = t('pptWorkspace.resumeTargetUnavailable')
    return
  }
  await store.resumeBuild()
  if (isCurrentAttempt(id, attempt)) await loadWorkspace()
}
async function resumeCurrentBuild() {
  const attempt = workspaceAttempt
  const id = courseId.value
  if (!store.buildTaskId || !await prepareToLeave() || !isCurrentAttempt(id, attempt)) return
  await store.resumeBuild()
  if (isCurrentAttempt(id, attempt)) await loadWorkspace()
}
...
const showManuscriptWorkflow = computed(() => {
  ...
  if (props.embedded) return activeStep.value === 1 && !generatorOpen.value
...
async function openManuscriptWorkflow() {
...
async function loadWorkspace() {
  ...
  try {
    const documentPromise = props.embedded && !props.sourceReady
      ? Promise.resolve(null)
      : loadDocumentEnvelope(id, attempt).catch(() => {
        if (isCurrentAttempt(id, attempt)) documentLoadError.value = t('pptWorkspace.documentLoadFailed', '加载课程源失败，请重试')
        return null
      })
    const registryPromise = (props.embedded ? store.load : store.ensure)(id, {
    ...
    const manuscriptPromise = isTeacherSurface.value
      ...
      : Promise.resolve(null)
    const [envelope, , loadedManuscript] = await Promise.all([
      documentPromise,
      registryPromise,
      manuscriptPromise,
    ])
    if (!isCurrentAttempt(id, attempt) || (envelope && envelope.source_format !== 'canonical')) return
    if (!props.embedded && !envelope) return
    ...
    } else if (store.courseId === id && (props.embedded || !store.representations.length) && !store.building) {
      const recovered = await store.recoverDurableBuild(id)
      observingRecoveredBuild = ['pending', 'running'].includes(String(recovered?.status || ''))
    }
    if (!isCurrentAttempt(id, attempt)) return
    ...
    if (props.embedded) activeStep.value = renderedCurrentManuscript.value ? 3 : loadedManuscript?.status === 'confirmed' ? 2 : 1
    if (!props.embedded && route.query?.regenerate === '1') {
      openGenerator(true)
...
async function loadDocumentEnvelope(id: string, attempt: number) {
  ...
  if (!props.embedded) courseStore.applyCourseDocumentEnvelope(envelope)
...
async function openGenerator(forceRebuild: boolean) {
  if (!await prepareToLeave()) return
  if (props.embedded && (!props.canGenerate || manuscriptDirty.value)) return
  activeStep.value = 1
...
async function generateVariant(value: {
  ...
  const launchAttempt = workspaceAttempt
  const launchCourseId = courseId.value
  if (!courseId.value || store.building) return
  if (slideEngineStatus.value === 'blocked') {
    generatorOpen.value = false
    logicUpgradeError.value = effectiveBuildFailure.value?.message || '当前课程逻辑尚未就绪。'
  ...
  if (props.externalBusy || (props.embedded && (!props.canGenerate || !props.sourceReady))) return
  if (props.beforeGenerate) {
    try { await props.beforeGenerate() } catch (error) { pptManuscriptConfirmError.value = pptAiErrorMessage(error); return }
  }
  ...
  try {
    ...
    const completed = await store.buildSlideDeckVariant(courseId.value, {
      ...
      forceRebuild: forceGeneratorBuild.value || props.generationAction === 'regenerate_from_latest_source',
      manuscriptOnly,
...
function backToCourse() {
  if (props.embedded) { activeStep.value = 1; return }
...
function openSameSourceCourse(state: PptSameSourceHighlightState) {
  if (props.embedded) { emit('open-source', state.sectionId); return }
...
async function confirmPptManuscript() {
  ...
  try {
    ...
    if (props.embedded) activeStep.value = 2
    emit('changed')
  } catch (error: any) {
...
async function generatePptFromConfirmedManuscript() {
  ...
  if (
    ...
    || store.building
    || !courseId.value
    || (props.embedded && !props.sourceReady)
    || manuscriptDirty.value
    || props.externalBusy
  ) return
  activeStep.value = 2
  pptManuscriptConfirmError.value = ''
  try {
    ...
    manuscriptWorkflowForced.value = false
    if (representation && !store.buildError) activeStep.value = 3
    emit('changed')
  } catch {
    return
  }
...
watch(() => store.building, (building, previous) => {
  if (previous && !building && observingRecoveredBuild) {
    observingRecoveredBuild = false
    void loadWorkspace()
    emit('changed')
  }
})
watch(() => props.sourceReady, ready => {
  if (ready && !documentEnvelope.value) void loadDocumentEnvelope(courseId.value, workspaceAttempt).catch(() => undefined)
})
watch(
  () => [store.selectedId, store.selectedSpec?.revision],
  () => { void loadPptAiCandidate() },
)
function protectUnsavedPpt(event: BeforeUnloadEvent) {
  if (manuscriptDirty.value || pptManuscriptSaving.value || pptAiCandidate.value?.status === 'pending' || deckWorkbench.value?.dirty) {
    event.preventDefault()
    event.returnValue = ''
  }
}
onMounted(() => {
  window.addEventListener('beforeunload', protectUnsavedPpt)
...
onUnmounted(() => {
  window.removeEventListener('beforeunload', protectUnsavedPpt)
  workspaceAttempt += 1
  stopPptAiResize()
  pptAiCandidateAttempt += 1
  if (store.teacherLessonId === teacherLessonId.value) store.setTeacherLessonScope('')
...
.ppt-workspace-body { display:flex; flex:1; min-width:0; min-height:0; position:relative; }
...
.ppt-workspace-preparation{width:100%;align-self:flex-start;padding:24px;border:1px solid #e0e4ea;border-radius:12px;background:#fff;color:#334155}
.ppt-workspace-preparation h2{font-size:20px;margin:0 0 12px;color:#27344a}
.ppt-workspace-preparation p{font-size:15px;line-height:1.7;margin:0 0 20px}
.ppt-workspace-preparation button{display:inline-flex;align-items:center;gap:8px;min-height:40px;padding:8px 16px;border:1px solid #d7ddeb;border-radius:8px;background:#fff;color:#3f4880;cursor:pointer;font-size:15px}
.ppt-workspace-preparation button.is-primary{background:#5141d9;border-color:#5141d9;color:#fff}
.ppt-workspace-preparation button:disabled{opacity:.55;cursor:not-allowed}
.ppt-workspace-preparation button:focus-visible{outline:2px solid #5b57e8;outline-offset:3px}
.ppt-workspace-preparation.is-generator{padding:0;overflow:hidden}
.ppt-workspace-preparation__hint{padding:0 20px}
.is-embedded .ppt-workspace-state{width:100%;min-height:240px;align-self:flex-start;background:transparent}
.is-embedded .ppt-workspace-state h1,.is-embedded .ppt-workspace-state__mark,.is-embedded .ppt-workspace-state__back{display:none}
.is-embedded .ppt-workspace-state p{font-size:15px}
.is-embedded .ppt-workspace-view__ai{position:absolute;inset:0 0 0 auto;z-index:10;box-shadow:-8px 0 24px rgba(30,41,59,.1)}
.is-embedded .ppt-workspace-view__ai-resizer{display:none}
</style>
