  ...
  <section ref="documentRoot" class="script-document" :class="{ 'is-ai-candidate': pendingCandidate }">
    ...
    <div v-if="scriptSections.length" class="script-continuous" :data-state="lesson.script.ready && !showWorkingPreview ? 'ready' : 'partial'">
      ...
      >
        ...
        <template v-else>
          <div v-if="pendingCandidate?.section_node_id === node.section_node_id && pendingCandidate.replacement_text" ref="candidateRef" class="script-content script-content--candidate-below" data-state="candidate" tabindex="-1">
            ...
            <MarkdownRenderer :key="`candidate-${pendingCandidate.candidate_id || pendingCandidate.section_node_id}`" :content="pendingCandidate.replacement_text" />
          </div>
          <div v-if="node.blocks?.length" class="script-modules">
          ...
          <div v-else-if="!node.blocks?.length && !contentFor(node)" class="script-empty">{{ tr('courseWorkbench.scriptPending') }}</div>
        </template>
      </article>
    </div>
