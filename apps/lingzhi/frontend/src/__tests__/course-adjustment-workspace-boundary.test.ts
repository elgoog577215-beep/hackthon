...
describe('课程审计与更新边界', () => {
  it('把材料审计与全课调整收入同一课程级链路', () => {
    ...
    expect(workspace).toContain('@open-course-adjustment="openCourseAdjustment"')
    expect(workspace).toContain('<CourseEvolutionWorkspace')
    expect(workspace).toContain('courseAdjustmentOpen.value = true')
