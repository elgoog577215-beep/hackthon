...
describe('teacher PPT source branching contract', () => {
  it('opens the lesson manuscript directly and removes the right sidebar from every PPT path', () => {
    ...
    expect(source).toContain("computed(() => activeStage.value !== 'ppt' && !contextPaneCollapsed.value)")
    expect(source).toContain("!['question-bank', 'ppt'].includes(activeStage)")
    expect(source).toContain('class="ppt-review-toolbar"')
    expect(source).toContain('<LessonPptWorkspace v-if="!legacyPptOpen && selectedLesson"')
