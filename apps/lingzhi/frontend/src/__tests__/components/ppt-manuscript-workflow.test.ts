...
describe('PptManuscriptWorkflow', () => {
  ...
  it('focuses one page and retains edits and position across navigation and saves', async () => {
    const state = { ...emptyState, revision: 'one', status: 'draft', confirmable: true, manuscript: {
      page_count: 2, pages: [
        { page_id: 'p1', page_number: 1, page_type: 'concept', title: '第一页', visible_copy: ['正文一'] },
        { page_id: 'p2', page_number: 2, page_type: 'concept', title: '第二页', visible_copy: ['正文二'] },
      ],
    } }
    ...
    await wrapper.get('[data-page-id="p1"]').trigger('click')
    expect((wrapper.get('.ppt-manuscript-workflow__title-field input').element as HTMLInputElement).value).toBe('已编辑')
    await wrapper.get('[data-page-id="p2"]').trigger('click')
