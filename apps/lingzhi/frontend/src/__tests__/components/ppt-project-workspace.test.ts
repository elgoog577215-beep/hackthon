...
it('blocks an unavailable default lecture and refreshes when its teacher revision is ready', async()=>{
 ...
 await flushPromises()
 const vm=wrapper.vm as any
 expect((wrapper.get('input[type="radio"]').element as HTMLInputElement).checked).toBe(false)
...
it('selects one lecture and starts only after Next', async()=>{
 const router=createRouter({history:createMemoryHistory(),routes:[{path:'/',component:{template:'<div />'}}]});await router.push('/')
 ...
 const choices=wrapper.findAll('input[type="radio"]');await choices[0]!.setValue(true)
 await wrapper.get('.selection-footer .primary').trigger('click');await flushPromises()
 expect(http.post).toHaveBeenCalledWith('/api/teacher/courses/c/ppt-projects',expect.objectContaining({lesson_ids:['l1']}),expect.anything())
...
it('restores an existing selection, reuses it and protects unsaved edits', async()=>{
 ...
 expect(wrapper.findAll('input[type="radio"]').some(x=>(x.element as HTMLInputElement).checked)).toBe(true)
 await wrapper.get('.selection-footer .primary').trigger('click');await flushPromises()
 expect(http.post).not.toHaveBeenCalled()
 wrapper.getComponent({name:'PptManuscriptWorkflow'}).vm.$emit('dirty-change',true);await flushPromises()
