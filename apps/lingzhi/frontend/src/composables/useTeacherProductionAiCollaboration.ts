...
export function buildTeacherProductionAiInstruction(
  ...
  const common = [
    ...
    `资料范围（只能使用下列精确资料）：\n${sourceList || '无额外资料'}`,
    `教师对话（越靠后优先级越高）：\n${requirements}`,
    '输出边界：只生成建议，不确认、不发布，也不自动改写下游正式内容。',
  ]

  if (scope.domain === 'outline') {
    return [
      '任务：依据教师对话，为当前课程大纲生成一份可审阅的结构调整建议。',
      ...common,
  ...
  if (scope.domain === 'script') {
    return [
      '任务：依据教师对话，为当前讲义小节生成一份可审阅的表达修改建议。',
      ...common,
      '修改原则：保持当前教案的教学目标、模块身份、知识事实与时间约束；只调整教师明确提出的讲解表达。',
    ].join('\n')
  }

  if (scope.domain === 'question-bank') {
    return [
      '任务：依据教师对话，为整门课程题库生成一份可确认的重建任务建议。',
      ...common,
  ...
  if (scope.domain === 'ppt') {
    return [
      '任务：依据教师对话，为当前 V6 PPT 页面生成一份可审阅的表达修改建议。',
      ...common,
  ...
  return [
    '任务：依据教师对话，为当前教案小节生成一份可审阅的结构化修改建议。',
    ...common,
