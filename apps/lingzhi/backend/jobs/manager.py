...
class TaskManager:
    ...
    def describe_task_recovery(self, task_id: str) -> dict[str, Any]:
        ...
        if task.get("type") in {
            ...
            return {"state": str(task.get("status") or "unknown"), "can_resume": False,
                    ...
                    "reason": "请在全局修改中继续，已完成分析与候选会保留", "checkpoint": {"plan_id": (task.get("request_snapshot") or {}).get("plan_id")}}
    ...
    async def _reconcile_task_after_restart(self, task_id: str) -> bool:
        ...
        if task.get("type") in {
            ...
            task["message"] = (
                "正在恢复未完成的整课影响分析"
                if task.get("type") == "teacher_course_change_analysis"
                else "正在恢复未完成的修改候选"
            )
            return True
        if task.get("type") == "course_import":
    ...
    async def _cleanup_task_artifacts(self, task: dict[str, Any]) -> None:
        if task.get("type") in {
            ...
            # Analysis plans and candidate assets belong to their domain repositories.
            return
        task_id = str(task.get("id") or "")
    ...
    async def _process_task(self, task_id: str) -> None:
        ...
        if task.get("type") == "teacher_course_change_analysis":
            from course_evolution.jobs import run_analysis
            await run_analysis(self, task_id)
            return
