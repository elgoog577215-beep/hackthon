    ...
    "general": {
        ...
        "profiles": {
            ...
            "visual_interaction_design": {
                "label": "视觉与交互设计",
                "keywords": ["UI设计", "界面设计", "交互设计", "用户体验设计", "UX设计", "视觉设计", "产品设计", "设计系统"],
                "professional_actions": ["分析用户任务", "梳理信息层级", "绘制线框图", "建立视觉规范", "制作交互原型", "开展可用性评审", "依据反馈迭代"],
                "canonical_artifacts": ["用户任务流程", "信息架构", "线框图", "高保真界面", "交互原型", "设计规范", "可用性测试报告"],
                "evidence_patterns": ["设计决策能对应用户任务与现实约束", "原型覆盖关键操作路径和界面状态", "界面满足信息层级、一致性和可访问性要求", "用户测试发现能推动具体修改"],
                "common_misconceptions": ["把视觉美观等同于易用", "只设计正常页面而遗漏空、错误、加载和禁用状态", "无依据照搬流行风格"],
                "source_policy": ["教师提供的设计规范、真实用户任务和平台准则优先", "参考作品的场景与约束要与当前任务区分"],
                "safety_boundaries": ["用户研究不收集无必要的个人信息", "不使用误导、强迫或隐藏后果的交互方式"],
                "quality_rules": ["每项界面决策能追溯到用户任务、内容或约束", "关键路径同时检查导航、内容、操作和状态", "评审标准能直接用于判断原型或界面是否达标"],
                "artifact_language": {
                    "outline": "用用户任务、界面成果和可检查标准组织讲次，不把设计术语清单当成课程进度。",
                    "lesson_plan": "写清教师如何示范和评审，学生如何分析、绘制、制作原型并依据反馈修改。",
                    "script": "像设计教师现场拆解案例和评图，说清每个选择对应的用户任务、约束和取舍。",
                    "question_bank": "提供用户、场景、内容和设备约束，要求学生产出可评审的设计结果并说明理由。",
                    "ppt": "用流程、线框图、状态对照和修改前后对比呈现设计判断，不用装饰图替代界面证据。"
                },
                "block_recipes": {
                    "activity": {"teacher_activity": "给出用户、场景、内容和设备约束", "student_activity": "梳理任务流程并绘制线框图或交互原型", "expected_output": "可评审的界面方案与设计理由"},
                    "feedback": {"teacher_activity": "根据任务完成、信息层级、一致性和关键状态评图", "student_activity": "根据反馈修改原型并说明取舍", "expected_output": "修订后的原型与变更说明"}
                }
            },
        },
    },
