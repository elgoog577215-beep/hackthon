...
def compact_teaching_request(request):
    """Remove repeated metadata; retain every complete source in numeric order."""
    payload = deepcopy(request)
    if payload.get("teaching_request") != "narrative":
        if "literal_source_ranges" in payload:
            payload["literal_source_ranges"] = [{"quote_id": quote["quote_id"], "quote": quote["quote"]}
                                                 for quote in payload["literal_source_ranges"]]
        return payload
    contexts = []
    for unit in payload.get("units", []):
        context = unit.pop("teaching_context", {})
        if context not in contexts:
            contexts.append(context)
        unit["teaching_context_index"] = contexts.index(context)
        for source in unit.get("sources", []):
            # NarrativeDraft returns numbered ranges; precise IDs and revisions
            # remain in the compiler and in later page requests.
            source.pop("block_revision", None)
            source.pop("block_id", None)
    payload["teaching_contexts"] = contexts
    payload.pop("required_first_use_order", None)
    return payload
...
async def invoke_teaching_provider(provider, request):
    ...
    response = await call(
        ...
        system_prompt=(
            ...
            "Use the supplied physical-page budget as pacing advice, including answer views and key-step views. Preserve complete learner tasks even if the suggestion is exceeded. "
            "When one planned task needs multiple readable slides, return {pages:[...]} of complete page drafts, each with a split_reason and its own "
...
async def plan_teaching_manuscript(document, graph, template, planner, *, source_context=None, checkpoint=None, on_checkpoint=None):
    ...
    async def fill_page(plan):
        ...
        available_pages = max(1, narrative.pacing.max_physical_pages - used_pages - pending_tasks)
        ...
        for attempt in range(3):
            ...
            response = await invoke({"teaching_request": "page", "page": request_plan,
                ...
                "repair_instruction": (
                    ...
                    "that cannot be shown readably, explain each split_reason and use physical_page_budget as advice, never truncate essential content to meet it. "
