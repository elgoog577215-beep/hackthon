...
def block_type_from_title(title: str, order: int = 0, content: str = "") -> str:
    ...
    explicit_patterns = [
        ("objective", r"^(本讲目标|本节任务|本节目标|学习目标|学习任务|目标与任务|要解决的问题)$"),
        ("orientation", r"^(引入|引入问题|直觉|直观理解|直觉入口|背景|问题导入|现象与问题)(?:[:：].*)?$"),
        ("prerequisite", r"^(前置|前置知识|前置诊断|准备知识)(?:[:：].*)?$"),
        ("counterexample", r"^(反例|适用条件与反例|边界与反例|对比|辨析|案例比较|观点比较|材料辨析)(?:[:：].*)?$"),
        ("misconception", r"^(错误分析|常见错误|常见误区|易错点|陷阱)(?:[:：].*)?$"),
        ("remediation", r"^(补救|局部补救|纠错|间隔复习)(?:[:：].*)?$"),
        ("feedback", r"^(课堂评价与反馈|检查与反馈|答案与评价标准|评价标准|运行结果与核对|运行结果|测试与结果分析|测试与质量)(?:[:：].*)?$"),
        ("activity", r"^(学生活动|学习者行动|实践任务|实战任务|改进练习|修改任务|变式练习|基础练习|控制练习|表达与运用|真实输出|讨论或写作|角色模拟|实验设计)(?:[:：].*)?$"),
        ("checkpoint", r"^(检查|理解检查|自测|自测练习|随堂练习)(?:[:：].*)?$"),
        ("summary", r"^(小结|总结|本节回顾|回顾)(?:[:：].*)?$"),
        ("example", r"^(例子|案例|示例|例题|例题讲解|例题推演|例子讲解|解释性例子|调试与修正|调试案例|案例分析|机制案例|案例拆解|基础示例与运行|最小可运行示例)(?:[:：].*)?$"),
        ("application", r"^(应用|情境应用|真实场景|应用场景|实践|预测与应用|工具与模板|数学建模)(?:[:：].*)?$"),
        ("reasoning", r"^(推理|推导|证明|原理|深入讲解与推理|实现原理|机制拆解|过程与机制|机制过程|观点与论证|数据分析|需求分析与方案设计|需求与设计)(?:[:：].*)?$"),
        ("concept", r"^(概念|核心概念|重点讲解|核心教学|概念关系|概念地图|多种表示|多重表征|定义|正式定义|模型与规律|架构设计|方法框架)(?:[:：].*)?$"),
        ("transfer", r"^(迁移|拓展应用|综合应用|综合迁移|跨情境迁移)(?:[:：].*)?$"),
    ]
