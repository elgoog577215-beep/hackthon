...
def test_notes_are_exported_after_policy_change_and_budget_is_recomputed(monkeypatch):
    ...
    assert revised.page_count == 3 and revised.quality_status == "passed"
    assert revised.quality_suggestions[0].code == "ppt_pacing_budget_exceeded"
    validate_reviewable_manuscript(doc, graph, revised, template)
    assert len(compile_slide_deck_v6_from_manuscript(doc, graph, revised, template).pages) == 3
...
def test_adjacent_duplicate_canvas_suggested_even_if_title_changes():
    ...
    assert revised.quality_status == "passed"
    assert revised.quality_suggestions[0].code == "ppt_pacing_duplicate_canvas"
    assert revised.quality_suggestions[0].page_id == "p2"
...
def test_budget_suggestion_does_not_force_repairs_or_discard_complete_siblings():
    import asyncio
    from ppt_teaching_planner import plan_teaching_manuscript
    doc, graph, template, manuscript = compiled_manuscript()
    page = manuscript.pages[0]
    ...
    result, _ = asyncio.run(plan_teaching_manuscript(doc, graph, template, planner))
    assert len(calls) == 2 and result.page_count == 2 and result.quality_status == "passed"
    assert result.quality_suggestions[0].code == "ppt_pacing_budget_exceeded"


def test_duplicate_suggestion_allows_zero_model_final_build(tmp_path):
    import asyncio
    ...
    assert draft.quality_status == "passed"
    assert draft.quality_suggestions[0].code == "ppt_pacing_duplicate_canvas"
    assert candidates.load("draft")["ppt_manuscript"]["quality_status"] == "passed"
    final = asyncio.run(orchestrator.build(task_id="final", story_planner=forbidden, confirmed_manuscript=draft, **args))
    assert final["status"] == "v6_ready"


def test_narrative_request_compaction_preserves_sources_and_deduplicates_context():
    from ppt_teaching_planner import compact_teaching_request
    request = {"teaching_request": "narrative", "required_first_use_order": ["b1", "b2"],
        "units": [{"teaching_context": {"key_points": ["共同目标"]},
                   "sources": [{"source_number": n, "block_id": f"b{n}", "block_revision": "r1", "full_text": text}]}
                  for n, text in [(1, "完整的第一段讲义"), (2, "完整的第二段讲义")]]}
    compact = compact_teaching_request(request)
    assert compact["teaching_contexts"] == [{"key_points": ["共同目标"]}]
    assert [u["sources"][0]["full_text"] for u in compact["units"]] == ["完整的第一段讲义", "完整的第二段讲义"]
    assert [u["sources"][0]["source_number"] for u in compact["units"]] == [1, 2]
    assert "block_revision" in request["units"][0]["sources"][0]


def test_quality_projection_reclassifies_only_known_pacing_reports_without_mutation():
    from ppt_manuscript_quality import quality_report
    value = {"teaching_content_contract_version": "page_teaching_v2", "quality_status": "blocked",
        "quality_issues": [{"code": "ppt_pacing_budget_exceeded"}], "manuscript_revision": "frozen"}
    before = deepcopy(value)
    assert quality_report(value)["passed"] and quality_report(value)["suggestions"]
    assert value == before
    value["quality_issues"].append({"code": "teaching_source_order_changed"})
    assert not quality_report(value)["passed"]
    assert quality_report(value)["issues"] == [{"code": "teaching_source_order_changed"}]


def test_page_request_compaction_preserves_exact_quotes_and_complete_sources():
    from ppt_teaching_planner import compact_teaching_request
    request = {"teaching_request": "page", "sources": [{"block_id": "b", "full_text": "原文全段"}],
        "literal_source_ranges": [{"quote_id": "q1", "quote": "原文", "block_id": "b", "block_revision": "r", "start": 0, "end": 2}]}
    compact = compact_teaching_request(request)
    assert compact["sources"] == request["sources"]
    assert compact["literal_source_ranges"] == [{"quote_id": "q1", "quote": "原文"}]
    assert request["literal_source_ranges"][0]["block_revision"] == "r"


def test_auxiliary_teaching_metadata_is_advisory_not_a_rendering_blocker(monkeypatch):
    from ppt_teaching_manuscript import revise_teaching_manuscript
    doc, graph, template, manuscript = compiled_manuscript()
    monkeypatch.setattr("ppt_teaching_manuscript.template_for_manuscript", lambda _: template)
    candidate = revise_teaching_manuscript(manuscript, [{"page_id": "p1", "page_goal": "",
        "audience_question": "比较两种执行方式", "expected_response": "", "observable_evidence": ""}])
    assert candidate.quality_status == "passed"
    assert {i.code for i in candidate.quality_suggestions} >= {"teaching_goal_suggested", "teaching_response_suggested"}
    from slide_deck_v6 import compile_slide_deck_v6_from_manuscript
    assert compile_slide_deck_v6_from_manuscript(doc, graph, candidate, template).pages


def test_layout_edit_recompiles_the_same_content_and_visual_binding(monkeypatch):
    from ppt_teaching_manuscript import revise_teaching_manuscript, manuscript_layout_options
    doc, graph, template, original = compiled_manuscript()
    assert [item["slug"] for item in manuscript_layout_options(original, template)] == ["compare-matrix"]
    value, _ = comparison_fixture()
    added = deepcopy(next(e for e in value["elements"] if e["element_id"] == "a-mode"))
    added.update(element_id="a-result", text="完成工作")
    value["elements"].append(added)
    ids = [e["element_id"] for e in value["elements"]]
    value["must_show"] = list(ids)
    value["source_dispositions"][0]["element_ids"] = list(ids)
    value["states"][0]["visible_element_ids"] = list(ids)
    value["expression"]["cells"][0]["element_ids"].append("a-result")
    value["expression"]["relations"] = [{"relation_id": "order", "source_id": "a-mode", "target_id": "a-result", "kind": "sequence", "sources": added["sources"]}]
    doc, graph, template, manuscript = compiled_manuscript(value)
    monkeypatch.setattr("ppt_teaching_manuscript.template_for_manuscript", lambda _: template)
    target = template.layout_id("compare-visual")
    candidate = revise_teaching_manuscript(manuscript, [{"page_id": "p1", "layout_id": target}])
    assert candidate.pages[0].teaching == manuscript.pages[0].teaching
    assert candidate.pages[0].visual_decision.resolved_template_layout_id == target
    from slide_deck_v6 import compile_slide_deck_v6_from_manuscript
    deck = compile_slide_deck_v6_from_manuscript(doc, graph, candidate, template)
    assert all(page.resolved_layout == target for page in deck.pages)
