...
def test_budget_suggestion_can_confirm_and_preserves_revision_and_last_confirmed(tmp_path, monkeypatch):
    ...
    assert blocked["status"] == "draft" and blocked["confirmable"]
    assert blocked["manuscript"]["quality_suggestions"][0]["code"] == "ppt_pacing_budget_exceeded"
    ...
    confirmed = repo.confirm_v6_ppt_manuscript_draft("course", "L1-1", manuscript_revision=blocked["revision"])
