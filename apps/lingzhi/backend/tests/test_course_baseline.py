...
@pytest.mark.asyncio
async def test_course_information_summary_does_not_project_teacher_body():
    course = {
        "course_id": "course-1",
        "course_name": "人工智能通识课",
        "owner_id": "teacher-a",
    ...
    class Repository:
        ...
        @staticmethod
        def load_course_view(_course_id):
            raise AssertionError("summary read must not project the teacher body")
    ...
    assert response["schema_version"] == "teacher_course_information_summary_v1"
    assert response["course_id"] == "course-1"
    assert response["revision"] == 3
    assert response["information"]["course_name"] == "人工智能通识课"
    assert "document_revision" not in response
