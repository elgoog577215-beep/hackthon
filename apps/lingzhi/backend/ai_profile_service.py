...
{self_evaluation or '无'}"""
            ...
            # 错误类记录只提供历史上下文，不能替代诊断结论。
            wa_text = self._format_wrong_answers(wrong_answers)
            notes_text = self._format_notes(notes)
            prompt = f"""## 练习历史中的未通过记录（仅作描述性事实）
{wa_text or '暂无未通过记录'}

约束：不得仅根据未通过次数、错误题目或低分推断能力弱项、知识缺陷或补救动作。只有已确认的诊断案例才能支持这类结论；当前输入未提供诊断案例。
...
{self_evaluation or '无'}"""
    ...
    def _format_wrong_answers(wrong_answers: list) -> str:
        ...
        for node, items in by_node.items():
            lines.append(f"【{node}】共 {len(items)} 条未通过历史：")
            for wa in items:
                q = wa.get("question", "")
