...
新讲义任务锁定 `script_ppt_bundle_v1`，由同一次分片模型请求返回完整正文和固定版式页面字段；沿现有检查点保留两种结果，页面失败仅修复失败部分。服务端解析精确来源，在教师仓储中原子保存当前正文修订与 `ppt_manuscript_v1`；普通 PPT 编辑不修改讲义修订。旧运行任务继续旧协议。

页面表单对模型暴露稳定的 `layout_key`，不暴露带主题和版本前缀的内部布局 ID。系统在当前冻结模板中把语义键绑定为精确 ID；历史完整 ID 按末段语义键迁移。未知 ID 只有在字段结构唯一匹配一个当前表单时才能本地纠正；多匹配、无匹配及 `layout_key`/`layout_id` 冲突继续阻断并进入有界局部修复。检查点最终只保存精确布局 ID，使预览、编辑与导出保持现有合同。

页面内容采用来源优先顺序：联合生成先完成当前块 `content`，再为每个可见内容项选择其中的连续原文，最后写屏幕文字；固定讲义修复先选择 `quote_id`。数字、公式、专有名词和代码标识符缺少 `sources` 时，只有全部受保护 token 能由供应范围覆盖才允许本地补绑定。模型两次仍保留无依据事实后，系统只把失败来源单元转换为可完整拼回原文的短要点页或测量代码续页，记录原因并保留已通过页面；JSON损坏、来源变化和无法完整重建继续失败。

当前讲次 PPT 使用“内容稿 / PPT 渲染”双 Tab，默认显示全部稿件。400ms 防抖保存采用乐观版本检查；保存成功只刷新受影响页。预览读取已保存稿件及统一页面执行计划，不调用模型或文件导出。PPTX 下载才进入持久导出任务和文件审计。新稿没有人工确认门；旧项目和原件审阅保持原协议。讲义来源变化标明受影响页，显式局部同步保留教师采用边界。

以下关于独立故事/视觉规划、项目创建与人工确认的描述仅适用于历史稿和显式兼容补齐；新主链以本段及任务 23 为准。模板版本、来源、权限和最后可用结果保护继续生效。
...
| 产品机制 | 整讲先定学习路径、任务和有理由的物理页预算，再逐页表达 | 规划显式提供 `ppt_pacing_v1` 的整讲最大物理页数及理由；不从字数、源块数或统一每分钟比例推出页数。容量修复先减次要屏幕内容，必要时拆分独立任务；页数预算为建议，拆分写明理由 |
| 交互设计 | 教师在确认前控制篇幅与展示方式 | 显示内容页数、实际物理页数和预算；允许修改预算及理由、整页/问答/关键停顿方式，保存后重算预览与问题。预算超出和普通重复作为建议，不阻断确认；来源、结构和溢出错误仍阻断；不丢本地编辑 |
...
整讲质量重新计算预算超出和相邻重复画面等建议，与来源错位、结构损坏、答案顺序和实际溢出错误分开；不能把普通建议升级成生成失败。失败保留可审阅草稿及最后确认稿；教师修正后无需为重算规则再次调用模型。预算和政策的任何修改都会改变稿件修订并失效确认。旧确认稿不得因新政策静默改变；工具版本不匹配时明确要求重新预检。

验收分别证明：来源合并不丢备注；普通页多个讲述步骤只出一页；问答不泄露；显式关键停顿及非累计状态保留；正式载荷不能绕过去重；全讲预算与普通重复建议不阻断确认，实际结构错误仍阻断；局部重生不改非目标页；旧稿读取不注入新字段；真实隔离讲义重新编排和导出，报告实际内容页/物理页与重复数。已有 119 页样本保留作反例。
...
## 工作区简化与主分支整合（2026-09-06）

使用共享分页切页，多选时展开页面目录；中央只挂载当前页阅读与编辑，右栏使用工作台同一状态与操作区；内容与排版使用现有分段控件。整讲安排、教学备注与来源按需展开。保存失败保留本地稿，保存成功保持当前页并刷新预览。原版 PPT 分支不显示讲义生成操作，成品 AI 审阅入口保留。

代码按统一仓库放入 `apps/lingzhi`，应用脚本、字体和模板版本随应用发布。主仓最新发布分流与其他任务改动保留。三阶段新建默认仍受既有生产验收门约束；推送 main 不等于已经开放默认新引擎。

## 工作台原地制作 PPT（2026-09-06）

教师在当前讲次的 PPT 阶段完成“1 内容稿 → 2 PPT”。模板选择放在内容稿生成前；确认后只进入第二步，教师点击右栏渲染按钮才启动原生成任务。第二步保留只读稿件并在完成后展示现有预览、下载、页面详情和 AI 修改。复用大纲步骤条、工作台共用右栏、只读资料摘要与生成准备弹窗，资料设置仅在准备时出现；不在中央重复放状态和主按钮。已有原版 PPT 继续进入原件审阅，不强制套入讲义生成流程。

`PptWorkspace` 由原独立页抽出，工作台与兼容路由复用同一组件、Store、教师 API 和任务恢复。单讲来源只存于组件，不覆盖整课 CourseStore；源不可用只限制新生成，最后可用 PPT 继续展示。工作台切讲、切阶段、离开页面和上传替换前统一保存脏稿，失败保留现场；候选未决时提示先处理。长生成离开后继续在后台运行，返回时按当前讲和真实任务 ID 恢复观察。

验收区分界面流程、真实保存确认、实际模型规划及产物渲染。本次 UI 验收不替代第 12—14 节的跨学科与生产模板认证，也不改变三阶段引擎默认关闭策略。


## 2026-09-06：启智固定版式接入

用户确认直接实施代码，不先做两边真实模型对比；成果由教师实际使用评阅。新增默认内置版本 `fixed_classroom_v1`，继续沿用 `ppt_manuscript_v1/page_teaching_v2`、任务检查点、稿件确认和 `ResolvedPageScene`，没有第二套公开 API、内容仓库或任务系统。历史默认稿及 `teaching_layout_v2` 按各自版本恢复；显式三阶段开关和个人模板认证保持原行为。

从启智本地 renderer 吸收封面、提纲、章节、要点、问答、小结的固定层级与单栏布局，转成灵知共同场景；补充双对象对比、三至四步流程、公式与解释、已有图片与说明。`ppt_fixed_templates` 负责版式目录、命名字段、数量与文本容量、固定坐标和字号。`ppt_fixed_draft` 只给模型所选版式的填写表单，并转换成已有结构化教学表达；引用、修订、屏幕/备注、问答顺序继续使用既有验证器。

主链：整讲选定页序/版式/来源 → 最多四页并发填字段 → 当前页结构与容量修复 → 保存稿件 → 教师确认 → 固定场景生成 PPTX。没有额外视觉模型调用。失败时等待同批独立任务完成并保存通过页，重试只补未完成页，任务失败归属精确批次。模型可以在确认前将单页任务拆成同一版式的多页，每页必须说明拆分原因；渲染不得临时改版、缩字号或拆页。

固定版式导出按实际 PPTX 回读文字、对象几何、字体、关系端点与备注，并继续执行通用对象越界/重叠/溢出检查；此版本不强制 PDF/OCR 外部工具。网页预览消费同一冻结场景，不宣称是 PPTX 转图。旧三阶段/个人原生模板保留原 PDF 认证与导出检查，不用固定版式的检查策略放宽其合同。`pypdf` 未安装时只记录工具不可用，不阻止默认应用及固定场景生成。

验收限于必要代码回归、实际文件读回和前端构建，不进行真实模型对比、截图轮询或生产课程操作。真实模型成功率与教师对美观的评价由后续使用形成，不由本轮代码测试代替。


## 2026-09-07：完整版式与内容层级修正

用户否定 v1 的成品：固定坐标没有继承原模板设计，多个页型成为白底文本框，样例内容重复且空洞。v2 保留既有表单与任务链，将蓝色封面、紫色章节、层级正文、对比表、居中流程、问答分离、公式、图文、数据比较和代码组织为完整的有限版式；不在同一版本下静默改变旧稿。`ppt_classroom_design` 是构图真源，`SceneObject` 明确水平/垂直对齐与装饰随内容显示条件，新字段默认值不序列化，保持旧场景摘要。

要点和小结可填写短标题与解释，分别保存来源并保持独立可编辑；每种命名字段有与版式对应的容量。整讲规划说明每页新增的教学作用，禁止用提纲关键词反复凑页，问答按两张实际页面计入预算。单页请求保留整讲上下文、页序和已采用材料；数据和代码延用原有精确引用校验，不把所有教学内容简化成短句。

PPTX 与网页消费同一背景、位置、字号与对齐；原生回读新增文字颜色及对齐检查，不以零线宽代替无边框，避免深色封面出现细白框。真实渲染检查是成品交付的一部分，不增加真实模型对比实验。仓库提供 `scripts/export-classroom-template-sample.py --output-dir <仓库外目录>`，通过正式内容稿编译与导出器生成明确标注为教学示例的“平均数与中位数”，用于复核视觉效果；它不是实际模型生成成功的证据。
