...
左侧目录切页，右侧只挂载当前页预览和编辑；内容与排版使用现有分段控件。整讲安排、教学备注与来源按需展开。保存失败保留本地稿，保存成功保持当前页并刷新预览。原版 PPT 分支不显示讲义生成操作，成品 AI 审阅入口保留。

代码按统一仓库放入 `apps/lingzhi`，应用脚本、字体和模板版本随应用发布。主仓最新发布分流与其他任务改动保留。三阶段新建默认仍受既有生产验收门约束；推送 main 不等于已经开放默认新引擎。

## 工作台原地制作 PPT（2026-09-06）

教师在当前讲次的 PPT 阶段完成“1 内容稿 → 2 渲染 PPT → 3 查看与使用”。模板选择放在内容稿生成前；确认后只进入第二步，教师点击渲染才启动原生成任务。第三步承载现有预览、下载、页面详情和 AI 修改。资料设置按需打开，已有原版 PPT 继续进入原件审阅，不强制套入讲义生成流程。
