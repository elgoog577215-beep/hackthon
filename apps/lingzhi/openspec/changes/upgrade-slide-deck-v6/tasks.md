...
- [ ] 18.4 更新正式文档，安全推送 main，检查自动发布及实际版本；保留新引擎默认关闭与教学验收边界。

本轮交互验收（2026-09-06）：完整前端回归 181 个文件、1565 项通过；随后补充上传与生成互斥，相关 2 文件、112 项通过，最终构建与 OpenSpec strict 通过。中文桌面隔离工作台验证了编辑切讲保存、返回保留输入、确认不自动渲染、三步切换、成品详情、AI 修改入口、全屏、设置及 PPTX 下载。保存与确认为真实教师 API；生成过程为模拟，下载为已验证旧样本，不能替代 14.1、16.6 的新整讲及教学验收。发布结果待 18.4 完成后记录。
