...
- **AND** before confirmation the allocation expands to the declared safe pages needed for that selected range and all required conditions and results, using the lesson pacing budget as advice; exceeding it preserves a complete editable result with a suggestion
...
- **THEN** it first reduces optional screen exposition and counts proposed physical pages against an advisory lesson budget without rejecting a complete page solely for exceeding it
...
- **THEN** the draft contains nonblocking page-scoped or lesson-scoped suggestions and remains confirmable when source, structure and rendering checks pass
...
### Requirement: Focused manuscript editing
The workspace SHALL show a page directory and one active page with its saved scene preview, content editing and compatible presentation controls.

#### Scenario: A teacher switches pages with unsaved edits
- **WHEN** the teacher edits a page and selects another page
- **THEN** the local edits remain when returning to the first page
- **AND** saving keeps the active page selected and refreshes the saved preview
- **AND** a failed save preserves local edits and exposes recovery feedback
