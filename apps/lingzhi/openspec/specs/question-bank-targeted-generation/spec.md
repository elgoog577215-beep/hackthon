...
- **WHEN** 题目的 `node_id` 指向某讲的直接小节而不是讲次根节点
- **THEN** 系统 SHALL 将该题归入拥有该小节的正式讲次
- **AND** 讲次题目数 SHALL 包含该题

#### Scenario: 正式讲次无法覆盖现有题目

- **WHEN** 当前正式讲次及其小节没有一个能够匹配现有题目
- **THEN** 系统 SHALL 回退到原全部题目列表
- **AND** 系统 SHALL 不显示全部为零的讲次导航和单一“其他题目”分组
