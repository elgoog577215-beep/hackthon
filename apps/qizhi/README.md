...
启智与灵知统一在 [elgoog577215-beep/hackthon](https://github.com/elgoog577215-beep/hackthon) 维护。启智代码位于本目录；灵知代码位于 `apps/lingzhi`。普通 `git clone` / `git pull --ff-only` 即可取得两部分代码，不再维护子模块或单独同步版本。
...
| 灵知新版课程应用 | [灵知应用](../lingzhi/README.md) |
...
```bash
python3 -m venv apps/qizhi/server/.venv
apps/qizhi/server/.venv/bin/python -m pip install -r apps/qizhi/server/requirements.txt
npm --prefix apps/qizhi/client/website ci
cp -n apps/qizhi/server/.env.example apps/qizhi/server/.env
cp -n apps/qizhi/client/website/.env.example apps/qizhi/client/website/.env.local
```
...
cp -n deploy/zju/.env.example deploy/zju/.env
...
Compose 从 `apps/lingzhi` 构建灵知。项目名仍为 `edu-ai-home`，原有 `postgres_data`、`uploads_data`、`lingzhi_data` 卷名保持不变；合仓不迁移数据库、课程 owner 或线上运行数据。服务器切换到新代码目录前，按[部署说明](./docs/灵知新版课程接入与部署.md)备份并核对环境与卷。

GitHub 的 `Qizhi Checks` 检查源码结构和两个 Web 前端构建。只修改 `apps/qizhi/` 不触发灵知独立生产发布；启智服务器发布仍需使用经过验收的整个仓库提交。
...
最新仓库分流、构建产物和学校自动激活接通状态以[统一发布说明](../../deploy/README.md)为准。
