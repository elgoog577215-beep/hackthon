...
class TestMockStorageKnowledgeGraph:
    """知识图谱存储测试"""

    def test_save_and_load_graph(self, mock_storage: MockStorage):
        graph = {"nodes": [{"id": "n1"}], "edges": []}
        mock_storage.save_knowledge_graph("c1", graph)
        loaded = mock_storage.load_knowledge_graph("c1")
        assert loaded is not None
        assert len(loaded["nodes"]) == 1

    def test_load_nonexistent_graph(self, mock_storage: MockStorage):
        assert mock_storage.load_knowledge_graph("nonexistent") is None

    def test_delete_course_removes_graph(self, mock_storage: MockStorage):
        mock_storage.save_course("c1", make_course(course_id="c1"))
        mock_storage.save_knowledge_graph("c1", {"nodes": []})
        mock_storage.delete_course("c1")
        assert mock_storage.load_knowledge_graph("c1") is None
