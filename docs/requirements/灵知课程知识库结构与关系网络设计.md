...
现有 `SubjectKnowledgeLibrary` 退出新课程与运行主链：

- 不再提供跨课程术语、别名、覆盖检查或生成校准。
- 不进入学生产品、AI 上下文、课程发布、练习分析或个人掌握投影。
- 只允许在维护者显式启动旧数据迁移时读取。
- 迁移结果必须复制为当前 `course_id` 下的新知识身份，并解除旧运行时引用。
- 任何课程知识变化都不得传播到其他课程。
...
首次生成时，后续小节若再次使用前序知识，必须通过小节级 `reused_knowledge_names` 引用已经定义的规范名称。编译器将其解析为同一 `knowledge_id` 的强化、练习或应用绑定；引用尚未定义的名称，或者在 `knowledge_structure` 中重新创建同名知识点，都会阻断发布。
...
关系网的节点集合不是“已有边的端点集合”，而是当前
`CourseKnowledgeBase` 中全部活动原子知识点。系统必须先冻结这份课程内 ID
注册表，再规划关系；每个节点都必须留下 `connected / course_entry` 决定。
`course_entry` 可以没有入边，但仍属于关系网节点，不能从学生视图中消失。
...
→ 原子性、重复和边界审查
→ 生成能力点、易错点和掌握标准
→ 冻结全课知识节点并由系统分配课程内稳定 ID
→ 按小节关系邻域读取已冻结 ID 注册表
→ 逐一生成 connected / course_entry 决定及六类关系
→ 合并全课关系图并验证 ID、类型、自环、重复、决定完整性与环路
→ 依据知识与关系生成正文、课件和练习
→ 建立精确教学绑定
→ 全课一致性与知识质量门
...
2. 节点生成模型不得创建内部 ID；确定性服务分配 ID 后，关系模型只能复制允许
   上下文中的不透明 ID。确定性服务负责引用、无环、重复、决定完整性和修订检查。
...
## 13. 当前实现对账（2026-07-19）

本轮已经完成的是从知识生成到下游消费的第一条纵向主链，而不是全部知识治理能力：

- `course_knowledge_base_v2` 已包含 `ConceptGroup / KnowledgePoint / SkillUnit / Misconception / MasteryCriterion / KnowledgeRelation / KnowledgeBinding`；新写入不再生成 `ImprovementPoint`。
- 首次生成在同一 `GenerationJob` 工作区先形成课程知识蓝图，再生成正文和学习资产；知识结构不通过时不能进入正文生成或正式发布。
- `course_generation_v7` 先用一次紧凑调用冻结全课知识规范名称、唯一首次负责小节和
  前序复用，再以服务级有界并发生成逐节知识详情；全部节点编译课程内稳定 ID 后，
  关系阶段继续按当前小节、显式前置小节和本章前序小节组成邻域，用同一预算并行生成
  六类关系与逐点决定。两类并发结果都按课程顺序确定性合并，单元失败不会重生同批
  已通过节点或关系邻域。
- 新生成只写 `source_knowledge_id / target_knowledge_id`；编译器继续读取历史
  `source_name / target_name` 关系，避免旧课程失效。学生关系视图展示全部活动
  原子知识点，包括尚无直接边的课程入口。
- 机器门会阻断无法解析、没有可用知识陈述、偏离已冻结知识身份、关系端点或类型非法、
  逐点决定缺失和精确绑定失效；标题相近、分组数量、边界、能力、掌握标准与易错详略
  继续作为审阅提示，由六步工作台中的用户确认承担一般内容质量判断。
- 正文块、课程目标、正式题目、大纲、教案、讲义、练习册、`SlideDeckSpec` 与 AI 老师已携带同一批课程知识引用；AI 默认只装配当前位置的一跳关系子图。
- 学生视图只发布通过质量门的知识节点；历史结构若不能证明内容质量，只显示 `degraded / needs_enrichment`，不会把章节名称包装成知识点。
- 历史课程的“重新生成知识库”现在调用独立 AI 知识化服务，按小节分批提取 v2 结构，并在整个候选通过同一质量门后才保存；课程内容在生成期间发生变化时拒绝覆盖，失败时保留原课程。

真实存量扫描得到 27 门可读取课程，其中活动 v2 为 0、降级课程为 27；另有 1 个既有课程 JSON 因格式损坏无法加载。这说明旧内容没有被错误升级，也说明历史迁移尚未完成：应先单独修复损坏数据，其余课程由用户按课程触发知识化，后续再把同步分批调用升级为可恢复的后台批量任务。

仍未完成的全局能力包括：独立且原子保存的知识库仓库、知识点拆分 / 合并 / 移动等领域命令、旧新 ID 与历史证据映射、课程和知识的双向影响分析、关系驱动的最小断点诊断，以及至少三种教学模式的真实模型与浏览器验收。上述能力继续受本设计和现有 OpenSpec 约束，不得退回章节复制或整节泛绑定。
