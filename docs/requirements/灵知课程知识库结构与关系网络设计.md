...
- `course_generation_v6` 已把逐节知识节点生成和关系生成拆开：全部节点先冻结并
  编译课程内 ID，关系阶段再按当前小节、显式前置小节和本章前序小节组成邻域，
  用 ID 生成六类关系与逐点决定。关系邻域单独保存检查点，单批失败不会重生节点。
- 新生成只写 `source_knowledge_id / target_knowledge_id`；编译器继续读取历史
  `source_name / target_name` 关系，避免旧课程失效。学生关系视图展示全部活动
  原子知识点，包括尚无直接边的课程入口。
- 质量门会拒绝章节或小节标题复制、每组少于两个知识点、缺少知识命题、条件或边界、可观察能力、掌握标准、关系理由、有效端点和精确绑定等伪知识库结构。
