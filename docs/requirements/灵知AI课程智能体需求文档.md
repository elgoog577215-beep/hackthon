...
首次生成时，逐节知识节点生成与关系生成必须分开。系统先冻结全课知识节点并分配
课程内稳定 ID，再让独立关系阶段按小节邻域复制这些 ID，逐一提交
`connected / course_entry` 决定和六类关系，最终合并为一张全课关系图。模型
不得在节点阶段用名称顺手生成少量关系，也不得创建内部 ID；关系视图必须展示全部
活动原子知识点，而不是只展示已有关系边的端点。
