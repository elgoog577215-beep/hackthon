...
| 名称 | 定义 | 真源边界 |
| --- | --- | --- |
| `SubjectKnowledgeLibrary` | 跨课程、版本化的正式学科知识库 | 可为课程提供规范坐标；学生 AI 不得自动改写 |
| `CourseKnowledgeBase` | 与一门课程同时生成、服务当前课程、可继续细化的课程学科知识库 | 当前课程内可增长；节点可映射正式学科知识，也可保持课程局部候选 |
| `CourseKnowledgeMap` | 课程目录、正文块、目标、题目与知识节点之间的引用和覆盖关系 | 只表达映射、课程顺序和颗粒度，不复制正文或正式知识真源 |
...
### 3.1 “课程学科知识库”与正式知识库的关系

用户所说“生成课程时同时生成、之后还可以继续细化的学科知识库”，在工程上定义为 `CourseKnowledgeBase`。它不是第二套跨课程正式标准库，而是当前课程的可生长语义层：

```text
SubjectKnowledgeLibrary（跨课程正式坐标，可选种子与校准）
                    ↓ 映射 / 校验
CourseKnowledgeBase（当前课程生成、可细化、可出现局部候选）
                    ↕
CourseKnowledgeMap（知识与章节、块、目标、题目之间的关系）
                    ↕
CourseDocument（当前课程结构与内容）
```

这样既满足“知识库随课程生成和修改”，又不让某个学生的一次课程变化污染全校共享的正式知识标准。未来若要把优秀局部知识节点升级到启智或正式库，必须经过教师或治理流程，不能在本阶段自动发生。
...
nodes[]
relations[]
formal_library_revision_id?
generation_source_refs[]
quality_status
created_at / updated_at
```

`CourseKnowledgeNode` 最低字段：

```text
node_id
parent_id?
node_type                 domain / topic / concept / knowledge_point
name / description / aliases
granularity_status        coarse / adequate / fine / needs_refinement
formal_knowledge_ref?
source_refs[]
course_block_refs[]
objective_refs[]
practice_refs[]
...
允许的知识操作包括新增、补写、拆分、合并、重命名、调整父子关系、增加或删除关系、绑定或解除课程引用。删除使用墓碑和映射迁移，不物理抹除被历史事实引用的节点。
...
- 跨课程正式知识库不在这条自动闭环中，避免个人课程反向污染公共标准。
...
AI 发现一个粗知识节点同时承担三个可独立解释和诊断的概念，提出拆分。接受后，系统同时更新课程知识库和映射，并为缺少相应解释或检查的正文位置生成关联候选；已经覆盖的块不重复生成。

### 场景 9：课程内容反向补知识库

学生接受一个新增边界条件的正文修改。系统识别该内容未被课程知识库表达，生成补写现有节点或新增细知识点的候选；它不能未经治理自动写入跨课程正式学科库。
...
- 明确 `CourseKnowledgeBase` 与现有 `SubjectKnowledgeLibrary / CourseKnowledgeMap` 的边界。
...
| 知识测试 | 节点拆分合并、正式库映射、课程映射更新、引用完整性和无环 |
...
| `SubjectKnowledgeLibrary` | 保留跨课程正式坐标，不允许学生 AI 自动改写 |
| `CourseKnowledgeMap` | 扩展连接 `CourseKnowledgeBase`、正文、目标、题目和正式坐标 |
...
教师公共课程、学生个人课程和正式学科知识库必须是三个不同权限域。个人课程可以自由生长，但不能自动反向改写教师公共课程；学生共性问题可以成为启智中的审核候选，而不是直接发布。
