...
| 名称 | 定义 | 真源边界 |
| --- | --- | --- |
| `SubjectKnowledgeLibrary` | 可选的跨课程术语与生成参考目录 | 只提供术语、别名、覆盖检查或校准候选；不拥有当前课程知识身份，也不是发布和运行门禁 |
...
CourseKnowledgeMap / KnowledgeBinding（正文、课件、目标、题目与教学作用）
                    ↕
LearnerModel 与正式学习事实（当前学生状态覆盖层）

SubjectKnowledgeLibrary（可选术语与生成参考，不进入运行时身份链）
                    ···→ CourseKnowledgeBase
```
...
`SubjectKnowledgeLibrary` 匹配失败、缺失或版本落后时，不能阻断当前课程知识库生成、发布、AI 使用和个人掌握投影。完整实体、关系与质量门见 [`灵知课程知识库结构与关系网络设计.md`](./灵知课程知识库结构与关系网络设计.md)。
...
- 可选参考目录是否给出术语或别名建议；该结果不得改变当前课程知识 ID，也不得阻断候选。
...
- 可选跨课程参考目录不在自动闭环中，个人课程变化既不依赖它，也不得反向写入它。
...
学生接受一个新增边界条件的正文修改。系统识别该内容未被课程知识库表达，生成补写现有节点或新增原子知识点及课程内关系的候选；即使可选参考目录没有对应术语，当前课程候选仍可正常审查和发布。
...
- 固定“一门课程一套知识库”，将 `SubjectKnowledgeLibrary` 降为可选参考，将 `CourseKnowledgeMap / KnowledgeBinding` 固定为教学绑定层。
...
| 知识测试 | 原子性、概念组与知识点拆分合并、六类关系语义与无环、精确绑定、可选参考非阻塞、引用完整性和旧新 ID 映射 |
...
| `SubjectKnowledgeLibrary` | 降为可选术语与生成参考；不作为学生产品知识库、运行时身份或发布门禁 |
...
教师公共课程、学生个人课程和可选参考目录必须是三个不同权限域。课程知识 ID 不跨课程共享，个人课程可以自由生长，但不能自动反向改写教师公共课程或参考目录；学生共性问题可以成为启智中的审核候选，而不是直接发布。
