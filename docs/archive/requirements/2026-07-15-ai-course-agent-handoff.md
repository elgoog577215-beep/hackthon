...
> 文档状态：历史产品与技术交接快照，已由当前蓝图和 OpenSpec 接管<br>
> 日期：2026-07-15<br>
> 适用产品：灵知学生端<br>
> 读者：产品负责人、技术负责人、AI 工程、前后端、测试<br>
> 当前产品真源：[`docs/product-blueprint.md`](../../product-blueprint.md)<br>
> 当前领域合同：[`docs/requirements/README.md`](../../requirements/README.md)<br>
> 当前实施规格：[`openspec/changes/build-structured-adaptive-course-ai/`](../../../openspec/changes/build-structured-adaptive-course-ai/)

本文保留 2026-07-15 形成初始交接时的术语、缺口、阶段和未来判断。`CourseChangeSet`、当时的“未来 PPT”等表述不再代表当前协议；不得用本文新增接口、判断完成度或覆盖当前产品状态。
...
新生成和运行链不读取 `SubjectKnowledgeLibrary`。旧数据只有在维护者显式迁移时才能复制为当前课程内的新身份；迁移完成后不得保留运行时依赖。完整实体、关系与质量门见 [`灵知课程知识库结构与关系网络设计.md`](../../requirements/灵知课程知识库结构与关系网络设计.md)。
