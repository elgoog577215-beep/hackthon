...
- 目录阶段没有单门课程总小节数上限。已知不超过 6 节的小课可先走一次紧凑调用；
  更大的课程先用一份轻量章节骨架冻结课程定位、目标、章节顺序和各章小节数，再按章
...
`course_generation_v13 / course_prompt_v22` 先把目录、教案和正文拆成有边界、可恢复
的小工作单元，再把硬门作为最后安全熔断。目录与教案都会根据明确规模选择紧凑路径或
`1 → N → 1` 分片路径；每次只发送当前单元的最小必要上下文，不发送整课正文、完整
证据库或随着小节数平方增长的前后列表。多小节单元仍不适配时继续拆分，最小单元才
允许本地确定性保底。紧凑调用只是一条快速路径；请求超限、provider 失败或结构修复
无效时自动切回标准分片链。保底产物必须记录原因和人工复核，不得冒充完整 AI 质量
通过。
默认值如下：
...
| 目录紧凑路径 / 章节批次 | 6 节 / 每批 6 节 | 大课只生成轻量章节骨架；不同章并行，同章多批顺序衔接 |
| 教案骨架分片 | 6 节 | 全课知识身份连续增长，但任何一次骨架请求不携带整课 |
| 目录批次 / 目录工作窗口 | 60 秒 / 180 秒 | 到期保留骨架与已完成批次，只局部处理缺失单元 |
| 教案单元调用时限 | 60 秒 | 教案失败只降级当前最小单元并保留检查点 |
| 全课教案阶段时限 | 360 秒 | 到期以已有结果和本地保底汇编，不无限等待 |
| 单节正文 / 全部正文时限 | 75 秒 / 480 秒 | 无草稿失败可本地保底；有草稿则保存，阶段超时取消未完成小节并停在可恢复正文检查点，不进入发布准备 |
| 目录章节 / 教案批次 / 正文并发 | 4 / 4 / 4 | 并发可下调，代码级上限防止误配 |
...
时限和输出字符数；不保存 Prompt 正文、密钥或模型推理。恢复时只重做失败或未完成
