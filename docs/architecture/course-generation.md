...
/api/course-generation/generate
  ...
  -> 小课：一次紧凑教案调用 -> 本地封装 CourseTeachingPlanV3
     大课：有界连续骨架分片 -> 有界并行详细批次 -> 本地汇编 CourseTeachingPlanV3
  -> 本地编译 CourseKnowledgeBase、稳定 ID、关系图与逐节绑定
  -> TaskManager 只读取冻结教案，以固定并发生成互不等待的正文
  -> 确定性发布预检
  -> 用户确认发布
...
- 大课骨架是一份连续增长的全课身份产物，但默认每次调用最多处理 6 节，只读取这些
  小节直接需要的前序知识边界；本地合并器负责重排分片临时键并维持唯一负责、复用、
  前置和模块边界。每片通过后持久化连续前缀；服务重启时先校验前缀和目录修订，再从
  第一个未完成分片续跑。详细批次默认最多 3 节、15 个知识点，同一课程默认并发 4，每批只
  携带当前知识与直接依赖闭包，不再重复发送全课知识注册表。完成一批立即持久化；
  无法适配预算、定点纠正失败或提供方失败时只本地编译该最小单元，其他 AI 成功批次
  不丢失；乱序完成不能改变目录顺序、知识身份或最终修订。
...
`course_generation_v12 / course_prompt_v21` 先让工作量适配预算，再把硬门作为最后
安全熔断。目录、教案和正文会根据原始源规模跳过必然超限的富 Prompt，在完整、紧凑
和最小语义合同中选择可发送的最完整版本；多小节单元仍不适配时继续拆分，最小单元
才允许本地确定性保底。最多三节课程的紧凑单次调用只是一条快速路径；请求超限、
provider 失败或结构修复无效时自动切回标准骨架—批次链，再按最小单元处理。保底产物
必须记录原因和人工复核，不得冒充完整 AI 质量通过。
默认值如下：

| 边界 | 默认值 | 作用 |
|---|---:|---|
| 单门课程小节数 | 24 | 超出时在目录调用前拒绝并要求拆课 |
...
| 骨架分片 | 6 节 | 全课身份连续增长，但任何一次骨架请求不携带整课 |
| 目录 / 教案单元调用时限 | 90 秒 / 60 秒 | 教案失败只降级当前最小单元并保留检查点 |
| 全课教案阶段时限 | 360 秒 | 到期以已有结果和本地保底汇编，不无限等待 |
| 单节正文 / 全部正文时限 | 75 秒 / 480 秒 | 无草稿失败可本地保底；有草稿则保存，阶段超时取消未完成小节并停在可恢复正文检查点，不进入发布准备 |
| 教案批次 / 正文并发 | 4 / 4 | 并发可下调，代码级上限防止误配 |
| 正文应用层重试 | 1 | 复用已保存草稿续写，且仍受单节点总时限约束 |

环境变量只能在代码允许的上下界内调节，不能关闭输入、尝试次数、课程规模或总时限
保护。每个正式阶段和节点只保存非敏感运行指标，包括调用数、累计与最大输入估算、
实际 Prompt 细节等级、自适应拆分与降级单元、耗时、并发、时限和输出字符数；不保存
Prompt 正文、密钥或模型推理。恢复时只重做失败或未完成单元，不重复已经通过的目录、
骨架和教案批次。直接绕过自适应链的超大请求仍会由最终硬门在本地拒绝。
