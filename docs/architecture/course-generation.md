...
`course_generation_v15 / course_prompt_v22` 先把目录、教案和正文拆成有边界、可恢复
的小工作单元，再把硬门作为最后安全熔断。目录与教案都会根据明确规模选择紧凑路径或
`1 → N → 1` 分片路径；每次只发送当前单元的最小必要上下文，不发送整课正文、完整
证据库或随着小节数平方增长的前后列表。多小节单元仍不适配时继续拆分，最小单元才
使用本地确定性预览。紧凑调用只是一条快速路径；请求超限、provider 失败或结构修复
无效时自动切回标准分片链。目录局部保底依然需要用户确认；教案本地预览不得冒充完整 AI 语义质量，
必须停止下游生成并续跑失败单元。
默认值如下：
...
| 教案紧凑路径 / 骨架分片 / 详细单元 | 1 节 / 2 节 / 1 节 | 真实复跑显示两节完整教案已可逼近 8k 输出上限；详细语义按节并行，全课身份仍连续汇编 |
