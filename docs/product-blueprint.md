...
目录阶段不再用课程总小节数拒绝请求。最多 6 节且规模明确的小课可以先尝试一次紧凑
目录调用；大课先冻结轻量章节骨架，再以章节为并行边界生成小节目录；单章超过 6 节
时只让该章继续分片，最终在本地按章节和小节顺序汇编。章节批次只读取课程定位、当前
章节、相邻章节边界、当前章已完成小节和最多 4 条相关证据提示，不重复广播全课资料。
紧凑目录失败时切换标准章节骨架—批次链；章节骨架自身没有合法语义时保持失败，不用
通用占位目录冒充课程。已有合法章节骨架后，单个目录批次失败才允许
本地补齐当前最小批次，并在目录确认前显式要求人工复核。

只有 1 节的课程使用紧凑教案单次调用；该调用只是延迟优化，
放不下、提供方失败或结构纠正仍无效时自动切回同一标准骨架—批次链，不让小课拥有
另一套失败语义。其余课程先形成一份连续增长的全课骨架，但每次只处理最多 2 节并读取这些小节必要的前序知识边界。详细教案
