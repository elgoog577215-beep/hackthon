...
AI 负责把这些信息编译成教学块节奏。系统同时保留三条不同结果路径：
...
AI 的观察和规划范围可以很高，但不得替用户确认正式课程变化。单条非常明确的强证据可立即形成局部候选；跨章节或全课程候选必须来自用户明确的广域指令，或多个独立位置的一致证据。系统不得把固定“三次”当作通用硬门槛。
...
- **课程生长**：服务当前学习者。每条用户输入都可登记为原始证据，错题、笔记、诊断和反馈共同形成 `AdaptationHypothesis / CourseEvolutionPlan`；学生确认后通过同一课程领域命令改变当前 `CourseDocument`，反证可以触发缩小、撤销或替换。

两者的共同目标不是把两套状态生硬合并，而是持续维护同一门当前课程。教师修改由教学语义影响分析门禁控制；学习驱动修改由证据充分性、范围合理性和学生确认门禁控制。两者最终都提交版本化课程命令，因此不会形成“基础课 + 个人叠加课”两套长期正文。学习证据不得绕过计划和确认直接改写课程，也不得修改其他课程或静默改写课程知识库。

两条主线使用同一套可解释变化语法，但不合并判断状态机。教师在 PPT 中修改学习目标时，系统先识别“计算技能 → 概念理解”等教学语义变化，分别列出应联动和保持不变的课程单元，确认后更新当前课程并安全重建派生表达。学生侧只有当提问、学习记录和正式作答等证据共同指向同一缺口时，才形成课程生长方案；方案从紫色待确认进入蓝色已应用，必须经过同知识或能力的后续正式任务才能转为绿色已验证，候选内容播放、阅读或点击完成本身不计掌握。

学习现场统一采用四种变化状态：黄色表示课程维护中受到影响的位置，紫色表示 AI 提出的待确认课程变化，蓝色表示已应用但等待正式复验的课程变化，绿色表示后续独立证据支持该变化；后续证据显示无效或有害时改为橙色待复核。颜色只表达变化生命周期，不取代文字、图标和读屏状态。
...
18. 所有学习事实按可信用户身份隔离；身份不确定时不得写入正式画像。
...
26. 所有媒体必须具备来源、许可、模型版本、无障碍替代与派生修订；个人证据生成的媒体不得跨用户复用。
...
| --- | --- | --- | --- | --- |
| 1. 课程生产与当前课程内容 | 生成侧十项能力、异常生命周期、可见生成现场、全课一致性与课程知识库 v2 第一条纵向主链已形成 | 学科模式、难度契约、资料证据、统一生成工作区、`CourseDocument / CourseBlock`、块级候选与恢复；首次生成现在先产出“路径引用 → 概念组 → 原子知识点 → 能力 / 易错 / 掌握标准 + 六类关系”，禁止标题复制，并让正文块、目标、题目、教学表达和 AI 消费同一课程内 ID；生成、资产、AI 老师、知识审核和迁移普通路径已停止读取跨课程知识设施 | `CourseKnowledgeBase` 仍随课程元数据保存，尚无独立原子仓库、知识拆分合并等领域命令、修订映射和双向影响分析；真实预检的 28 个当前课程文件中，27 个可解析、1 个损坏，6 门虽有显式知识结构但均未通过 v2 质量门，其余 21 门没有可确认的显式结构，因此本轮没有强制迁移任何真实课程；大课程重建仍是同步分批调用 | 建立独立知识仓库与变更命令，把知识变化接入课程、课件、题目和历史证据的精确影响预览；先修复损坏课程数据，再通过独立 AI 知识化补齐缺失字段，并让可恢复后台迁移只原子保存通过同一质量门的结果 |
| 2. 学习工作区与 `LearningRuntime` | 正式学习现场已进入主链；课程生长候选、正文记录和课程修订已使用同一现场 | 课程库、左目录中正文的学习现场、块级 AI、按需全局 AI、正文任务入口、四类教学资源覆盖层、课程顶栏 PPT 一级入口与独立全屏工作台、断点提示、正文学习记录块、运行时适配块；确认后的解释、动画、理解检查和后续过渡成为正式 `CourseBlock`，不再由第二份长期覆盖层提供 | 尚无通用行级差异、个人知识状态提示、部分接受、统一方案历史视图和更复杂操作的目录定位 | 完成 `CourseDocument + PendingCourseEvolutionProjection + LearningRecord` 的完整投影与操作历史；阅读、任务、记录、知识、适配与 AI 继续共用同一学习现场 |
...
| 4. 学习者模型 | 唯一正式模型已进入主链；正式题目与诊断资产已携带当前课程知识和能力引用，但个人知识状态投影尚未完整切换 | `learner_model_v1` 从同批正式事实确定性重算，区分阅读、掌握、自评和推断，并返回状态、证据引用与置信度；新学习资产不再依赖外部学科包才能获得课程知识坐标 | 旧历史事实仍含旧知识引用；知识拆分合并后的当前投影、修订映射与历史解释规则尚未实现 | 按当前课程知识 ID 重算个人知识状态并保留发生时修订；补齐旧新 ID 映射，个人状态不得写回知识库 |
| 5. 统一 AI 老师 | 正式事实、学习者模型、单课程知识切片和主动课程生长方案已接入同一入口 | 块级解释、举例、简化、提问、原位追问、来源锚定、回答自动入正文、显式效果反馈、按需全局会话、`AIContextPackage`；AI 只读取当前课程中通过质量门的知识、能力、易错、掌握标准和一跳关系子图；有足够证据时会给出明确认知缺口、调整范围、验证方法以及接受、拒绝、撤销和效果状态 | 知识侧尚缺沿关系网定位最小前置断点的正式诊断器；课程生长侧尚缺行级操作、难度向量、部分接受和广域规划 | 分别扩展知识关系诊断与 `CourseEvolutionPlan` 操作代数；AI 可生成当前课程方案但不得替学生确认，不得修改其他课程或绕过独立知识维护门禁 |
| 6. 课程质量反馈与持续迭代 | 两条 Demo 纵向链已进入同一课程真源，知识库 v2 只提供当前课程语义坐标，但两个 OpenSpec 尚未全部完成 | 教师侧能从独立 PPT 工作台修改学习目标，把变化识别为教学维度变化；全屏影响图从真实绑定展开教案重点、讲义引导、相关讲解/例题与理解检查，同时列出明确保持不变的单元，经确认才写入正式课程目标并安全发布五类同源表达；同步回执逐单元展示真实前后差异，并区分实际改写与来源校验。真实《矩阵与线性变换》课程已按“整套规划 → 逐页填充 → 质量门”生成 25 页中文课件，LaTeX 会统一转译为可直接展示的数学符号、Unicode 上下标、矩阵/方程组括号和运算箭头；原始命令、替换字符与错误解码会阻断发布，并已在原生 PowerPoint 验证可编辑对象、讲者备注及公式无乱码；学生侧形成 `提问 + 学习记录 + 正式作答 → 明确认知缺口 → CourseEvolutionPlan → 目录范围 → 接受/拒绝 → CourseDocument 新修订 → 动画交互 → 正式任务复验 → 效果状态` | 同源侧尚缺真正的单页/单段/单题组异步增量构建、跨产物深层一致性、通用图解和外部 PPT 回导；课程生长侧尚缺完整操作代数、跨域恢复、行级变化、部分接受、历史视图和真实长期效果样本 | 分别完成剩余故障矩阵和真实长期效果验收；最后统一验证两条触发链对同一课程修订的并发、冲突、撤销和派生表达重建，任何学习证据都不得跨课程或直接改知识库 |
...
→ 用户确认 → CourseDocument 新修订 → 动画交互 → 正式独立复验
→ 蓝色等待证据 / 绿色验证有效 / 橙色待复核
