...
- 用户提交后立即进入与该课程卡绑定的“生成现场”，同时看到需求整理、资料解析、教学结构、课程蓝图、章节正文、学习资产、质量检查和发布等真实阶段；生成现场属于任务详情，不建立独立课程工作室。
...
| --- | --- | --- | --- | --- |
| 1. 课程生产与当前课程内容 | 生成侧十项能力、正常运行和异常生命周期已形成主链；个体课程生长完成需求与 OpenSpec 设计，尚未实现 | 学科模式、难度契约、资料证据、课程蓝图、统一生成任务、隔离工作区、创建补偿、请求幂等、暂停续跑、取消删除级联、分层质量门、带建议发布、`CourseDocument / CourseBlock` 仓库与命令、块级候选、恢复描述与修订冲突保护；后端已产生阶段进度、节点状态和正文增量事件；任务、课程库、正式文档和学习页已完成纵向对账 | 前端生成任务仍主要展示百分比和阶段名，没有把真实正文增量装配为可恢复的生成现场；当前也只支持单块替换候选，尚无随课程生成的 `CourseKnowledgeBase`、完整结构操作、跨域变更集和知识双向影响分析 | 先让任务详情呈现按章节隔离、草稿与定稿分层、刷新可恢复的真实生成过程；随后按 `build-structured-adaptive-course-ai` 实现课程知识层、通用 `CourseChangeSet`、完整课程命令、修订映射与可恢复命令组；个人节点不得自造正式知识 ID |
| 2. 学习工作区与 `LearningRuntime` | 正式学习现场已进入主链；持久候选层已定稿，尚未实现 | 课程库、左目录中正文的学习现场、块级 AI、按需全局 AI、正文任务入口、覆盖层、断点提示、正文学习记录块、运行时适配块；知识库提供学科语义树、本课覆盖、能力/易错/提升、正文定位和移动端详情 | 尚无行级高亮、未来章节目录标记、目录范围选择、跨会话待确认变化和统一候选历史 | 建立 `CourseDocument + PendingChangeOverlay` 投影和唯一候选 Store；阅读、任务、记录、知识、候选与 AI 继续共用同一学习现场 |
| 3. 学习事实与正式证据 | 正式主链已完成；面向课程适应的原始证据索引尚未实现 | `LearningEvent` 通用幂等与领域关联、`LearningSnapshot`、`LearningRecord`、`PracticeAttempt`、诊断、补救、独立复验与安装级匿名身份 | 当前每条用户输入尚未作为 `EvidenceItem` 接入课程适应；缺少原始证据、反证和适应假设的分层 | 在不复制全文、不污染正式模型的前提下登记当前课程思维与事实证据，完成来源查看、课程隔离、纠正、删除和导出验收 |
...
| 5. 统一 AI 老师 | 正式事实、学习者模型和正式知识切片已接入；主动课程生长仅完成规格 | 块级解释、举例、简化、提问、原位追问、来源锚定、显式回答反馈、按需全局会话、`AIContextPackage`、动作提案、确认、回执和撤销 | 尚不能从动态证据形成局部、依赖范围或广域 `CourseChangeSet`，也不能提前为后续章节准备持久候选 | 实现证据解释器、混合决策器、课程规划器、影响分析器和质量评估器；AI 可高权限生成候选但不得替用户确认 |
| 6. 课程质量反馈与持续迭代 | “结构化同源 + 个体化生长”完整需求、设计与任务已建立，代码尚未进入正式主链 | 生成侧蓝图、节点和最终质量检查，块级候选、正式学习事实、学习者模型和 AI 提案回执可复用 | 缺少 `EvidenceItem → AdaptationHypothesis → CourseChangeSet → 用户确认 → 效果复验` 主链；尚无动态门槛、范围、反证、课程知识双向联动和修改效果基线 | 按 OpenSpec 六个依赖阶段全部实现，并用真实样本完成至少一条“发现问题 → 改变课程 → 后续改善或回退”的闭环 |
...
| 4. 大纲预览与修改 | 生成前蓝图审阅已接入统一首次生成链 | 蓝图读取、修改和确认使用同一任务工作区，正式课程在质量通过前保持空外壳；生成后自然语言改写已在第 8 项完成块级候选与课程命令接管，整体结构级修改仍待后续命令补全 |
| 5. 多课程并行生成 | 统一调度与隔离工作区已进入新主链，两门课程并行运行、独立取消和结果归属已完成真实验收 | 继续验证更高并发下的公平调度、模型限流、进程中断和 provider 容量边界 |
| 6. 进度与过程可见 | 统一任务状态、阶段进度、节点增量事件与前端任务中心已经存在；新任务、WebSocket 完成、轮询完成和本地缓存恢复均按服务端真源对账，但用户目前仍只看到百分比与阶段名 | 把现有 `stream_chunk` 从内存缓冲接入任务详情，形成按章节隔离的真实正文流，并补齐 `task_id / sequence / draft_revision`、服务端累计草稿读取、断线追帧、草稿/定稿替换和并行章节切换；继续验证多课程切换、并发压力、阶段准确性及刷新后无丢字、重字、串流 |
...
| 10. 质量检查与课程优化 | 生成侧质量检查和发布分级已完成；学习后持续优化归整体产品模块 6 | 严格质量结论与最低发布许可分别表达，完整课程可带非阻断建议发布，关键内容、证据、必用资料和资产缺陷继续阻断；“反复困惑或错误 → 质量问题 → 改进提案 → 确认修改 → 效果复验”作为跨课程、事实、模型和 AI 的独立闭环推进 |
...
→ [随后继续：产品模块 6，规格已完成、实现待开始]
CourseKnowledgeBase + EvidenceItem + AdaptationHypothesis + CourseChangeSet
→ 动态局部 / 依赖 / 广域候选
→ 理由与范围审阅
→ 可恢复领域命令组
→ 修改效果复验
...
同日确认课程生成必须进一步从“状态可见”升级为“产出过程可见”：当前后端已经在节点生成时发送真实 `stream_chunk`，前端 Store 也会累计正文增量，但任务中心只渲染阶段、百分比和节点数量，正式课程又因隔离工作区在质量通过前保持空外壳，因此用户无法看到已经发生的生成。目标不是播放假日志或暴露模型隐藏推理，而是在同一任务详情中按章节展示真实正文增量、草稿/定稿状态和可恢复检查点；关闭、刷新和断网重连后仍以服务端工作区为真源继续显示。本条代表产品契约确立，不代表前端生成现场和增量恢复协议已经完成。
...
2026-07-15 已完成模块 6 的产品与实施定稿：最高原则收束为“结构化同源 + 个体化生长”；课程新增可生长但不污染正式库的 `CourseKnowledgeBase`；当前课程每条用户输入与正式学习对象可登记为原始证据；动态证据判断不使用固定次数；AI 可规划当前、向后和向前的完整结构修改，但一律进入持久候选层。详细真源为 `docs/requirements/灵知AI课程智能体需求文档.md`，实施变更为 `openspec/changes/build-structured-adaptive-course-ai/`。本条只代表设计完成，不代表代码能力已经存在。
...
这不代表第 10 节的目标架构已经全部成立。当前已接管一门标准历史课程、所有新课程的首次生成发布、规范课程的单块局部重新生成及两条生成链的失败恢复；其余历史课程与其他旧节点修改接口仍未完全收口。`CourseKnowledgeBase`、原始证据索引、适应假设、通用课程变更集、未来章节候选和效果复验目前只有需求与 OpenSpec，尚未实现。下一阶段按 `build-structured-adaptive-course-ai` 的依赖顺序推进，不能把文档定稿误报为功能完成。
