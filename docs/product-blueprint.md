...
>
> 2026-07-19 首次课程生成链路修正：新课程默认且只走“需求 → 目录 → 课程生成 → 确认发布”四步生产线。提交需求视为确认第 1 步；用户确认目录后，系统一次规划全课所有小节教案，再由同一教案确定性编译知识库、稳定 ID、关系图和逐节绑定，并按真实硬前置并行生成正文。教案与知识库不再作为独立用户确认门；最终发布前仍必须通过同源版本链检查。
>
...
- 用户提交后立即进入与该课程卡绑定的“生成现场”，看到需求整理、资料解析、课程目录、全课小节教案规划、知识库本地编译、章节正文、学习资产、结构预检和发布等真实内部阶段；用户步骤只保留需求、目录、课程生成和确认发布。生成现场直接复用学习工作区的左侧目录与中间正文，不建立独立课程工作室；任务中心只保留跨课程总览、暂停、取消、恢复和错误处理。
...
- 新课程先一次规划当前 `course_id` 的全课小节教案，再从同一计划本地编译 `CourseKnowledgeBase`、稳定 ID、六类关系和精确教学绑定；生成与发布不得读取跨课程知识参考，也不得另行调用知识或图谱模型。
...
→ 一次整课 CourseTeachingPlan 调用规划所有小节：原子知识、能力、掌握标准、首次负责/复用和局部课程块强调
...
3. **课程生成**：系统一次规划全课小节教案，本地编译知识库与关系图，并行生成可进入学习现场审阅的完整正文和资产；用户在最终课程形态上确认。
...
课程生产中的每个阶段都必须保存独立、可校验的正式中间产物。全课教案不得生成
...
或一般化环路。结构输出损坏时最多执行一次定向纠正，第二次仍不合法则保留目录
检查点并明确失败。系统不得按小节补洞，不得用综合质量分追加整课 AI 修复，任何
半成品也不能混入当前课程。

全课规划的正常模型调用数不随小节数增长：无论课程有 2 节还是 21 节，整课教案
固定一次，知识库模型调用为零，关系图模型调用为零。正文仍按稳定教学合同逐节生成，
以保留每节足够的上下文和输出预算；教案通过后，满足硬前置的正文立即按波次并行，
不再等待第二个全局模型。正文并发继续使用服务级预算，结果按课程顺序确定性合并，
单个正文失败时保留教案、知识库和其他已完成草稿。
...
资产与发布预检从生成工作区中清除，再从新目录一次规划全课教案。当前只开放目录的
...
| 10. 结构发布门与课程优化 | 生成后 AI 评分、AI 审稿、全课 AI 修复和逐题 AI 预检已退出首次生成主链；发布前只保留结构、ID、引用、覆盖、依赖、绑定、答案合同和同源版本链硬门禁 | 通过生产指标验证整门课只调用一次 `CourseTeachingPlan`，知识库与关系图模型调用均为 0，正文在教案通过结构门后按依赖波次并行；学习后的“反复困惑或错误 → 改进提案 → 确认修改 → 效果复验”仍按整体产品模块 6 推进 |
...
2026-07-19 首次生成主链最终升级为 `course_generation_v9 / course_prompt_v17`。
...
`answer_spec` 重新写回公开题目或另建一套题库。来源不足、高风险、私有解答合同与题面不一致和未知
