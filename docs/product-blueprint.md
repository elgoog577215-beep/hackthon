...
通用占位目录冒充课程。已有合法章节骨架后，单个目录批次失败或总工作窗口到期才允许
本地补齐当前最小批次，并在目录确认前显式要求人工复核。

最多 3 节且输入/输出预算安全的小课可以先尝试紧凑教案单次调用；该调用只是延迟优化，
...
尝试。目录小单元默认 60 秒、目录可恢复工作窗口 180 秒、教案单元 60 秒、全课教案
360 秒；流式正文不设单节或整课固定总时限，连续 90 秒无新内容才停止当前小节。
