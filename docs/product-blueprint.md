...
| --- | --- | --- | --- | --- |
| 1. 课程生产与当前课程内容 | 生成侧十项能力、正常运行、异常生命周期、首版可见生成现场、课程知识库首次生成和全课一致性已形成主链；课程知识结构重构仍处于设计完成、实现未开始 | 学科模式、难度契约、资料证据、课程蓝图硬约束、统一生成任务、隔离工作区、创建补偿、请求幂等、暂停续跑、取消删除级联、分层质量门、带建议发布、`CourseDocument / CourseBlock` 仓库与命令、块级候选、恢复描述与修订冲突保护；首次生成已有课程局部知识、能力、易错、学习资产和发布门的初步引用 | 当前知识点仍可能由章节标题投影、颗粒度过粗、整节批量绑定、通用易错与提升模板填充；尚无“章节 / 小节路径引用 → 概念组 → 原子知识点 → 能力 / 易错 / 掌握标准”的完整 schema、六类关系网、独立修订仓库和精确影响分析 | 按 `灵知课程知识库结构与关系网络设计.md` 先完成 schema、原子性与关系质量门、禁止标题复制的生成链，再接正文、课件、题目、AI 和证据的精确绑定 |
...
| 4. 学习者模型 | 唯一正式模型已进入主链，但知识状态仍主要依赖旧正式知识引用，尚未全面切换到当前课程知识坐标 | `learner_model_v1` 从同批正式事实确定性重算，区分阅读、掌握、自评和推断，并返回状态、证据引用与置信度 | 无外部知识包课程的局部知识 ID 尚未完整进入作答、诊断、AI 和状态投影；知识拆分合并后的当前投影与历史解释规则尚未实现 | 让正式题目、诊断和学习事实精确引用当前课程知识与能力 ID，按同一课程重算个人覆盖层；历史事实保留发生时修订，个人状态不得写回知识库 |
...
2026-07-17 已完成课程知识库结构与关系网络的产品定稿：灵知明确采用“一门课程一套知识库”，`CourseKnowledgeBase` 成为当前课程唯一产品知识库和运行时知识坐标；学生视图按“章节 / 小节路径引用 → 概念组 → 原子知识点 → 能力 / 易错 / 掌握标准”展开，横向只保留前置、推导、等价、对比、应用和一般化六类可驱动教学动作的关系。`SubjectKnowledgeLibrary` 降为可选术语与生成校准参考；`ImprovementPoint` 退出新写入核心实体，稳定目标归能力、具体任务归练习、个人提升归 AI 建议。详细数据契约、生成链、质量门、绑定协议和实施顺序见 `docs/requirements/灵知课程知识库结构与关系网络设计.md`；本条只代表设计与规格完成，不代表生成代码和下游绑定已经重构。
...
- 将 `CourseKnowledgeBase` 升级为当前课程唯一产品知识库，建立概念组、原子知识点、能力、易错、掌握标准、六类关系与精确绑定，并完成课程与知识的双向影响分析。
