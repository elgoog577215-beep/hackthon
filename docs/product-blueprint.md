...
>
> 2026-07-17 边界修正：灵知按“结构化同源 + 个体化生长”建设，但两者不是同一条变化链。结构化同源维护基础 `CourseDocument / CourseKnowledgeBase` 及其教学表达；个体化生长把学习证据编译为 `PersonalAdaptationPlan`，确认后写入 `PersonalCourseOverlay`，不得直接修改基础课程。两者只共享稳定课程/知识引用、修订、任务和审计底座。
>
...
flowchart TB
    ...
    subgraph AI["统一 AI 助手：一个入口，多种现场能力"]
        ...
        AUTHORING["CourseAuthoringChange<br/>基础课程语义修改与影响"]
        PERSONAL["PersonalAdaptationPlan<br/>个人证据、范围、适配与效果"]
    ...
    ANSWER --> LEARN
    ADAPT --> STREAM
    AUTHORING -->|"未确认基础课程差异"| STREAM
    PERSONAL -->|"未确认个人适配差异"| STREAM
    COMMAND -->|"教师/维护者确认后的基础课程操作"| DOC
    COMMAND -->|"教师/维护者确认后的课程知识与映射操作"| CKB
    PERSONAL -->|"学生确认后写入 PersonalCourseOverlay"| RESOLVE
...
→ 学习者模型
→ 当前课程原始证据与可解释适应假设
→ AI 助手回答、自动插入低风险临时教学块，或生成个人适配方案
→ 学生接受 / 拒绝 / 重生 / 调整范围
→ PersonalCourseOverlay 安全投影个人变化，基础课程与课程知识库保持不变
→ 对修改后的学习效果继续取证
→ 用后续正式证据评价内容变化与表达选择
→ 新的学习行为
```
...
2. **个人适配方案**：AI 可以规划个人解释、例子、桥接、检查点、补救路径、难度支架和后续章节预调；方案长期高亮保留，只有学生确认后才写入 `PersonalCourseOverlay`，不修改基础 `CourseDocument`。
...
- 未确认时跨刷新与退出持续保留；确认后只改变个人覆盖层修订，基础课程修订始终不变。
...
```text
稳定 CourseBlock
+ 持久待确认 PersonalAdaptationPlan 投影
+ 临时 AdaptiveBlock
...
灵知后续 AI 能力由两条独立业务主线统领；它们共享知识坐标和工程底座，但不共享业务状态机：

- **结构化同源**：服务教师和课程维护者。基础课程结构、课程知识、目标与题目各有唯一真源；一次经确认的语义修改通过 `CourseAuthoringChange`、课程领域命令和派生依赖图联动正文、讲义、PPT 与练习。
- **个体化生长**：服务当前学习者。每条用户输入都可登记为原始证据，错题、笔记、诊断和反馈共同形成 `AdaptationHypothesis / PersonalAdaptationPlan`；确认后只改变 `PersonalCourseOverlay`，反证可以缩小、撤销或替换个人适配。

两者只能通过受控接口协作：基础课程更新后重新定位个人覆盖层；多名学习者长期出现同类问题时只能形成基础课程改进建议，仍须课程维护者确认。个人证据不得直接修改基础课程或课程知识库。

两条主线使用同一套可解释变化语法，但不合并状态机。教师在 PPT 中修改学习目标时，系统先识别“计算技能 → 概念理解”等教学语义变化，分别列出应联动和保持不变的课程单元，确认后才更新基础课程并安全重建派生表达。学生侧只有当提问、学习记录和正式作答等证据共同指向同一缺口时，才形成个人适配方案；方案从紫色待确认进入蓝色已应用，必须经过同知识或能力的后续正式任务才能转为绿色已验证，候选内容播放、阅读或点击完成本身不计掌握。

学习现场统一采用四种变化状态：黄色表示基础课程维护中受到影响的位置，紫色表示 AI 提出的个人待确认变化，蓝色表示已应用但等待正式复验的个人内容，绿色表示后续独立证据支持该变化；后续证据显示无效或有害时改为橙色待复核。颜色只表达变化生命周期，不取代文字、图标和读屏状态。

灵知实行“一门课程一套知识库”：`CourseKnowledgeBase` 是当前课程唯一的产品知识库和运行时知识坐标，内部以“章节 / 小节路径引用 → 概念组 → 原子知识点 → 能力点 / 易错点 / 掌握标准”组织，知识点之间只保留能驱动教学动作的前置、推导、等价、对比、应用和一般化关系。`CourseKnowledgeMap / KnowledgeBinding` 连接知识与课程块、目标、课件和题目；现有 `SubjectKnowledgeLibrary` 只作为可选术语与生成校准参考，不进入学生心智，也不作为发布、AI 使用或个人掌握的必选前置。
...
| `SubjectKnowledgeLibrary`（兼容参考） | 可选提供规范术语、别名、覆盖检查和生成校准候选 | 不作为学生知识库、运行主链、必须映射目标或当前课程知识身份 |
...
| `CourseAuthoringChange / AuthoringOperation` | 教师或课程维护者对基础课程语义的结构化候选、影响与确认 | 不消费个人证据自动写课，不绕过基础课程领域命令 |
| `PersonalAdaptationPlan / PersonalAdaptationOperation` | 学习证据驱动的个人解释、支架、检查与路径适配 | 只写 `PersonalCourseOverlay`，不修改基础课程或课程知识库 |
| `PersonalCourseOverlay` | 当前学习者已确认的稀疏个人变化和基础修订引用 | 不复制完整课程，不成为公共课程真源 |
...
- 新课程先生成 `CourseKnowledgeBase` 知识蓝图，再与课程块、课件、目标和题目建立可追溯的精确教学绑定；可选术语参考不得成为发布前置。
...
- 关系网络：知识点之间只允许前置、推导、等价、对比、应用和一般化六类关系；父子层级、课程顺序、内容绑定和无具体教学含义的 `related` 不进入正式语义关系网。
...
→ 教学 brief
→ 学习目标、章节与小节唯一责任
→ 概念组、原子知识点、能力、易错、掌握标准与六类关系组成知识蓝图
→ 知识原子性与关系语义审查
→ 蓝图硬约束验收 + 全课一致性契约
→ CourseBlock 编排计划
→ 分块生成与正式资产生成
...
→ 生成概念组与原子知识点
→ 生成能力、可信易错和掌握标准
→ 生成六类知识关系及理由、条件和联合关系组
→ CourseKnowledgeMap / KnowledgeBinding 保存知识与正文、目标、课件、题目及记录的教学作用
→ 内容生成、教学表达、练习、诊断、AI 老师和学习者模型共享当前课程知识坐标
```

新课程必须先通过知识原子性、关系结构与语义、能力可观察性和绑定精度质量门，再共同发布课程与知识库。旧课程缺少真实结构时保留可阅读状态并标记 `degraded / needs_enrichment`，不得再把章节标题自动复制成同名知识点。现有 `SubjectKnowledgeLibrary` 可以提供术语候选，但匹配结果不改变当前课程知识库的身份和可用性。

#### 学习运行链

```text
当前 CourseDocument
+ 当前用户 PersonalCourseOverlay / PendingAdaptationOverlay
...
```text
持久 CourseBlock 流
+ PersonalAdaptationPlan 投影的待确认变化
...
→ PersonalAdaptationPlan：个人内容 + 支架 + 检查点 + 路径操作
→ 持久高亮候选：理由 + 范围 + 影响
→ 用户接受 / 拒绝 / 重生 / 部分接受
→ 写入 PersonalCourseOverlay 的可恢复个人操作组
→ 修改前后同类证据比较
```
...
- 替学生确认 `PersonalAdaptationPlan`，或把个人适配自动写入基础课程、课程知识库、其他学习者或跨课程参考目录。
...
11. AI 可主动生成合法个人适配方案，但未确认方案只作为投影；已确认方案也只能改变 `PersonalCourseOverlay`，不能改变基础课程修订。
...
26. 静态课程检查不得冒充个性化反馈；多任务或长参考内容必须保留任务级结构，数学表达与程序代码必须使用不同语义边界。
...
`P0 / P1 / R1 / R2` 只允许描述施工依赖，不允许替代产品验收。灵知后续按两条纵向闭环分别验收：结构化同源必须从基础课程真源编译多种教学表达、精确追踪来源、识别影响并安全回写基础课程；个体化生长必须从个人证据形成可解释假设和适配方案，经学生决定后只改变 `PersonalCourseOverlay`，并能撤销和用后续表现判断效果。任一链可以独立达到闭环；整体验收额外检查两者通过稳定引用协作且绝不越权写入。

### 9.2 整体产品实施全景

| 产品模块 | 当前成熟度 | 已有且应保留的能力 | 当前真实缺口 | 进入下一成熟度的门槛 |
| --- | --- | --- | --- | --- |
| 1. 课程生产与当前课程内容 | 生成侧十项能力、异常生命周期、可见生成现场、全课一致性与课程知识库 v2 第一条纵向主链已形成 | 学科模式、难度契约、资料证据、统一生成工作区、`CourseDocument / CourseBlock`、块级候选与恢复；首次生成现在先产出“路径引用 → 概念组 → 原子知识点 → 能力 / 易错 / 掌握标准 + 六类关系”，禁止标题复制，并让正文块、目标、题目、教学表达和 AI 消费同一课程内 ID | `CourseKnowledgeBase` 仍随课程元数据保存，尚无独立原子仓库、知识拆分合并等领域命令、修订映射和双向影响分析；历史 27 门可读取课程均被真实标记为 `degraded`，另有 1 个既有课程 JSON 损坏而无法读取；大课程重建仍是同步分批调用 | 建立独立知识仓库与变更命令，把知识变化接入课程、课件、题目和历史证据的精确影响预览；先修复损坏课程数据，再把按课程重建升级为可恢复后台迁移，并用多教学模式真实课程验收 |
| 2. 学习工作区与 `LearningRuntime` | 正式学习现场已进入主链；个人适配方案和已接受个人补充已接入运行时 | 课程库、左目录中正文的学习现场、块级 AI、按需全局 AI、正文任务入口、教学资源覆盖层、断点提示、正文学习记录块、运行时适配块；已接受的个人解释、动画、理解检查和后续过渡仍在同一正文流展示；左目录按受影响小节及其父节点显示紫色待确认、蓝色已应用、绿色已验证和橙色待复核数量 | 尚无通用行级差异、个人知识状态提示、部分接受、统一方案历史视图和更复杂操作的目录定位 | 把当前受控个人补充扩展为完整 `CourseDocument + PersonalCourseOverlay + PendingAdaptationOverlay` 投影；阅读、任务、记录、知识、适配与 AI 继续共用同一学习现场 |
...
| 4. 学习者模型 | 唯一正式模型已进入主链；正式题目与诊断资产已携带当前课程知识和能力引用，但个人知识状态投影尚未完整切换 | `learner_model_v1` 从同批正式事实确定性重算，区分阅读、掌握、自评和推断，并返回状态、证据引用与置信度；新学习资产不再依赖外部学科包才能获得课程知识坐标 | 旧历史事实仍含旧知识引用；知识拆分合并后的当前投影、修订映射与历史解释规则尚未实现 | 按课程知识 ID 重算个人覆盖层并保留发生时修订；补齐旧新 ID 映射，个人状态不得写回知识库 |
| 5. 统一 AI 老师 | 正式事实、学习者模型、课程知识切片和主动个人适配方案已接入同一入口 | 块级解释、举例、简化、提问、原位追问、来源锚定、显式回答反馈、按需全局会话、`AIContextPackage`；AI 只读取通过质量门的当前位置知识、能力、易错、掌握标准和一跳关系子图；有足够个人证据时会把提问、记录和正式作答并列展示，给出明确认知缺口、知识/能力/易错命中、调整范围、验证方法以及接受、拒绝、撤销和效果状态，知识或证据不足时不伪造建议 | 知识侧尚缺沿关系网定位最小前置断点的正式诊断器；个人侧尚缺行级覆盖、个人难度向量、部分接受、重新生成和广域规划 | 分别扩展知识关系诊断与 `PersonalAdaptationPlan` 操作代数；AI 可生成个人方案但不得替学生确认，也不得修改基础课程或课程知识库；基础课程改进只能进入独立 `CourseAuthoringChange` 链路 |
| 6. 课程质量反馈与持续迭代 | 两条 Demo 纵向链均已进入正式主链，知识库 v2 是共享语义坐标，但两个 OpenSpec 尚未全部完成 | 教师侧能把 PPT 学习目标的语义变化识别为教学维度变化，动态区分受影响和保持不变单元，经确认写入正式课程目标，并安全发布五类同源表达；学生侧已形成 `提问 + 学习记录 + 正式作答 → 明确认知缺口 → PersonalAdaptationPlan → 目录范围 → 接受/拒绝 → PersonalCourseOverlay → 动画交互 → 正式任务复验 → 效果状态`，且个人适配不越权写基础课程 | 同源侧尚缺真正的单页/单段/单题组异步增量构建、跨产物深层一致性、通用图解和外部 PPT 回导；个人侧尚缺完整适配操作代数、知识修订后的覆盖重定位、跨域恢复、行级覆盖、部分接受、历史视图和真实长期效果样本 | 分别完成剩余故障矩阵和真实长期效果验收；最后只做基础课程更新后的个人覆盖重定位、群体问题转教师建议这两类受控协作，个人证据不得直接写入基础课程或知识库 |
...
教师修改 PPT 学习目标 → 识别教学语义变化 → 动态影响与保持不变清单
→ 用户确认 → 正式课程目标修订 → 五类表达安全发布

学生提问 + 学习记录 + 正式错题 → 明确认知缺口 → 当前与后续范围候选
→ 用户确认 → PersonalCourseOverlay → 动画交互 → 正式独立复验
→ 蓝色等待证据 / 绿色验证有效 / 橙色待复核

→ [下一收束门槛：不按 P0/P1 拆散]
同源表达真正的单页/单段/单题组异步增量构建与跨产物一致性
+ 个人覆盖重定位、完整操作代数、行级候选、跨域恢复和长期效果样本
...
2026-07-17 已完成课程知识库结构与关系网络的产品定稿，并落地第一条 v2 纵向实现：灵知采用“一门课程一套知识库”，`CourseKnowledgeBase` 成为当前课程唯一产品知识库和运行时知识坐标；学生视图按“章节 / 小节路径引用 → 概念组 → 原子知识点 → 能力 / 易错 / 掌握标准”展开，横向只保留前置、推导、等价、对比、应用和一般化六类可驱动教学动作的关系。新课在同一生成工作区先形成知识蓝图，标题复制、缺定义或边界、无可观察能力、无掌握标准、无理由或悬空关系、无精确绑定都会阻断发布；正文块、目标、题目、教学表达与 AI 使用同一课程内 ID，`SubjectKnowledgeLibrary` 只作可选术语参考，`ImprovementPoint` 停止新写入。历史课程通过独立 AI 知识化入口分批重建，只有通过同一质量门才原子保存；真实扫描的 27 门可读取课程当前全部保持 `degraded`，没有被标题投影伪装成正式知识库，另有 1 个既有课程 JSON 因格式损坏无法进入扫描。当前尚未完成损坏数据修复、独立知识仓库、知识拆分合并命令、课程与知识双向影响、可恢复批量迁移及多教学模式真实课程验收，不能据此宣称整个知识系统完成。
