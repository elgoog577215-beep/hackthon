...
>
> 2026-07-22 完整课程模式修正：当前主链升级为 `course_generation_v16 / course_prompt_v23`。课程级“小课紧凑调用”已从目录和教案控制流、Prompt、配置与恢复入口中删除；所有课程一律使用“轻量骨架 → 有界批次 → 本地唯一汇编”的完整链路。用户明确指定章数或小节数时严格遵守；未明确规模时，需求 brief 默认写入至少 6 章、18 节的完整课程基线。每批 6 节、骨架每片 2 节和详细教案每次 1 节都只表示单次模型工作单元，不得压缩整门课程。旧 `compact_single_call` 检查点和非分层目录不会继续复用，恢复时按 v16 标准链重建。
>
