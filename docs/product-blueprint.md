...
| --- | --- | --- | --- | --- |
| 1. 课程生产与当前课程内容 | 生成侧十项能力、异常生命周期、可见生成现场、全课一致性与课程知识库 v2 第一条纵向主链已形成 | 学科模式、难度契约、资料证据、统一生成工作区、`CourseDocument / CourseBlock`、块级候选与恢复；首次生成现在先产出“路径引用 → 概念组 → 原子知识点 → 能力 / 易错 / 掌握标准 + 六类关系”，禁止标题复制，并让正文块、目标、题目、教学表达和 AI 消费同一课程内 ID；新课程生成、资产、AI 老师和知识审核只读取当前课程知识库 | `CourseKnowledgeBase` 仍随课程元数据保存，尚无独立原子仓库、知识拆分合并等领域命令、修订映射和双向影响分析；大课程重建仍是同步分批调用 | 建立独立知识仓库与变更命令，把知识变化接入课程、课件、题目和学习证据的精确影响预览；新课程必须在发布前通过统一知识质量门，不再为历史课程增加迁移和兼容流程 |
