...
学生侧正式采用一条本节课程生长链，而不是“手动重生成”和“证据适配”两套功能：`用户明确要求 / 挑战就绪证据 → 解析本节要强化的教学作用与难度方向 → 盘点现有块 → 已有块 REPLACE、缺失块 INSERT → 逐块候选检查点 → 本节知识同源与整组质量门 → 用户整体确认 → 单次原子课程提交 → 受影响教学表达重建 → 新难度独立复验`。前端固定显示“需求、结构、候选、同源、确认、复验”六步，用户能够看到哪些内容保持不变、哪些块被升级、哪些块是新增，以及为什么这样改。正式提交前，当前课程正文不发生变化；提交后，历史作答、笔记和旧难度掌握记录不被覆盖。
...
| 5. 统一 AI 老师 | 正式事实、学习者模型、单课程知识切片和本节课程生长计划已接入同一入口 | 块级解释、举例、简化、提问、原位追问、来源锚定、回答自动入正文、显式效果反馈、按需全局会话、`AIContextPackage`；AI 老师使用唯一 `courseEvolution` 状态承接手动本节调整与证据建议，按真实教学作用决定升级或新增；AI 只读取当前课程中通过质量门的知识、能力、易错、掌握标准和一跳关系子图 | 知识侧尚缺沿关系网定位最小前置断点的正式诊断器；课程生长侧尚缺行级操作、完整难度向量、部分接受和广域规划 | 扩展知识关系诊断、行级与多节点操作；AI 可生成当前课程方案但不得替学生确认，不得修改其他课程或绕过独立知识维护门禁 |
| 6. 课程质量反馈与持续迭代 | 两条 Demo 纵向链已进入同一课程真源，本节课程生长已补齐“块内升级 + 缺失块新增 + 原子提交 + 撤销恢复 + 挑战建议”闭环，但两个 OpenSpec 尚未全部完成 | 教师侧能从独立 PPT 工作台修改学习目标并安全发布五类同源表达；学生侧同时支持 `用户明确要求` 与 `重复正式通过` 两类入口，经同一 `CourseEvolutionPlan` 生成候选。现有教学作用保留块 ID 升级，缺失作用新增块；每个候选即时保存，整组通过同源门后由用户确认并单次提交，撤销会恢复旧内容并退役新增块。重复全对不会制造知识缺口，而会形成保留旧掌握事实的挑战升级建议；应用后同步重建受影响教学表达并等待新难度独立复验 | 同源侧尚缺真正的单页/单段/单题组异步增量构建、跨产物深层一致性、通用图解和外部 PPT 回导；课程生长侧尚缺行级变化、部分接受、多节点规划、跨域恢复、方案历史视图和真实长期效果样本 | 完成剩余操作代数和故障矩阵，验证并发、冲突、跨域恢复、部分接受与长期效果；任何学习证据都不得修改其他课程或直接改知识库 |
