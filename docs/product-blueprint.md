...
>
> 2026-07-19 首次课程生成链路修正：新课程默认且只走“需求 → 目录 → 课程生成 → 确认发布”四步生产线。提交需求视为确认第 1 步；用户确认目录后，系统先冻结全课知识职责，再按预算分批生成详细教案并确定性汇编为唯一正式全课教案，由该教案编译知识库、稳定 ID、关系图和逐节绑定，并按真实硬前置并行生成正文。教案与知识库不再作为独立用户确认门；最终发布前仍必须通过同源版本链检查。
...
→ 一次轻量 CourseTeachingPlanSkeletonV3 冻结全课知识键、规范陈述、首次负责/复用、前置与模块边界
→ 按章节边界和输入/输出预算拆分 CourseTeachingPlanBatchV3，以课程级并发上限展开能力、易错、掌握标准与局部课程块职责
...
批次和最终全课教案分别保存状态、输入修订、尝试次数与结构报告。全课教案不得生成
...
最多 3 节且输入/输出预算安全的小课可以使用紧凑单次调用；大课必须先形成一次全局
骨架，再按每批最多 2-3 节、12-15 个知识点的默认预算拆分。详细批次默认并发 2，
规划调用数只随有界批次数增长。无论规模如何，知识库与关系图模型调用均为零，最终
只有一份正式教案。正文仍按稳定教学合同逐节生成；教案通过后，满足硬前置的正文
立即按波次并行。单个批次或正文失败时保留骨架、其他已完成批次和草稿。
...
| 10. 结构发布门与课程优化 | 生成后 AI 评分、AI 审稿、全课 AI 修复和逐题 AI 预检已退出首次生成主链；发布前只保留结构、ID、引用、覆盖、依赖、绑定、答案合同和同源版本链硬门禁 | 通过生产指标验证 `1 → N → 1` 只汇编一份正式 `CourseTeachingPlanV3`，每批遵守预算且可独立恢复，知识库与关系图模型调用均为 0，正文在教案通过结构门后按依赖波次并行；学习后的“反复困惑或错误 → 改进提案 → 确认修改 → 效果复验”仍按整体产品模块 6 推进 |
