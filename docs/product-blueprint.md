...
| 2. 学习工作区与 `LearningRuntime` | 正式学习现场已进入主链；个人适配方案和已接受个人补充已接入运行时 | 课程库、左目录中正文的学习现场、块级 AI、按需全局 AI、正文任务入口、教学资源覆盖层、断点提示、正文学习记录块、运行时适配块；已接受的个人解释、静态步骤、理解检查和后续过渡仍在同一正文流展示 | 尚无通用行级差异、未来章节目录标记、个人知识状态提示、部分接受和统一方案历史视图 | 把当前受控个人补充扩展为完整 `CourseDocument + PersonalCourseOverlay + PendingAdaptationOverlay` 投影；阅读、任务、记录、知识、适配与 AI 继续共用同一学习现场 |
...
| 5. 统一 AI 老师 | 正式事实、学习者模型、课程知识切片和主动个人适配方案已接入同一入口 | 块级解释、举例、简化、提问、原位追问、来源锚定、显式回答反馈、按需全局会话、`AIContextPackage`；AI 只读取通过质量门的当前位置知识、能力、易错、掌握标准和一跳关系子图；有足够个人证据时展示适配理由、来源、范围、接受、拒绝、撤销和效果状态，知识或证据不足时不伪造建议 | 知识侧尚缺沿关系网定位最小前置断点的正式诊断器；个人侧尚缺行级覆盖、个人难度向量、部分接受、重新生成和广域规划 | 分别扩展知识关系诊断与 `PersonalAdaptationPlan` 操作代数；AI 可生成个人方案但不得替学生确认，也不得修改基础课程或课程知识库；基础课程改进只能进入独立 `CourseAuthoringChange` 链路 |
| 6. 课程质量反馈与持续迭代 | 两条纵向链均有首个可运行切片，知识库 v2 已成为两者共享的语义坐标，但尚未分别达到完整 OpenSpec | 基础课程会记录可重放修订事件；同一 `CourseDocument` 可确定性编译大纲、教案、讲义、练习册和 `SlideDeckSpec`，各表示共享课程知识 ID；PPT 编辑先区分表现、等义、语义与模糊变化。个人侧已形成 `EvidenceItem → AdaptationHypothesis → PersonalAdaptationPlan → 接受/拒绝 → PersonalCourseOverlay → 撤销/效果比较` | 同源侧尚缺异步增量重建、最后可用版本降级、图解与动画规范、完整 `CourseAuthoringChange`、知识双向影响和外部 PPT 回导；个人侧尚缺完整适配操作代数、知识精确映射、跨域恢复、行级覆盖、部分接受和真实长期效果样本 | 两条链分别完成演示与故障矩阵；最后只做基础课程更新后的个人覆盖重定位、群体问题转教师建议这两类受控协作验收，个人证据不得直接写入基础课程或知识库 |
...
+ EvidenceItem → AdaptationHypothesis → PersonalAdaptationPlan
→ 理由与范围审阅 → 接受 / 拒绝 / 撤销 → 运行时投影 → 后续效果比较

→ [下一收束门槛：不按 P0/P1 拆散]
同源表达异步精准重建、失败降级、基础课程语义回写
