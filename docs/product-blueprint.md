...
>
> 2026-07-23 大会双 Demo 闭环：录屏使用可重复重置的《矩阵与线性变换》确定性起点，但后续全部走正式能力链。教师端从六类教学材料总览进入 PPT 语义修改，确认后返回真实课程修订、材料重编译与可按材料类型逐项查看的前后差异，不再使用录屏合成回执。学生端的 AI 助手与“调整课程”入口共享强学习证据合同，确认后生成课程新修订；新增的针对性正式练习使用无需外部模型的确定性评分，并与动画交互共同推动效果状态进入“本轮独立复验通过”。`./dev.sh --recordly-demo` 只在启动前重置专用课程与演示学习者，不改变其他课程。
>
