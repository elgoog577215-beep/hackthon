...
>
> 2026-08-04 教案工作台升级规划（尚未实施）：现有“总体教案 / 分小节教案”只读投影将升级为结构化编辑工作台，但仍只维护一份正式 `CourseTeachingPlanV3`，不新增 Markdown 教案真源或独立资源树。编辑采用“可自动保存草稿 → 结构化差异与确定性影响预览 → 用户确认应用 → 生成不可变正式修订 → 按影响范围定向重建”的链路；稳定知识 ID、来源绑定、目录结构和系统质量字段保持只读，目录增删排序返回现有目录编辑入口。AI 只能生成带基础修订、理由、影响和过期时间的候选，不得直接写入正式教案。详细范围、协议、分阶段任务和验收门以 `openspec/changes/upgrade-teaching-plan-workbench/` 为正式开发计划；该规划不改变首次课程生成阶段、模型调用次数、目录确认门或发布门。
>
