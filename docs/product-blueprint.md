...
>
> 2026-07-20 生成链结构化拆分修正：当前主链为 `course_generation_v13 / course_prompt_v22`，单门课程不再设置总章节数硬上限。最多 6 节且规模明确的小课可先走一次紧凑目录调用；更大的课程先生成只含课程定位、章节焦点和每章小节数的轻量章节骨架，再按章节并行生成小节目录，单章超过 6 节时继续按顺序分片，最后由本地代码汇编唯一目录。目录确认后继续复用“有界知识骨架分片 → 有界详细教案批次并行 → 本地汇编”，正文则以单个小节为请求单元并发生成。20,000 字符与 7,000 mixed-language estimated tokens 只约束一次真实 `system + user` 请求，不约束课程总量；单元失败或工作窗口到期只影响当前小单元并保留检查点，任何阶段都不得重新拼回全课资料、全课教案或全课正文形成三万字级单次输入输出。
>
