...
| --- | --- | --- | --- | --- |
| 1. 课程生产与当前课程内容 | 生成侧十项能力、正常运行、异常生命周期、首版可见生成现场和课程知识库首次生成已形成主链；个体课程持续生长仍处于分阶段实现 | 学科模式、难度契约、资料证据、课程蓝图、统一生成任务、隔离工作区、创建补偿、请求幂等、暂停续跑、取消删除级联、分层质量门、带建议发布、`CourseDocument / CourseBlock` 仓库与命令、块级候选、恢复描述与修订冲突保护；首次生成会同步编译知识点、能力点、易错点、提升点及课程位置映射，内容 prompt、学习资产和发布质量门消费同一课程知识库 | 当前正文增量事件仍缺少 `sequence / draft_revision`，断线恢复依赖累计草稿检查点而非精确追帧；课程知识库目前随生成工作区和课程资产持久化，尚无独立修订仓库、候选式细化、完整结构操作、跨域变更集和知识双向影响分析 | 补齐增量序列、修订号和并行章节压力验收；继续按 `build-structured-adaptive-course-ai` 实现课程知识库独立仓库、领域命令、通用 `CourseChangeSet`、修订映射与可恢复命令组；个人节点不得自造正式知识 ID |
...
→ [随后继续：产品模块 6 的个体化生长]
EvidenceItem + AdaptationHypothesis + CourseChangeSet
→ 动态局部 / 依赖 / 广域候选
→ 理由与范围审阅
→ 可恢复领域命令组
→ 修改效果复验
...
这不代表第 10 节的目标架构已经全部成立。当前已接管一门标准历史课程、所有新课程的首次生成发布、规范课程的单块局部重新生成及两条生成链的失败恢复；其余历史课程与其他旧节点修改接口仍未完全收口。`CourseKnowledgeBase` 已接入首次生成、课程映射、内容 prompt、学习资产、学生读取和发布质量门，但独立修订仓库、候选式细化及知识双向影响尚未实现；原始证据索引、适应假设、通用课程变更集、未来章节候选和效果复验仍只有需求与 OpenSpec。下一阶段按 `build-structured-adaptive-course-ai` 的依赖顺序推进，不能把局部实现误报为完整个体化生长闭环。
