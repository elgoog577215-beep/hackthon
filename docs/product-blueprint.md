...
| 2. 学习工作区与 `LearningRuntime` | 正式学习现场已进入主链；课程生长候选、正文记录和正式课程修订已使用同一现场 | 课程库、左目录中正文的学习现场、块级 AI、按需全局 AI、正文任务入口、四类教学资源覆盖层、课程顶栏 PPT 一级入口与独立全屏工作台、断点提示、正文学习记录块、运行时适配块；确认后的解释、动画、理解检查和后续过渡成为当前 `CourseDocument` 中的正式 `CourseBlock`，正文、练习和课程版本直接读取同一课程文档 | 尚无通用行级差异、个人知识状态提示、部分接受、统一方案历史视图和更复杂操作的目录定位 | 完成 `CourseDocument + PendingCourseEvolutionProjection + LearningRecord` 的完整投影与操作历史，并补齐撤销后的课程版本对账；阅读、任务、记录、知识、适配与 AI 继续共用同一学习现场 |
...
| 5. 统一 AI 老师 | 正式事实、学习者模型、单课程知识切片和主动课程生长方案已接入同一入口 | 块级解释、举例、简化、提问、原位追问、来源锚定、回答自动入正文、显式效果反馈、按需全局会话、`AIContextPackage`；AI 只读取当前课程中通过质量门的知识、能力、易错、掌握标准和一跳关系子图；证据面板区分观察、正式支持与反证，并能展示调整前、课程变化和独立复验三段证据链 | 知识侧尚缺沿关系网定位最小前置断点的正式诊断器；课程生长侧尚缺行级操作、难度向量、部分接受和广域规划 | 分别扩展知识关系诊断与 `CourseEvolutionPlan` 操作代数；AI 可生成当前课程方案但不得替学生确认，不得修改其他课程或绕过独立知识维护门禁 |
| 6. 课程质量反馈与持续迭代 | 两条 Demo 纵向链已进入同一课程真源，课程生长已补齐正式写入和分级复验，但两个 OpenSpec 尚未全部完成 | 教师侧能从独立 PPT 工作台修改学习目标，把变化识别为教学维度变化；全屏影响图从真实绑定展开教案重点、讲义引导、相关讲解/例题与理解检查，同时列出明确保持不变的单元，经确认才写入正式课程目标并安全发布五类同源表达；同步回执逐单元展示真实前后差异，并区分实际改写与来源校验。真实《矩阵与线性变换》课程已按“整套规划 → 逐页填充 → 质量门”生成 25 页中文课件，LaTeX 会统一转译为可直接展示的数学符号、Unicode 上下标、矩阵/方程组括号和运算箭头；原始命令、替换字符与错误解码会阻断发布，并已在原生 PowerPoint 验证可编辑对象、讲者备注及公式无乱码；学生侧形成 `观察证据 → 正式表现确认缺口 → CourseEvolutionPlan → 目录范围 → 接受/拒绝 → CourseDocument 新修订 → 主动动画判断 → 新独立正式题 → 初步支持/持续确认` | 同源侧尚缺真正的单页/单段/单题组异步增量构建、跨产物深层一致性、通用图解和外部 PPT 回导；课程生长侧尚缺完整操作代数、跨域恢复、行级变化、部分接受、方案历史视图和真实长期效果样本 | 分别完成剩余故障矩阵和真实长期效果验收；最后统一验证两条触发链对同一课程规范修订的并发、冲突、撤销和派生表达重建，任何学习证据都不得修改其他课程或直接改知识库 |
