...
| 6. 课程质量反馈与持续迭代 | 两条 Demo 纵向链已进入同一课程真源，知识库 v2 只提供当前课程语义坐标，但两个 OpenSpec 尚未全部完成 | 教师侧能从独立 PPT 工作台修改学习目标，把变化识别为教学维度变化，动态区分受影响和保持不变单元，经确认写入正式课程目标并安全发布五类同源表达；真实课程已按“整套规划 → 逐页填充 → 质量门”生成 19 页中文课件，并在原生 PowerPoint 验证可编辑对象、讲者备注和无乱码；学生侧形成 `提问 + 学习记录 + 正式作答 → 明确认知缺口 → CourseEvolutionPlan → 目录范围 → 接受/拒绝 → CourseDocument 新修订 → 动画交互 → 正式任务复验 → 效果状态` | 同源侧尚缺真正的单页/单段/单题组异步增量构建、跨产物深层一致性、通用图解和外部 PPT 回导；课程生长侧尚缺完整操作代数、跨域恢复、行级变化、部分接受、历史视图和真实长期效果样本 | 分别完成剩余故障矩阵和真实长期效果验收；最后统一验证两条触发链对同一课程修订的并发、冲突、撤销和派生表达重建，任何学习证据都不得跨课程或直接改知识库 |
