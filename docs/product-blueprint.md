...
→ 按小节生成概念组、原子知识点、能力、易错与掌握标准
→ 每个小节独立执行知识原子性检查、局部纠正与检查点保存
→ 系统冻结全课知识节点并分配课程内稳定知识 ID
→ 独立关系阶段按小节邻域读取已冻结的全课 ID 注册表
→ AI 只复制系统 ID，逐一提交“已连接 / 课程入口”决定与六类关系
→ 系统合并关系批次，检查 ID、类型、自环、重复、决定完整性与环路
...
| 视觉与交互一致性 | 两个主空间和 3 月体验基线已经确定，学习主外壳已收束；知识库已采用浅色高密度覆盖层，并通过桌面、移动详情和英文模式验收 | 其余页面继续按真实截图逐页验收；细化课程知识库时不得把维护者审核操作塞进学生知识视图 |
