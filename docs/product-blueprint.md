...
>
> 2026-07-20 性能与稳定性硬门修正：当前主链为 `course_generation_v11 / course_prompt_v20`。最终 `system + user` 输入必须在调用提供方前同时通过 20,000 字符与 7,000 mixed-language estimated tokens 两道硬门；估算按 ASCII 与非 ASCII 文本分别计算，不能用平均字符比低估中文。单门课程默认最多 24 节，候选模型共享最多 2 次尝试，目录/教案单次、全课教案、单节正文和全文阶段分别有代码级时限。教案批次只携带当前知识与直接依赖闭包，正文只读取冻结教案并默认并发 4；学习先修关系不再形成生产波次。任何环境配置都不能把这些边界放大为无上限请求或小时级自动等待。
>
...
骨架，再按每批最多 2-3 节、12-15 个知识点的默认预算拆分。每批只发送当前知识与
直接依赖闭包，详细批次默认并发 4，规划调用数只随有界批次数增长。无论规模如何，
知识库与关系图模型调用均为零，最终只有一份正式教案。正文仍按稳定教学合同逐节
生成；教案通过后，全部待生成正文直接进入默认并发 4 的队列。`prerequisite_node_ids`
只约束学生学习顺序，不约束生产调度。单个批次或正文失败时保留骨架、其他已完成
批次和草稿。

所有首次生成模型调用都必须经过最终载荷硬门：默认单次输入同时不超过 20,000 字符
和 7,000 mixed-language estimated tokens，单门课程不超过 24 节，候选模型共享最多 2 次尝试；目录或教案最小调用默认
90 秒、全课教案 360 秒、单节正文 150 秒、全文阶段 480 秒。环境变量只能在代码上限
内调节，不能关闭这些保护。阶段与节点保存调用数、累计/最大输入估算、耗时、并发和
输出字符等非敏感指标，不保存 prompt 或推理。达到边界时必须在提供方调用前拒绝，或
停止当前最小单元并保留检查点，不能继续累积成小时级等待。
