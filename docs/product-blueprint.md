...
校验、依赖计算、检查点、重试范围与发布
判断。目录错误只重做目录，单节知识包错误只重做当前小节，关系错误只重做对应
关系批次，正文错误只重做目标块；不得因为一个局部错误重新生成已经通过检查的
整门课程。任何半成品都不能直接混入当前课程。
...
新课程必须先通过知识原子性、关系结构与语义、能力可观察性和绑定精度质量门，再共同发布课程与知识库。旧课程缺少真实结构时保留可阅读状态并标记 `degraded / needs_enrichment`，不得再把章节标题自动复制成同名知识点。历史 `SubjectKnowledgeLibrary` 只允许在受控迁移中读取，不能为新课程提供跨课程术语、知识 ID、关系或掌握标准。
...
| 2. 学习工作区与 `LearningRuntime` | 正式学习现场已进入主链；本节课程生长六步工作台、候选、正文记录和正式课程修订已使用同一现场 | 课程库、左目录中正文的学习现场、块级 AI、按需全局 AI、正文任务入口、四类教学资源覆盖层、课程顶栏 PPT 一级入口与独立全屏工作台、断点提示、正文学习记录块；AI 老师中的本节工作台统一承接用户主动调整和证据触发建议，展示需求、结构、候选、同源、确认、复验六步，明确区分升级旧块与新增缺失块；确认后的变化成为当前 `CourseDocument` 中的正式 `CourseBlock` | 尚无通用行级差异、个人知识状态提示、部分接受、统一方案历史视图和多小节复杂操作的目录定位 | 完成行级与多小节 `PendingCourseEvolutionProjection`、部分接受和完整操作历史；阅读、任务、记录、知识、适配与 AI 继续共用同一学习现场 |
...
| 5. 统一 AI 老师 | 正式事实、学习者模型、单课程知识切片和本节课程生长计划已接入同一入口；轻量场景判断已进入 Workflow | 块级解释、举例、简化、提问、原位追问、来源锚定、回答自动入正文、显式效果反馈、按需全局会话、`AIContextPackage`；AI 老师使用唯一 `courseEvolution` 状态承接手动本节调整与证据建议。受约束 AI 先判断场景、目标教学作用、难度差异和资料要求，非法结果或模型失败由规则保底；系统再按真实教学作用决定升级或新增。AI 只读取当前课程中通过质量门的知识、能力、易错、掌握标准和一跳关系子图 | 知识侧尚缺沿关系网定位最小前置断点的正式诊断器；课程生长侧尚缺行级操作、部分接受、广域规划和可信时效资料自动接入 | 扩展知识关系诊断、行级与多节点操作；AI 可理解场景并生成当前课程方案，但不得替学生确认、指定任意块、修改其他课程或绕过独立知识维护门禁 |
| 6. 课程质量反馈与持续迭代 | 两条 Demo 纵向链已进入同一课程真源，本节课程生长已补齐“场景判断 + 块内升级 + 缺失块新增 + 原子提交 + 撤销恢复 + 挑战建议”闭环，但两个 OpenSpec 尚未全部完成 | 教师侧能从独立 PPT 工作台修改学习目标并安全发布五类同源表达；学生侧同时支持 `用户明确要求` 与 `重复正式通过` 两类入口，经同一 `CourseEvolutionPlan` 生成候选。AI 语义判断只形成受约束计划输入，系统过滤未知作用和越界难度并记录资料要求；现有教学作用保留块 ID 升级，缺失作用新增块。每个候选即时保存，整组通过同源门后由用户确认并单次提交，撤销会恢复旧内容并退役新增块。重复全对不会制造知识缺口，而会形成保留旧掌握事实的挑战升级建议；应用后同步重建受影响教学表达并等待新难度独立复验 | 同源侧尚缺真正的单页/单段/单题组异步增量构建、跨产物深层一致性、通用图解和外部 PPT 回导；课程生长侧尚缺行级变化、部分接受、多节点规划、跨域恢复、方案历史视图、时效资料获取和真实长期效果样本 | 完成剩余操作代数和故障矩阵，验证并发、冲突、跨域恢复、部分接受、资料时效门和长期效果；任何学习证据都不得修改其他课程或直接改知识库 |
