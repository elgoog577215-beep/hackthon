...
| 6. 课程质量反馈与持续迭代 | “结构化同源 + 个体化生长”已进入共享内核 P0：课程正式写入会持久化可重放修订事件，教学表达拥有来源绑定、课程隔离注册表和派生依赖图，可根据块、章节或整课修订精准标记过期；个体化证据到正式变化的闭环仍待实现 | 生成侧蓝图、节点和最终质量检查，块级候选、正式学习事实、学习者模型、AI 提案回执、`CourseRevisionEvent / SourceBinding / TeachingRepresentation / AssetDerivationGraph` 与对账 API 可复用 | 缺少 `EvidenceItem → AdaptationHypothesis → CourseChangeSet → 用户确认 → 效果复验` 主链；尚未实现教案、讲义、题目投影和 `SlideDeckSpec` 的真实编译、增量重建及 PPT 反向语义编辑 | 先按 `build-same-source-teaching-representations` 完成 R1 最小表达闭环，同时按 `build-structured-adaptive-course-ai` 完成证据与变化协议；最后用“结构化同源演示 A + 个体化生长演示 B”联合验收 |
...
这不代表第 10 节的目标架构已经全部成立。当前已接管一门标准历史课程、所有新课程的首次生成发布、规范课程的单块局部重新生成及两条生成链的失败恢复；其余历史课程与其他旧节点修改接口仍未完全收口。`CourseKnowledgeBase` 已接入首次生成、课程映射、内容 prompt、学习资产、学生读取和发布质量门，但独立修订仓库、候选式细化及知识双向影响尚未实现。2026-07-16 已完成同源教学表达 P0：`CourseRevisionEvent` 将课程新旧修订向量和变化源持久化到正式操作日志，`SourceBinding / TeachingRepresentation / RepresentationSet / AssetDerivationGraph` 已建立按课程隔离的原子注册表，局部块变化只会使真实依赖表示过期，课程级表示在任一语义变化后过期，重复事件可幂等重放，读取与对账 API 已进入主路由。当前尚未生成真实教案、讲义、PPT 或动画；下一阶段进入 `build-same-source-teaching-representations` 的 R1，同时继续 `build-structured-adaptive-course-ai` 的证据与变化主链，不能把共享底座误报为两张目标体验已经完成。
