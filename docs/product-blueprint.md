...
| --- | --- | --- | --- | --- |
| 1. 课程生产与当前课程内容 | 生成侧十项能力、正常运行、异常生命周期和首版可见生成现场已形成主链；个体课程生长完成需求与 OpenSpec 设计，尚未实现 | 学科模式、难度契约、资料证据、课程蓝图、统一生成任务、隔离工作区、创建补偿、请求幂等、暂停续跑、取消删除级联、分层质量门、带建议发布、`CourseDocument / CourseBlock` 仓库与命令、块级候选、恢复描述与修订冲突保护；学习页可从工作区读取安全草稿投影，接收正文增量，并在发布后切换正式文档 | 当前正文增量事件仍缺少 `sequence / draft_revision`，断线恢复依赖累计草稿检查点而非精确追帧；当前也只支持单块替换候选，尚无随课程生成的 `CourseKnowledgeBase`、完整结构操作、跨域变更集和知识双向影响分析 | 补齐增量序列、修订号和并行章节压力验收；随后按 `build-structured-adaptive-course-ai` 实现课程知识层、通用 `CourseChangeSet`、完整课程命令、修订映射与可恢复命令组；个人节点不得自造正式知识 ID |
