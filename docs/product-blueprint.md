...
> 2026-07-19 生成现场与学习页合并：新建课程后直接进入同一 `course_id` 的学习工作区，顶部以“需求 → 目录 → 教案与知识库 → 正文生成 → 确认发布”五个可见生产阶段解释真实内部进度，但只在目录和发布处等待用户。顶部一级视图收敛为“教案 / 课程 / 练习 / PPT”，目录继续由左侧课程导航承载；生成阶段自动跟随教案或正文，用户手动切换后保持当前视图，可随时恢复跟随。PPT 在发布前保留位置但禁用，发布完成后同一 URL 原地成为正式学习页并开放独立全屏 PPT 工作台。
>
> 2026-07-23 课程入口收束：课程卡片直接进入学习工作区，不再增加“统一课程源 → 教学设计 → 学习现场 → 课程生长”的第二套顶部流程条或默认中转总览。结构化同源是底层事实，课程生长是 AI 老师内的调整链，都不得与“教案 / 课程 / 练习 / PPT”并列竞争一级导航。
> 2026-07-23 教案与知识同源收束：一门课程只保留一份正式全课教案，前端以“总体教案 / 分小节教案”两种视图服务教师阅读；总体视图承载目标、对象、起点、策略、评价、章节进程和全课知识坐标，分小节视图承载课堂路径、课程块职责、原子知识、能力、易错、掌握标准及前后衔接。知识标签必须绑定当前课程知识库的稳定身份。课程蓝图继续作为后台机器合同，但只能由当前正式教案与确定性约束单向编译，不成为独立产品概念或独立教学语义真源。
>
> 2026-07-24 课程类型与个性化起点收束：新建课程不再同时要求用户理解“课程目的”和“课程编排偏好”，一级入口统一为系统学习、项目实战、问题探究和考试冲刺。第一期只开放系统学习与项目实战；后两类冻结协议但保持不可选。课程类型决定宏观骨架，主辅学科决定专业讲法，学习者暂定起点决定 `focus / standard / compressed / verify_in_project / milestone` 路径角色。项目实战第一期使用项目、交付物、已有经验和当前不确定点组成的结构化问题组，不新增动态追问模型接口。资料整理退出类型，个性补弱回到适配策略。
> 2026-07-23 总体教案引领生成：当前主链保持 `course_generation_v16`，Prompt 升级为 `course_prompt_v24`。教师视图与生成模板共用同一份确定性总体教学引领；课程定位、总体成果、教学对象、学习起点、教学主线、章节责任、评价证据和小节前后衔接进入现有详细教案与正文模板。该变化不增加模型调用、生成阶段或确认门，也不得改变冻结目录、知识身份、课程块集合、资料与难度合同。
> 2026-07-23 学科教学模板 V2：当前主链保持 `course_generation_v16`，Prompt 升级为 `course_prompt_v25`。八个一级学科模式继续作为稳定产品边界；其下只为确有教学方法差异的领域增加内部学科分型，并为每个一级模式建立五种课型。系统依据课程主题、小节目标和全课阶段确定性选择课型，每个课型同时声明教学目的、必需课程块、成果证据和质量底线。旧的“一个学科全部模块复制到每节”退出新课程生成；分型、课型与质量合同进入章节、详细教案、正文和教师教案投影，但不新增模型调用，不改变目录、知识身份、编排偏好、难度、检查点或恢复协议。旧课程不迁移。
>
> 2026-07-24 大会双 Demo 示例更新：矩阵样本退出录屏故事线。教师端使用人工智能通识课的前沿内容更新，展示一次语义修改如何精准联动教案、讲义、PPT 与练习；学生端选择项目实战，以同一玻璃杯设计任务和不同专业起点生成不同个人路径，并在学习中继续生长。两条视频仍必须走正式课程修订、材料重编译、起点画像、课程生成和确认链；`./dev.sh --recordly-demo` 只在启动前重置专用课程与演示学习者，不改变其他课程。
>
> 2026-08-03 PPT V5 与任务可观察性收束：具备正式课程逻辑的课程默认使用 `slide_deck_v5`，先形成整套叙事与章节教学弧，再完成语义分组、视觉决策、最终版式和发布质量门；浏览器预览与 PPTX 导出共同消费 `resolved_layout / resolved_composition`。来源约束规则图、公式、表格和图表进入确定性可编辑渲染，可信图片可进入内容寻址资产仓库，不可靠视觉必须降级为完整文字版式。课程生成与 Markdown 导入统一进入课程任务中心，以“资料接收、解析与分类、检索证据、内容生成、质量检查、导出与发布”六阶段展示实时动作、心跳、阻断原因和后端允许的恢复范围；正文齐备不得再被误报为已经发布。
>
> 2026-08-04 教案工作台首期已实施：现有“总体教案 / 分小节教案”在唯一正式 `CourseTeachingPlanV3` 上新增了“自动保存草稿 → 结构化差异与确定性影响预览 → 用户确认应用 → 不可变正式修订 → 下游表示过期标记”的闭环，不新增 Markdown 教案真源或独立资源树。当前可编辑总体定位、对象、起点、目标、策略理由，以及小节目标、关键点、教学目的/指导和知识描述/能力；稳定知识 ID、来源绑定、目录结构和系统质量字段保持只读。AI 只返回可选的结构化字段候选，不能直接写入正式教案。已提供修订比较、恢复为新修订、V2/旧课程只读兼容与 `TEACHING_PLAN_WORKBENCH_ENABLED` 开关。教学模块增删排序、目录回跳、定向重建候选、AI 小节/全课重写、离开保护和完整多端 UI 自动化仍按 `openspec/changes/upgrade-teaching-plan-workbench/` 的未勾选任务继续推进；首期不改变首次课程生成阶段、模型调用次数、目录确认门或发布门。
>
...
>
> 2026-07-20 AI 主链历史收敛：首次生成只让模型承担轻量目录、教案语义和正文三类不可约产出。教学画像、难度、课程块硬骨架、稳定 ID、知识库、关系图、校验、恢复和发布全部由确定性代码负责；自动教学画像不再增加目录前的模型分类调用。该版本曾按课程规模选择执行路径；这一分叉已由 2026-07-22 的 v16 决策删除。正式产物始终统一为 `CourseTeachingPlanV3`，V2 只保留历史读取兼容。每次必要模型请求必须先切换到真实阶段，立即失败也要准确指出目录、骨架、批次或正文。
>
> 2026-07-22 正文生命周期修正：当前主链为 `course_generation_v14 / course_prompt_v22`。目录和教案继续沿用 v13 的有界 `1 → N → 1` 分片；正文仍以单个小节为请求单元、默认四路并发，但不再设置单节 75 秒或整课 480 秒固定总时限。只要小节持续收到新内容就继续，连续 90 秒无新内容才停止当前小节并保留草稿；整课正文以所有小节结算为结束条件，课程规模不会自然耗尽一个固定倒计时。每个节点的流式草稿写入独立原子检查点，读取时叠加恢复，定稿、上游失效、发布或删除时清理，流式热路径不得每 8 秒重写整门课程文件。20,000 字符与 7,000 mixed-language estimated tokens 只约束一次真实 `system + user` 请求，不约束课程总量；任何阶段都不得重新拼回全课资料、全课教案或全课正文形成三万字级单次输入输出。
>
> 2026-07-22 真实链路结构修正：当前主链升级为 `course_generation_v15 / course_prompt_v22`。目录、教案和正文不再各自用固定并发直接撞向提供方，而是在真实模型调用层共享按提供方与模型计算的容量队列；连续成功后渐进扩容，429 或额度错误立即收缩、冷却并切换，排队本身不会被误判为卡死。目录与教案的 60/180/360 秒墙钟时限已经退出控制流，统一改为连续无模型输出窗口；教案骨架默认每片最多 2 节，详细教案一次只生成 1 节。任一教案分片或详细批次使用本地语义兜底时只保存 `retry_required` 预览并停在正文之前，恢复只重跑非模型单元。正文定稿逐节闭合公式并重新质检；练习合同把空对象当作缺失，确定性补齐输入方式和“依据—过程—检查”量规；发布审阅展示与真实发布门一致的正文和资产阻断项。
>
> 2026-07-22 完整课程模式修正：当前主链升级为 `course_generation_v16 / course_prompt_v23`。课程级“小课紧凑调用”已从目录和教案控制流、Prompt、配置与恢复入口中删除；所有课程一律使用“轻量骨架 → 有界批次 → 本地唯一汇编”的完整链路。用户明确指定章数或小节数时严格遵守；未明确规模时，需求 brief 默认写入至少 6 章、18 节的完整课程基线。每批 6 节、骨架每片 2 节和详细教案每次 1 节都只表示单次模型工作单元，不得压缩整门课程。旧 `compact_single_call` 检查点和非分层目录不会继续复用；无论停在目录确认还是已经继续到发布确认，恢复时都按 v16 标准链重建。
> 2026-07-22 正式练习链修正：课程正文完成后先确定性编译学习资产，只把未通过质量门的“小节 × 练习位”交给通用练习生成编排器；每个小节完成即保存题库检查点，恢复时复用已通过题目，不重做目录、教案、正文或其他练习。可直接开放给学生的核心练习仍必须通过输入、答案、领域与可判定性门禁；尚未批准的诊断、补救验证和综合测评候选稿保持隔离并进入待复核，不再与首次课程发布错误耦合。质量阻断必须显示为发布未完成并提供“只修复练习”的恢复动作，修复通过后重新出现最终确认发布。
>
> 2026-07-22 目录生长现场修正：目录阶段不再用整页灰色占位伪装进度。后端把已保存的章节骨架、当前批次、已完成小节和检查点投影为用户安全的 `course_outline_growth_v1`；前端按真实批次在最终目录位置逐章、逐节生长，允许展开与收起，并同时表达进行中、已保存、等待、中断和恢复。并发章节的心跳不得让已形成目录回缩；无动效偏好、中英文与窄屏共用同一状态语义。目录完成后原地进入现有可编辑确认态，不新建第二套目录或确认流程。
>
...
```mermaid
flowchart TB
    U0["课程意图<br/>课程类型、主辅学科、学习者起点、资料与难度"]
...
- 课程创建弹窗继承旧版“纵向难度列表 + 可扫读图形选项”的核心选择节奏，课程类型、类型化目标与难度构成第一阅读路径；选择类型后只展示该类型需要的结构化字段。主辅学科、学习者起点和资料边界属于同一需求上下文，不再保留“课程目的 + 课程编排偏好”两套重叠选择，也不能与难度拼成左右控制台。
...
新增能力必须先从用户当时的任务和上下文设计交互，再决定入口。单个来源对象上的即时动作优先出现在正文边缘或原位展开；短输入与解释使用已有 AI 区域；正式练习和临时完整任务使用现有覆盖层；多对象比较、范围选择、差异确认与提交使用全宽工作台。功能名称本身不能推出新页面、新路由、常驻状态卡或右侧栏目，尤其不能因为能力叫“地图”“时间线”“历史”就先决定它的界面位置。

当前课程界面是视觉与交互真源，不是 AI 可以借新增功能重新设计的旧壳。任何改变用户工作流、信息架构、功能位置、视觉重心或 AI 主动程度的方案，都应先给出 2–3 个可比较方案、桌面与移动线框、完整状态和接口/状态影响，由用户确认后再实施；实现完成还要以真实中英文流程和截图验收，测试或构建通过不能替代用户审美判断。
...
- 在创建课程时选择系统学习或项目实战，并说明相应目标、主辅学科、暂定起点、资料和难度；问题探究与考试冲刺在专用规划器完成前保持不可选。用户不需要理解内部模块权重或再次选择编排风格。
...
系统按“课程类型规划器 + 主辅学科必需模块 + 学习者起点 + 节级难度投影 + 资料证据边界”把这些信息编译成教学块节奏，并在后台派生兼容的编排画像。课程类型和起点不能删除学科必需模块；每个模块实例保留块角色、编排来源和块级难度契约。系统同时保留三条不同结果路径：
...
个体化不只发生在课程生成之后。新建课程时先以 `course_type` 决定学习骨架，以主辅
学科教学画像决定专业讲法，再以学习者暂定起点决定重点展开、正常学习、压缩通过和
项目中验证。首次生成形成一门独立 `course_id` 的个人课程；后续提问、作答与笔记再
沿课程生长链持续校准。创建时的自述只能作为带来源的暂定证据，不能直接变成掌握事实。
...
多模态不是在课程外增加图片、音频、PPT 和视频生成器，而是让同一个结构化课程块拥有多种可选择、可追溯、可验证的教学表达。`CourseDocument / CourseBlock` 继续是课程语义与顺序的唯一真源；图解、幻灯、讲解音频、结构化动画、交互模拟、视频和数字人均作为 `TeachingRepresentation` 挂回原块，不复制课程树、正文、知识、题目或学习状态。PPT 在产品界面上使用课程工作区第四个一级标签进入独立全屏工作台，在数据上仍读取同一 `SlideDeckSpec`、来源绑定与派生注册表；“独立工作台”只解决备课和授课空间，不意味着建立第二课程真源。
...
- 从课程库进入简洁创建流程，先选择课程类型，再填写该类型的目标、主辅学科、学习者起点、难度感受和资料用途。课程类型只有 `systematic / project / inquiry / exam`，分别表示系统学习、项目实战、问题探究和考试冲刺；第一期只开放系统学习与项目实战，后两类在专用规划器完成前显示为不可选的规划中状态。
- 系统学习输入主题与目标难度，可选目标说明和已有基础，目录按知识结构与先修关系展开。项目实战显示项目任务、最终交付物、已有经验、当前不确定点和约束组成的结构化起点问题组；项目与交付物必填，已有经验和不确定点可跳过并编译为 `insufficient`。第一期不新增动态追问模型接口，目录先表达项目里程碑和成果，再表达个人知识路径。
- 资料整理不是课程类型：教材、课件、题库和其他资料继续作为所有类型可使用的证据来源。个性补弱也不是课程类型：它由学习者起点、适配偏好和后续正式学习证据共同驱动。
- 课程类型决定宏观学习骨架，八类学科教学画像决定概念讲法、实践任务和掌握证据，学习者起点决定内容展开程度、顺序与支架。系统不枚举“类型 × 学科”组合，而是把三个维度编译进同一 `CourseTeachingPlanV3`。旧课程编排偏好退出新建课程一级 UI，仅作为后台块编排画像和历史恢复输入。
...
- 正文完成后先确定性编译正式练习；只有被质量门拒绝的练习位进入有界 AI 修复，并按小节保存。核心练习未通过时停在发布阶段，用户继续后只修复失败练习；未批准的诊断、补救验证与综合测评候选稿不对学生开放，也不阻断正文和核心练习已经合格的课程首次发布。
- 生成现场与正式学习页使用同一 URL、同一课程目录和同一正文容器。顶部只保留“教案 / 课程 / 练习 / PPT”：目录在左侧导航中展开，练习和 PPT 在发布前可见但禁用；教案阶段默认展示全课教案占位与真实返回结果，正文阶段默认展示当前生成小节。用户手动选择视图或节点后停止自动跳转，并通过“跟随生成”恢复。
- 教案工作台只读取当前课程唯一正式教案，并在同一数据上提供“总体教案 / 分小节教案”两种视图。总体教案帮助老师判断整门课为什么这样设计，分小节教案帮助老师检查每节课怎样落地并作为正文生成依据；知识点以可点击标签呈现，发布后直接定位到当前课程知识库中的对应知识。
...
- 课程蓝图保留为后台执行合同，只能从当前正式教案、教学画像、难度、资料约束和当前课程知识绑定单向编译，并记录所依据的教案修订。它不在前端形成独立产品入口，也不得脱离教案单独增加、删除或改写教学语义。
- 全课教案必须保存规范化课程类型、主辅学科上下文、学习者起点修订、后台编排画像、全课块角色分布和每节有序模块实例；每个模块实例说明来自学科必需、学科可选还是后台编排，并带本节具体教学职责、难度摘要和 `focus / standard / compressed / verify_in_project / milestone` 路径角色及依据。它是课程正文的内部合同，不再单独等待用户确认；用户在完整课程审阅时一起判断实际课程化结果。路径角色只说明当前课程怎样安排，不得冒充正式掌握结论。
...
```text
课程类型 + 类型化目标
→ 主辅学科上下文 + 学习者暂定起点 + 资料边界
→ 版本化 CourseIntent
→ MaterialAsset 与资料卡
→ 解析与 EvidenceUnit
→ 教学 brief：类型规划器给出宏观骨架，学科适配器给出讲法与证据，起点给出展开策略
→ 轻量课程目录：课程定位、学习目标、章节、小节与每节唯一责任
→ 目录形状与责任验收
→ 用户确认目录修订
→ 冻结全课小节职责、前序承接和后续保留边界
→ 学科模板、后台编排画像与难度系统先形成每节允许的课程块基线
→ CourseTeachingPlanSkeletonV3 按最多 2 节连续分片，逐片冻结全课知识键、规范陈述、首次负责/复用、前置与模块边界
...
→ 目录、教案和各节正文的真实模型请求共享自适应提供方容量，各工作单元排队并独立生成
...
1. **需求**：用户选择课程类型并填写相应目标、主辅学科、学习者起点、难度、规模和资料；项目实战在这里完成结构化起点问题组与确认，点击开始即冻结本次 `CourseIntent` 修订。
...
课程生产中的每个阶段都必须保存独立、可校验的正式中间产物。目录章节骨架、目录
小节批次、教案全局骨架、每个详细批次和最终正式产物分别保存状态、输入修订、尝试
次数、输入预算、耗时与结构报告。全课教案不得生成
...
目录阶段不再用课程总小节数拒绝请求，也不存在课程级紧凑目录调用。所有课程先冻结
轻量章节骨架，再以章节为并行边界生成小节目录；单章超过 6 节
时只让该章继续分片，最终在本地按章节和小节顺序汇编。章节批次只读取课程定位、当前
章节、相邻章节边界、当前章已完成小节和最多 4 条相关证据提示，不重复广播全课资料。
章节骨架自身没有合法语义时保持失败，不用通用占位目录冒充课程。已有合法章节骨架后，单个目录批次失败才允许
本地补齐当前最小批次，并在目录确认前显式要求人工复核。

每门课程都先形成一份连续增长的全课骨架，包括用户明确要求只有 1 节的课程；每次
只处理最多 2 节并读取这些小节必要的前序知识边界。详细教案
按小节拆成独立、可并行、可续跑的工作单元，每次只发送当前小节知识与直接依赖
闭包；本地工作单元最多并发 4，真实模型调用统一进入共享容量队列。最终仍只有一份正式
教案，知识库与关系图模型调用均为零。只有教案达到 `ai_complete` 后，正文工作单元才
进入同一共享容量队列；`prerequisite_node_ids` 只约束学习顺序。
每个骨架分片完成后立即保存连续前缀及其修订；刷新、断网或进程重启后必须先校验该
前缀，再从第一个未完成分片继续，不得把“已保存检查点”变成从头重跑。
骨架分片或详细批次无法适配预算、结构纠正失败或提供方失败时，只为该最小单元生成
可恢复预览，保留其他 AI 成功结果，并记录 `retry_required / fallback_reason`；此状态不
得继续正文。无草稿正文的确定性保底仍可形成待人工审阅的小节，但不得自动发布。
...
mixed-language estimated tokens。课程总章节数不设产品硬上限；候选模型共享最多 2 次
尝试。目录、教案和正文都不设整阶段墙钟时限；单个流式调用连续 90 秒没有内容、推理
分片或调度进展才停止当前最小单元。真实模型层按提供方与模型共享容量，默认从 2 个
在途请求起步，连续成功后最多渐进到 4，429 或额度错误立即收缩和冷却。
环境变量只能在代码上限内调节。阶段与节点保存调用数、累计/最大输入估算、
实际采用的 Prompt 细节等级、拆分与降级单元、耗时、并发和输出字符等非敏感指标，
不保存 Prompt 正文或推理。每个小节草稿使用独立原子检查点，不能在流式热路径重写
整门课程。任何真正发出的请求都不能携带四万字原文；正文卡死只取消无进展小节，
健康小节与队列继续，整课不得因固定总倒计时被误暂停，也不能在没有新检查点的情况下
累积成小时级等待。
...
- 课程类型识别、项目结构化起点编译、主辅学科确认、个人路径解释与课程搭建建议。
...
`P0–P6` 只允许描述施工前置关系，不允许替代产品验收、严重程度或工作量排序；完整定义见[整体开发需求与依赖路线图](./requirements/灵知整体开发需求与依赖路线图.md)。灵知后续按两条纵向闭环分别验收：结构化同源必须从当前课程真源编译多种教学表达、精确追踪来源、识别影响并安全回写当前课程；课程生长必须从当前课程内的个人证据形成可解释假设和适配方案，经学生决定后通过领域命令形成 `CourseDocument` 新修订，并能删除、重做、撤销和用后续表现判断效果。任一链可以独立达到闭环；整体验收额外检查两者共同维护单一课程真源，同时知识和证据绝不跨课程。
...
| 2. 学习工作区与 `LearningRuntime` | 正式学习现场已进入主链；课程生成现场也复用同一页面与 URL，本节六步工作台与全课程多节点审阅都使用同一方案状态 | 课程库、左目录中正文的学习现场、块级 AI、按需全局 AI、正文任务入口、以“教案 / 课程 / 练习 / PPT”为顶部导航的全屏课程工作区、左侧目录、全屏知识库、独立全屏 PPT 工作台、断点提示、正文学习记录块；教案工作台在唯一 `CourseTeachingPlan` 上提供总体与分小节双视图：总体视图组织目标、教学对象、起点、策略、评价、章节推进和全课知识坐标，分小节视图组织学习目标、课堂路径、知识边界、可观察能力、掌握证据、易错纠偏和前后衔接，知识标签可定位当前课程知识库，不另存 Markdown 教案真源；新建课程直接进入五阶段生成现场，按阶段自动展示全课教案或实时正文，手动选择后保持当前视图，发布后原地切换正式学习态；AI 老师普通输入支持当前小节与当前全课程硬范围，正文块优化支持仅当前内容与全课程同类内容，系统按自然语言教学作用及当前块角色硬锚点匹配真实块；全课程调整从开始就进入独立大尺寸工作台，逐项读取真实保存检查点，完成后原地审阅前后差异、纳入与排除，窄侧栏只保留入口和状态摘要；所选操作一次性写入当前 `CourseDocument` | 尚无通用行级差异、个人知识状态提示、任意章节组合、目录多节点框选、逐项重生和统一方案历史视图 | 完成行级与任意目录范围的 `PendingCourseEvolutionProjection`、依赖重算和完整操作历史；阅读、任务、记录、知识、适配与 AI 继续共用同一学习现场 |
...
| 稳定性与失败降级 | 本地启动已有依赖、端口与健康门禁；首次生成拥有请求幂等、创建补偿、持久检查点、启动对账、手动续跑、取消/删除级联和发布回执防重；生产部署构建与服务器激活分离，活动生成任务不再永久阻断发布，而是经优雅停服后由新版本按同一检查点自动恢复；provider 隐藏重试已关闭，流式错误不会写入正文；严格质量状态与发布许可分离；前端统一对账任务、课程库和正式正文；真实新课生成、并行任务、暂停恢复、取消删除、重复提交、断网重连和带建议发布均已验收 | 把故障矩阵纳入长期回归，并继续验证真实模型提供方长时间不可用时的降级表现 |
| 视觉与交互一致性 | 两个主空间和 3 月体验基线已经确定，学习主外壳已收束；目录由左侧导航承载，教案、课程和练习统一为顶部一级视图；教案采用当前小节备课工作纸而不是第二套目录或通篇 Markdown，页面层级与结构化教学数据一一对应；知识库继续使用无遮罩、无外边距、无圆角框的浅色全屏工作台 | 其余页面继续按真实截图逐页验收；页面顶部承担一级导航时不得再套用居中弹窗语法；细化课程知识库时不得把维护者审核操作塞进学生知识视图 |

### 9.4 课程生成十项全景对账

| 项目 | 当前状态 | 仍需完成的工作 |
| --- | --- | --- |
| 1. 学科设计 | V2 已进入主链，待真实成课效果对照 | 八个一级模式、必要学科分型、40 种课型、主辅模式和模块注册表直接编译为每节课程块、成果证据与质量底线；不得退回“一个学科全部模块复制到每节”或旧节点结构 |
...
| 10. 结构发布门与课程优化 | 生成后 AI 评分、AI 审稿、全课 AI 修复、逐题 AI 预检和自动教学画像 AI 分类已退出首次生成主链；所有课程统一使用完整 `1 → N → 1` 链路并写入一份正式 `CourseTeachingPlanV3`，知识库与关系图模型调用均为 0；发布前只保留结构、ID、引用、覆盖、依赖、绑定、答案合同和同源版本链硬门禁 | 继续用生产指标验证每批预算、独立恢复、正文依赖波次、模型 P50/P95 与提供方容量；学习后的“反复困惑或错误 → 改进提案 → 确认修改 → 效果复验”仍按整体产品模块 6 推进 |
...
→ [当前进入依赖路线图 P0：可信执行与真源基座]
后端全量回归必须完成全部用例并自然退出
+ 固化运行时、lint、数据/产物边界、异步生命周期和正式写路径基线

→ [P0 完成后进入 P1：课程语义内核]
独立 CourseKnowledgeBase 修订仓库与双向影响分析
+ 课程/知识完整操作代数、命令组事务、幂等、冲突和跨域恢复

→ [P1 产出稳定课程与知识坐标后]
再按 P2 证据诊断 → P3 可审阅课程生长 → P4 同源表达
→ P5 效果验证 → P6 课程原生 Agent 的前置关系推进
→ 各层完成故障矩阵后再归档对应 OpenSpec
```
...
2026-07-23 修复生产部署被活动生成任务永久阻断的问题：发布包仍在 GitHub Actions 构建并上传，服务器切换前继续读取活动任务用于日志与追踪，但不再以 `exit 75` 丢弃整次发布。FastAPI 优雅关闭会等待任务并保存状态，超过等待窗口的任务取消后仍保留工作区草稿与完成检查点；新版本启动继续使用既有 `service_restart` 对账自动恢复同一任务。上传包只在新版本通过健康检查后删除，失败时保留排查与重试条件。该变更不放宽发布包校验、依赖变化、磁盘、备份、锁、原子切换或健康回滚门禁。
...
同日主链进一步升级为 `course_generation_v13 / course_prompt_v22`。v13 删除单门课程
24/32 节的产品级硬拒绝，并把仍然单次返回整课的目录阶段改为和教案一致的
`1 → N → 1`：轻量章节骨架只冻结全局推进和每章小节配额；章节之间并行扩展，每个
目录批次最多 6 节，同一章超出时按顺序续跑；唯一正式目录由本地汇编。48 节回归会
形成 1 次章节骨架和 8 个并行目录批次，所有真实请求仍分别通过 20,000 字符与 7,000
estimated tokens 双门。课程知识范围合同同时从保存每节全部前序与全部后续的
`O(n²)` 结构收敛为直接前置、相邻承接和下一节保留边界，课程规模增长不再导致内部
上下文平方膨胀。此后“6 节、3 节、并发 4、180/360/480 秒”都只描述模型单元或可恢复
工作窗口，不能再被解释为课程规模限制。

2026-07-22 主链升级为 `course_generation_v14 / course_prompt_v22`。真实历史工作区显示，
正常小节曾持续生成 80–113 秒，而 v13 用 75 秒总时限包住整个流；22 节课程按四路并发
也会自然逼近或超过 480 秒整课时限。v14 删除这两个与课程规模冲突的固定总倒计时，
改为连续无新内容窗口：窗口内持续收到正文块就继续，真实卡死才停止当前小节。全部
正文以所有小节完成、失败或明确取消为结算条件。与此同时，流式草稿从 2–3 MB 的整课
工作区 JSON 中拆为节点级原子 sidecar；每次检查点只写当前小节，读取工作区时再叠加，
定稿、目录失效、发布和删除时清理。这样恢复能力不变，但并行流不会再同步深拷贝、
格式化、`fsync` 和替换整门课程文件。

同日真实 6 章 12 节课程审计进一步推动主链升级为 `course_generation_v15 / course_prompt_v22`。
审计发现 29 个业务模型单元至少触发 31 次提供方尝试、5 次额度耗尽和 7 次限流；教案
五个批次恰好在 60 秒墙钟处被取消，全部教案语义最终由本地模板代替，系统却继续生成
71,504 字正文，最后又被 2 个公式和 26 个练习资产问题阻断。v15 因此把并发决策下沉到
共享提供方容量，把结构化调用改为活跃超时，并在教案与正文之间建立语义门；公式与练习
合同在最小单元定稿时确定性收口，发布审阅与真实阻断列表保持一致。

同输入真实复跑进一步证明：两节详细教案仍可生成约 2 万字符并触发 8k token 截断，
因此最终执行粒度收敛为“骨架每片 2 节、详细教案每次 1 节”。复跑首轮保住 11/12 节 AI 教案，
断点续跑只重做剩余 1 节，最终达到 12/12 AI 教案、12/12 模型正文、80,622 字符，
正文与资产阻断均为 0，实际发布成功。两段真实执行累计约 899 秒（15 分钟）；最大单次输入
为 13,667 字符 / 6,554 estimated tokens，未出现三万或四万字级请求。当前剩余成本主要是
详细教案首轮结构命中率和长正文生成，不再是墙钟误杀、限流风暴或失败后仍继续生成整课。

同日服务器现场出现未指定规模的“深度神经网络”课程只生成 3 章 6 节。任务没有模型
失败或本地降级，根因是旧预算器把空 `course_shape` 自动判为课程级紧凑模式，并把
“单次目录最多 6 节”误用成整门课程默认上限。主链因此升级为
`course_generation_v16 / course_prompt_v25`：删除目录和教案的课程级紧凑分支、相关
环境变量与 Prompt；所有课程都经过骨架、批次和唯一汇编。未指定规模时默认至少 6 章
18 节，显式规模仍精确遵守且不改变执行链。恢复时不再复用旧的非分层目录或
`compact_single_call` 教案检查点，避免历史半成品继续锁定 3×2 结构。单次批次的
“compact / minimal”载荷等级仍可用于缩减当前请求上下文，但它不再代表课程模式，
也没有权力改变章数或小节数。v24 在这一稳定链内增加与教师视图同源的总体教学引领；
v25 进一步让学科分型、当前课型、成果证据与质量底线进入章节、详细教案和正文，并以
当前课型而不是整个学科模块清单确定每节课程块，两次升级都不增加模型调用。
...
这不代表第 10 节的目标架构已经全部成立。当前已接管一门标准历史课程、所有新课程的首次生成发布、规范课程的单块局部重新生成及两条生成链的失败恢复；其余历史课程与其他旧节点修改接口仍未完全收口。课程知识库 v2 已接入首次生成、课程映射、内容 prompt、学习资产、学生读取、AI 上下文和发布质量门，但独立修订仓库、候选式细化及知识双向影响尚未实现。2026-07-16 同源教学表达已从共享底座推进到真实产物：`CourseRevisionEvent / SourceBinding / TeachingRepresentation / RepresentationSet / AssetDerivationGraph` 共同维护课程修订、单元来源、状态和影响，大纲、教案、讲义、正式练习册与 `SlideDeckSpec` 均可从同一课程编译。2026-07-18 PPT 从通用教学资源覆盖层拆出，独立全屏工作台承载完整页序、同源依据、页级编辑、AI 检查、课堂演示与右上角 PPTX 导出；2026-07-23 入口进一步收进课程工作区第四个一级标签，不再额外占用页面顶栏或建立闭环中转页。2026-08-03 PPT 主链升级为 V5：整套叙事、章节节奏、语义分页、来源约束规则图、最终版式与质量门进入同一编译合同，网页预览和 PPTX 使用同一最终页面结构；不可靠图片或视觉决策安全降级为文字版式，旧签名课件按需重新生成。课程生成与 Markdown 导入同时接入持久化任务、六阶段投影、心跳、精确错误和有界恢复合同。PPT 标题修改继续先做四类语义判定，用户可选择只改当前 PPT 或进入课程修改候选。学习侧的 `EvidenceItem → AdaptationHypothesis → CourseEvolutionPlan` 已支持范围选择、接受、拒绝、撤销与后续效果比较；确认结果写入当前 `CourseDocument` 新修订，不保留长期 `PersonalCourseOverlay`，同时学习证据不得跨课程或写入课程知识库。当前未完成项仍包括异步精准重建、跨产物深层一致性、自由图片生成的稳定质量、外部 PPT 回导、完整课程命令代数、知识双向影响门禁、课程生长的知识精确映射、跨域恢复和真实长期学习效果，不能把已完成的课件纵向链误报为整个课程智能体 OpenSpec 已完成。

同日后续交互收束：顶部主导航最终只保留“教案 / 课程 / 练习 / PPT”，目录由左侧课程导航承载；前述“大纲作为顶部一级入口”和额外课程闭环流程条仅是过渡状态，不再代表当前产品真相。
