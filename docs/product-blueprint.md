...
>
> 2026-07-19 自然语言范围与扫描工作台修正：AI 老师普通输入由用户先选择“只影响当前小节”或“应用到全课程”，正式正文优化则在生成前选择“仅优化当前内容”或“应用到全课程同类内容”；这些都是模型不可越过的硬边界。正文块入口选择全课程时，发起块的 `CourseBlock.role` 是语义硬锚点，反馈中用于描述改写方式的“应用、例子、理解、检查”等词不得把目标扩大到其他教学作用。全课程只指当前 `course_id`，系统只扫描已存在的同类块，不把局部抱怨机械外推，也不向每个小节盲目补块。全课程生成一开始就进入独立大尺寸审阅工作台，后端保存的语义判断、匹配节点和逐项候选检查点会实时追加，全部通过后在同一工作台原地转入纳入或排除；AI 老师窄侧栏只保留发起入口与紧凑状态摘要。服务端只接受方案内合法操作 ID，并以一次原子课程命令提交所选项。未选节点、其他教学作用、其他课程、历史作答和笔记原文保持不变。
>
...
2026-07-18 补齐“用户明确全课程要求”的首条可运行链：AI 老师输入旁新增 `current_section / whole_course` 范围选择，范围由用户决定且不能被模型扩大；自然语言场景判断只输出教学 `role`，全课程规划器据此扫描当前 `course_id` 中已存在的匹配块，并形成包含所属小节、前后预览、理由、同源绑定和稳定操作 ID 的多节点方案。前端全宽审阅层允许逐项纳入或排除；服务端校验操作白名单并以单次课程命令应用所选项，回执和撤销仅覆盖实际应用操作。2026-07-19 将同一硬范围补到正文块“改进正式正文”入口：用户在生成前选择“仅当前内容 / 全课程同类内容”；仅当前严格生成当前块候选，全课程则复用 `CourseEvolutionPlan` 和全宽审阅层，并把发起块的教学 `role` 作为该入口的硬锚点。反馈里用于描述改写方式的“应用、例子、理解、检查”等词不得把目标扩大到其他教学作用；如需自由指定作用组合，应从普通全课程生长入口发起。同日把全课程方案从“生成完才弹出”改为“扫描开始即进入独立工作台”：进度接口只读持久化检查点，不重新评估证据；弹窗实时追加排队、生成、通过与失败节点，完成后原地切换为勾选审阅；AI 老师侧栏不再承载长篇扫描结果。该实现完成的是“当前块 / 当前小节 / 当前全课程 + 已有同类块 + 实时检查点 + 逐项纳入”的纵向闭环，不代表选区、行段、任意章节集合、目录框选、逐项重生或部分接受后的依赖图重算已经完成。
