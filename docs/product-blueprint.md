...
>
> 2026-07-18 自然语言范围修正：AI 老师输入旁由用户先选择“只影响当前小节”或“应用到全课程”，这是模型不可越过的硬边界；AI 再把自然语言解释为例子、推导、应用、检查或概念等教学作用。全课程只指当前 `course_id`，系统按真实 `CourseBlock.role` 扫描已存在的匹配块，不把一句局部抱怨机械外推到全部内容，也不向每个小节盲目补块。多节点结果必须先进入全宽影响审阅，展示每个节点的原内容、候选、理由和所属小节；用户逐项纳入或排除后，服务端只接受方案内合法操作 ID，并以一次原子课程命令提交所选项。未选节点、其他教学作用、其他课程、历史作答和笔记原文保持不变。
>
...
| 2. 学习工作区与 `LearningRuntime` | 正式学习现场已进入主链；本节六步工作台与全课程多节点审阅都使用同一方案状态 | 课程库、左目录中正文的学习现场、块级 AI、按需全局 AI、正文任务入口、以“大纲 / 教案 / 课程 / 练习”为顶部导航的全屏课程工作区、全屏知识库、课程顶栏 PPT 一级入口与独立全屏工作台、断点提示、正文学习记录块；AI 老师输入旁支持当前小节与当前全课程硬范围，系统按自然语言教学作用匹配真实块；全课程方案在全宽覆盖层展示逐节点前后预览、纳入与排除，所选操作一次性写入当前 `CourseDocument` | 尚无通用行级差异、个人知识状态提示、任意章节组合、目录多节点框选、逐项重生和统一方案历史视图 | 完成行级与任意目录范围的 `PendingCourseEvolutionProjection`、依赖重算和完整操作历史；阅读、任务、记录、知识、适配与 AI 继续共用同一学习现场 |
...
| 5. 统一 AI 老师 | 正式事实、学习者模型、单课程知识切片和课程生长计划已接入同一入口；轻量场景判断与用户硬范围已进入 Workflow | 块级解释、举例、简化、提问、原位追问、来源锚定、回答自动入正文、显式效果反馈、按需全局会话、`AIContextPackage`；AI 老师使用唯一 `courseEvolution` 状态承接主动调整与证据建议。用户先选当前小节或当前全课程，受约束 AI 再判断场景、目标教学作用、难度差异和资料要求；系统按真实作用匹配块，并允许逐项纳入后原子提交。AI 只读取当前课程中通过质量门的知识、能力、易错、掌握标准和一跳关系子图 | 知识侧尚缺沿关系网定位最小前置断点的正式诊断器；课程生长侧尚缺行级操作、任意章节组合、证据主动推荐广域范围、逐项重生和可信时效资料自动接入 | 扩展知识关系诊断、行级与任意目录节点操作；AI 可理解场景并生成当前课程方案，但不得替学生确认、扩大用户硬边界、修改其他课程或绕过知识维护门禁 |
...
2026-07-18 补齐“用户明确全课程要求”的首条可运行链：AI 老师输入旁新增 `current_section / whole_course` 范围选择，范围由用户决定且不能被模型扩大；自然语言场景判断只输出教学 `role`，全课程规划器据此扫描当前 `course_id` 中已存在的匹配块，并形成包含所属小节、前后预览、理由、同源绑定和稳定操作 ID 的多节点方案。前端全宽审阅层允许逐项纳入或排除；服务端校验操作白名单并以单次课程命令应用所选项，回执和撤销仅覆盖实际应用操作。该实现完成的是“当前小节 / 当前全课程 + 已有同类块 + 逐项纳入”的纵向闭环，不代表选区、行段、任意章节集合、目录框选、逐项重生或部分接受后的依赖图重算已经完成。
