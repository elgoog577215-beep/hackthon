...
- 替用户确认 `CourseChangeSet`，或把个人课程知识变化自动写入其他课程、教师公共课程或跨课程参考目录。
...
| 5. 统一 AI 老师 | 正式事实、学习者模型、当前课程知识切片和主动课程生长候选已接入同一入口 | 块级解释、举例、简化、提问、原位追问、来源锚定、显式回答反馈、按需全局会话、`AIContextPackage`；AI 只读取已通过质量门的当前位置知识、能力、易错、掌握标准和一跳关系子图，知识库降级时不伪造知识上下文 | 当前生长操作仍限于解释、静态步骤、理解检查和后续过渡；尚缺沿关系网定位最小前置断点的正式诊断器、行级修改、知识双向变化、部分接受、重新生成和广域规划 | 扩展统一 `CourseChangeSet` 操作代数、关系诊断和跨域影响分析；AI 可高权限生成候选但不得替用户确认，也不得修改共享课程 |
...
→ [历史底座已完成、当前结构待重构：旧 CourseKnowledgeBase 首次生成融合]
课程蓝图可编译旧课程局部知识字段与课程映射
→ 新阶段改为先生成概念组、原子知识点、能力、易错、掌握标准与六类关系
→ 内容 prompt、课件、学习资产、学生知识视图和发布质量门消费同一课程知识坐标与精确绑定
