...
def test_retryable_failure_requeues_only_failed_work_without_progress_rollback(
    tmp_path: Path,
) -> None:
    repository = SlideBuildProgressRepositoryV2(tmp_path)
    ...
    tracker.add_work([
        SlideWorkItemV2(item_id="story-1", kind="ai_batch", stage="story", label="Story 1"),
        SlideWorkItemV2(item_id="story-2", kind="ai_batch", stage="story", label="Story 2"),
    ], now=_now(1))
    tracker.start("story-1", now=_now(2))
    tracker.complete("story-1", now=_now(3))
    tracker.start("story-2", now=_now(4))
    tracker.fail(
        "story-2",
        stage="story",
        code="story_ai_batch_timeout",
    ...
    restored = SlideBuildProgressTrackerV2.load(
        "task-retryable",
        repository=repository,
    )
    restored.resume_failed(now=_now(6))

    assert restored.manifest.status == "active"
    assert restored.manifest.failure is None
    assert restored.manifest.items[0].status == "completed"
    assert restored.manifest.items[1].status == "pending"
    assert restored.manifest.display_percent == before
