...
from teacher_asset_readiness import teacher_lesson_plan_readiness
...
def test_material_absorption_creates_linked_candidates_without_changing_current_documents(tmp_path):
    repository = TeacherLessonAuthoringRepository(tmp_path)
    repository.set_outline("course-1", "outline-confirmed")
    current = repository.save_plan_revision(
        "course-1",
        "L1-1",
        standard_lesson_plan(),
    ...
    assert view["lessons"]["L1-1"]["working_revision_id"] == current["working_revision_id"]
    assert "confirmed_revision_id" not in view["lessons"]["L1-1"]
    drafts = repository.current_material_drafts("course-1")
    assert "confirmation_required" not in drafts["outline"]
...
def test_retired_teacher_confirmation_and_history_routes_are_not_registered():
    ...
    assert f"{lesson_prefix}/arrangement/confirm" not in paths
    assert f"{lesson_prefix}/plan/confirm" not in paths
    assert f"{lesson_prefix}/script/confirm" not in paths
    assert not any("/plan/revisions/" in path and path.endswith("/restore") for path in paths)
    assert not any("/script/revisions/" in path and path.endswith("/restore") for path in paths)

    assert (
        f"{lesson_prefix}/ppt-v6/manuscript/confirm"
        in paths
    )
    assert (
        f"{lesson_prefix}/ppt-import/reviews/"
        "{review_id}/confirm"
        in paths
    )
...
def test_lesson_arrangement_projects_existing_modules_without_example_exam_collision():
    ...
    assert "confirmed" not in arrangement
...
def test_arrangement_revision_marks_dependent_preparation_assets_stale(tmp_path):
    ...
    first_arrangement_revision_id = saved_arrangement["arrangement"]["working_revision_id"]
    ...
    updated = deepcopy(repository.current_arrangement("course-1", "L1-2"))
    ...
    readiness = teacher_lesson_plan_readiness(repository.lesson("course-1", "L1-2"))
    assert readiness == {"ready": False, "unavailable_reason": "source_stale"}


def test_standard_lesson_plan_quality_gate_controls_current_revision_readiness():
...
def test_lesson_plan_clock_and_module_order_follow_current_arrangement():
    arrangement = {
        "blocks": [
            {
                "block_id": "a-1",
...
def test_canonical_outline_revision_recovers_current_state_and_blocks_weak_plan(tmp_path):
    ...
    assert teacher_lesson_plan_readiness(weak) == {
        "ready": False,
        "unavailable_reason": "content_incomplete",
    }
...
def test_uploaded_ppt_review_report_names_sources_without_fake_score():
    ...
    assert {item["label"] for item in report["sources"]} == {"当前教案", "当前讲义"}
...
def test_plan_save_rejects_a_stale_editor_revision(tmp_path):
    repository = TeacherLessonAuthoringRepository(tmp_path)
    repository.set_outline("course-1", "outline-v1")
    first = repository.save_plan_revision(
        "course-1",
    ...
    second_plan = standard_lesson_plan()
    second_plan["sections"][0]["learning_objective"] = "第二个页面先保存的修改"
    second = repository.save_plan_revision(
        "course-1",
        "L1-1",
        second_plan,
        source_outline_revision_id="outline-v1",
        expected_working_revision_id=first_revision,
    )
    saved_count = len(second["revisions"])
    ...
    assert raised.value.code == "lesson_plan_revision_conflict"
    assert len(repository.lesson("course-1", "L1-1")["revisions"]) == saved_count


def test_plan_save_api_refreshes_downstream_state_and_rejects_stale_editor(tmp_path):
    repository = TeacherLessonAuthoringRepository(tmp_path)
    repository.set_outline("course-1", "outline-v1")
    source = single_section_course_data()
    source["blueprint_revision_id"] = "outline-v1"
    initial = repository.save_plan_revision(
        "course-1",
        "L1-1",
        standard_lesson_plan(),
        source_outline_revision_id="outline-v1",
    ...
    initial_revision = initial["working_revision_id"]
    script = repository.save_script_revision(
        "course-1",
        "L1-1",
        [{
        ...
        source_lesson_plan_revision_id=initial_revision,
        generation_source="teacher_edit",
    )
    ...
    edited_plan = deepcopy(standard_lesson_plan())
    edited_plan["sections"][0]["learning_objective"] = "修改后的学习目标"

    with TestClient(app) as client:
        saved = client.patch(
            "/api/teacher/courses/course-1/lessons/L1-1/plan/draft",
            json={
                "plan": edited_plan,
                "source_outline_revision_id": "outline-v1",
                "expected_current_revision_id": initial_revision,
            },
            headers={"X-User-Id": "teacher-1"},
        )
        assert saved.status_code == 200
        projected = saved.json()["lesson"]
        current_revision = projected["plan"]["working_revision_id"]
        assert current_revision != initial_revision
        assert projected["script"]["ready"] is False
        assert projected["script"]["source_state"] == "stale"
        assert projected["plan"]["ppt_assets"][0]["source_state"] == "stale"
        saved_revision_count = len(
            repository.lesson("course-1", "L1-1")["revisions"]
        )
        ...
        assert conflict.json()["detail"]["code"] == "lesson_plan_revision_conflict"
        assert len(repository.lesson("course-1", "L1-1")["revisions"]) == (
            saved_revision_count
        )

        retired_history_route = client.post(
            f"/api/teacher/courses/course-1/lessons/L1-1/plan/revisions/"
            f"{current_revision}/restore",
            json={"expected_current_revision_id": current_revision},
        )
        assert retired_history_route.status_code == 404
...
def test_teacher_lesson_v6_source_is_synthetic_and_covers_only_one_lesson():
    ...
    assert view["teacher_lesson_source"]["script_revision_id"] == "script-v1"
    assert view["nodes"][1]["content_blocks"][0]["content"] == "这是当前讲义的一手讲授内容。"
...
def test_teacher_preview_projects_current_script_without_confirmation_gate():
    ...
    projected = project_current_teacher_scripts(preview, authoring_state)
...
def test_ppt_manuscript_draft_update_checks_revision_and_retains_recovery_points(tmp_path):
    repository = TeacherLessonAuthoringRepository(tmp_path)
    lesson = repository.save_plan_revision(
        "course-1",
        "L1-1",
    ...
    repository.save_v6_ppt_manuscript(
        "course-1",
        "L1-1",
        source_lesson_plan_revision_id=plan_revision,
        source_script_revision_id=script_revision,
    ...
    repository.confirm_v6_ppt_manuscript_draft(
        "course-1", "L1-1", manuscript_revision="pptman-1"
    )
    repository.bind_v6_ppt_manuscript_result(
        "course-1",
    ...
    second = {
        **first,
        "manuscript_revision": "pptman-2",
        "pages": [{"page_id": "page-1", "title": "第一页修订"}],
    }
    updated = repository.update_v6_ppt_manuscript_draft(
        "course-1",
        "L1-1",
    ...
    assert updated["revision"] == "pptman-2"
    assert updated["generated_representation_id"] == ""
    assert updated["last_confirmed_manuscript"]["manuscript_revision"] == "pptman-1"
    assert updated["last_good_manuscript"]["manuscript_revision"] == "pptman-2"

    with pytest.raises(TeacherLessonAuthoringError) as conflict:
        repository.update_v6_ppt_manuscript_draft(
            "course-1",
            "L1-1",
            expected_manuscript_revision="pptman-1",
            manuscript={**second, "manuscript_revision": "pptman-conflict"},
        )
    assert conflict.value.code == "lesson_ppt_manuscript_revision_conflict"
    assert conflict.value.details["current_revision"] == "pptman-2"
    assert repository.current_v6_ppt_manuscript(
        "course-1", "L1-1"
    )["revision"] == "pptman-2"

    blocked = repository.update_v6_ppt_manuscript_draft(
        "course-1",
        "L1-1",
        expected_manuscript_revision="pptman-2",
        manuscript={
            **second,
            "manuscript_revision": "pptman-3",
            "quality_status": "blocked",
            "quality_issues": [{"code": "page-invalid"}],
        },
    )
    assert blocked["status"] == "draft"
    assert blocked["last_good_manuscript"]["manuscript_revision"] == "pptman-2"
    assert blocked["last_confirmed_manuscript"]["manuscript_revision"] == "pptman-1"


def test_ppt_manuscript_source_rebase_is_revision_checked_and_atomic(tmp_path):
    repository = TeacherLessonAuthoringRepository(tmp_path)
    plan = repository.save_plan_revision(
        "course-1",
        "L1-1",
    ...
    plan_revision = plan["working_revision_id"]
    first_script = repository.save_script_revision(
        "course-1",
        "L1-1",
        [{"section_node_id": "L2-1-1", "title": "1.1", "content": "第一版讲义。"}],
        source_lesson_plan_revision_id=plan_revision,
    )
    first_script_revision = first_script["working_script_revision_id"]
    first = {
        "schema_version": "ppt_manuscript_v1",
        "teaching_content_contract_version": "page_teaching_v1",
        "manuscript_revision": "pptman-source-1",
        "source_lesson_plan_revision_id": plan_revision,
    ...
    repository.save_v6_ppt_manuscript(
        ...
        source_lesson_plan_revision_id=plan_revision,
        source_script_revision_id=first_script_revision,
        source_material_revision="pptrefs-current",
        task_id="task-manuscript-source",
        mode="teaching",
        theme="qizhi-classroom",
        manuscript=first,
    )
    repository.confirm_v6_ppt_manuscript_draft(
        "course-1", "L1-1", manuscript_revision="pptman-source-1"
    )
    second_script = repository.save_script_revision(
        "course-1",
        "L1-1",
        [{"section_node_id": "L2-1-1", "title": "1.1", "content": "第二版讲义。"}],
        source_lesson_plan_revision_id=plan_revision,
    )
    second_script_revision = second_script["working_script_revision_id"]
    assert repository.current_v6_ppt_manuscript(
        "course-1", "L1-1"
    )["source_state"] == "stale"
    ...
    second = {
        **first,
        "manuscript_revision": "pptman-source-2",
        "source_script_revision_id": second_script_revision,
        "pages": [{"page_id": "page-1", "title": "第一页新内容"}],
    }
    with pytest.raises(TeacherLessonAuthoringError) as conflict:
        repository.update_v6_ppt_manuscript_draft(
            "course-1",
            "L1-1",
            expected_manuscript_revision="pptman-wrong",
            manuscript=second,
            source_rebase=True,
            source_lesson_plan_revision_id=plan_revision,
            source_script_revision_id=second_script_revision,
    ...
    with pytest.raises(TeacherLessonAuthoringError) as material_conflict:
        repository.update_v6_ppt_manuscript_draft(
            "course-1",
            "L1-1",
            expected_manuscript_revision="pptman-source-1",
            manuscript=second,
            source_rebase=True,
            source_lesson_plan_revision_id=plan_revision,
            source_script_revision_id=second_script_revision,
            source_material_revision="pptrefs-changed",
        )
    assert material_conflict.value.code == "lesson_ppt_source_stale"
    assert repository.current_v6_ppt_manuscript(
        "course-1", "L1-1"
    )["source_state"] == "stale"
    ...
    assert updated["source_state"] == "current"
    assert updated["source_lesson_plan_revision_id"] == plan_revision
    assert updated["source_script_revision_id"] == second_script_revision
    assert updated["source_material_revision"] == "pptrefs-current"
    assert updated["revision"] == "pptman-source-2"
    assert updated["status"] == "draft"
    assert updated["last_confirmed_manuscript"]["manuscript_revision"] == (
        "pptman-source-1"
    )
    assert updated["last_good_manuscript"]["manuscript_revision"] == (
        "pptman-source-2"
    )


def test_script_revision_change_stales_bound_v6_ppt_without_confirmation(tmp_path):
    repository = TeacherLessonAuthoringRepository(tmp_path)
...
def test_current_complete_script_unlocks_the_v6_ppt_api_without_confirmation(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        assert "script_confirmation" not in repository.lesson("course-1", "L1-1")
...
def test_script_generation_edit_candidate_and_ppt_share_one_asset_chain(tmp_path, monkeypatch):
    repository = TeacherLessonAuthoringRepository(tmp_path)
    ...
    with TestClient(app) as client:
        ...
        projection = saved.json()["lesson"]
        assert projection["plan"]["current_revision"]["revision_id"]
    ...
    assert rewrite_context["current_lesson_plan"]["node_id"] == "L2-1-1"
...
def test_lesson_type_is_adjusted_from_outline_without_confirming_teaching_blocks(tmp_path):
    ...
    assert "status" not in arrangement
    assert "confirmed" not in arrangement
    assert arrangement["blocks"] == []
...
def test_teacher_lesson_api_generates_only_requested_lesson(tmp_path):
    ...
    class FakeCourseService:
        ...
        async def prepare_teacher_lesson_plan(
            ...
            *,
            ...
            assert "status" not in lesson_arrangement
    ...
    assert "status" not in generated_revision
