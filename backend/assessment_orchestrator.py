...
def _form_directive(question_form: str) -> str:
    ...
    if question_form == "fill_blank":
        return (
            ...
            "其余用 exact。\n"
            # 文本空的判等是「归一化后字符串相等」，措辞一变就判错。归因实测：
            # 失败几乎全是求解器写「系统内能增加」而标准答案是「内能增加」这类
            # 同义不同形。要求逐空穷举同义写法，是把这条判等口径补齐，
            # 不是放宽——判等规则本身一个字没动。
            "【文本空必答】凡 match_mode=exact 且答案不是纯数字的空位，"
            "acceptable_answers **必须至少给 2 条**同义写法，覆盖："
            "(a) 加/不加主语（「内能增加」/「系统内能增加」）；"
            "(b) 常见近义词（「增加」/「增大」/「上升」）；"
            "(c) 加/不加单位或量词的写法。"
            "只写一种说法的填空题会被判定为不合格。"
        )
