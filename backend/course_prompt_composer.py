...
7. `relation_type` 按语义从六类中选，不要一律写 `prerequisite`；缺必填字段的关系整条丢弃。
   ...
   **覆盖要求：本节每个新知识点都要至少出现在一条关系里**（作为 `source_key` 或
   `target_key` 都算）。提交前把本节新知识键逐个列出来点名核对，确认每个键都至少
   出现过一次。确实与本节其他知识、也与已学知识都构不成教学关系的，才允许缺席；
   一节里这种孤立知识点通常不超过一个，整节一条关系都没有基本说明是漏找而不是真没有。
