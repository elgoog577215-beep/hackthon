...
class AILearningService(AIBase):
    ...
    async def generate_learning_path(
        ...
        for progress in progress_data:
            ...
            if progress.get('read_time_minutes', 0) < 5 and progress.get('is_read', False):
                weak_points.append({
                    "node_id": node_id,
                    "node_name": node_name,
        ...
        for node_id in diagnostic_nodes:
            if node_id not in [wp['node_id'] for wp in weak_points]:
                ...
                if node:
                    weak_points.append({
                        "node_id": node_id,
                        "node_name": node.get('node_name', 'Unknown'),
                        "weakness_type": "confirmed_diagnostic_issue",
                        "severity": "high",
                        "suggested_action": f"继续完成 {node.get('node_name', '该章节')} 的诊断或补救任务"
                    })
