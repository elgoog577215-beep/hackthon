...
class CourseDocumentRepository:
        ...
        *,
        ...
        async with self._command_lock(course_id):
            ...
            if (
                ...
                and str(raw.get("course_document_revision") or "")
    ...
    async def confirm_generated_teacher_source(
        self,
        course_id: str,
        document: CourseDocument,
        *,
        ...
        """Adopt a completed generation workspace as the teacher's source.

        This deliberately mirrors the persistence half of generation
        publication without creating ``course_document_publication``.  The
        teacher can therefore establish an editable lesson-plan baseline and
        build slides before deciding what students may see.
        """
        async with self._command_lock(course_id):
            raw = self.load_raw(course_id)
            self._require_generation_shell(raw, job_id)
            ...
            if current.document_revision != expected_revision:
                raise CourseDocumentConflict(
                    "Course document revision changed before teacher confirmation",
                )
            if document.course_id != course_id:
                raise CourseDocumentConflict("Generated document belongs to another course")
            if not document.sections or not document.blocks:
            ...
            confirmed.update(self._generated_metadata(metadata))
            confirmed.update({
                "course_id": course_id,
                "course_name": updated.title,
                "course_schema_version": COURSE_DOCUMENT_SCHEMA,
                ...
                "current_course_version_id": updated.document_revision,
                "generation_job_id": job_id,
                "generation_status": str(raw.get("generation_status") or "completed_with_warnings"),
            })
            receipt = {
                "command_id": command_id,
                "operation": "confirm_teacher_authoring",
                "previous_revision": current.document_revision,
                "document_revision": updated.document_revision,
                "affected_block_ids": [block.block_id for block in updated.blocks],
                "committed_at": datetime.now(timezone.utc).isoformat(),
            ...
            operation_log.append({
                "command_id": command_id,
                "operation": receipt["operation"],
                "reason": "教师确认采用生成工作区作为课程生产来源",
                "actor": "teacher_authoring",
                "receipt": receipt,
            })
            confirmed["course_operation_log"] = operation_log[-200:]
            confirmed["course_revision_vector"] = revision_change.current.model_dump(mode="json")
            confirmed.pop("nodes", None)
            await self._save_raw(course_id, confirmed)
            _publish_course_revision(course_id, receipt)
            return receipt
