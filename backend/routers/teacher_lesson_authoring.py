...
from learner_context import require_actor_id, resolve_user_id
...
class PublishTeacherCourseRequest(BaseModel):
    confirm: bool = False
    expected_document_revision: str = Field(default="", max_length=160)
...
def _require_teacher_course_owner(raw: dict[str, Any], request: Request) -> str:
    actor = require_actor_id(request.headers.get("X-User-Id"))
    owner_id = str(raw.get("owner_id") or "")
    if raw.get("authoring_surface") == "teacher" and owner_id and actor != owner_id:
        raise HTTPException(
            status_code=404,
            detail={
...
def _question_bank_publication_status(course_id: str) -> dict[str, Any]:
    bundle = question_bank_repository.load_bundle(course_id)
    ...
    items = [item for item in bundle.get("items") or [] if isinstance(item, dict)]
    approved = [
        item
        for item in items
        if str(item.get("review_status") or item.get("lifecycle_status") or "")
        == "approved"
    ...
    return {
        ...
        "excluded_count": len(items) - len(approved),
        # Course publication never promotes question lifecycle state. The
        # learner question-bank endpoint remains the only public guard.
        "published_with_course": False,
    }


def _outline_identity(source: dict[str, Any]) -> list[tuple[str, str, int, str]]:
    return [
        (
            str(node.get("node_id") or ""),
            str(node.get("parent_node_id") or ""),
            int(node.get("node_level") or 0),
            str(node.get("node_name") or ""),
        )
        for node in source.get("nodes") or []
        if isinstance(node, dict) and int(node.get("node_level") or 0) in {1, 2}
    ]


def _confirmed_teacher_outline_source(
    current_source: dict[str, Any],
    repository: TeacherLessonAuthoringRepository,
    course_id: str,
) -> tuple[dict[str, Any], str]:
    """Recover the immutable outline snapshot consumed by confirmed plans.

    A later outline save may change the broad blueprint fingerprint by dropping
    generation-only metadata even when node identity, order, hierarchy and
    titles are unchanged. In that case the confirmed immutable snapshot is the
    accurate publication source; a real structural change still blocks.
    """
    assets = repository.view(course_id).get("lessons") or {}
    source_revision_ids: set[str] = set()
    for lesson in assets.values() if isinstance(assets, dict) else []:
        if not isinstance(lesson, dict):
            continue
        confirmed_revision_id = str(lesson.get("confirmed_revision_id") or "")
        revision = next(
            (
                item
                for item in lesson.get("revisions") or []
                if isinstance(item, dict)
                and item.get("revision_id") == confirmed_revision_id
            ),
            None,
        )
        source_revision_id = str((revision or {}).get("source_outline_revision_id") or "")
        if source_revision_id:
            source_revision_ids.add(source_revision_id)
    if len(source_revision_ids) != 1:
        return current_source, "current"
    source_revision_id = next(iter(source_revision_ids))
    if source_revision_id == _canonical_outline_revision(current_source):
        return current_source, "current"
    try:
        confirmed_source = course_version_repository.get_blueprint_revision(
            course_id,
            source_revision_id,
        )
    except KeyError:
        return current_source, "missing_confirmed_snapshot"
    if _outline_identity(confirmed_source) != _outline_identity(current_source):
        return current_source, "structure_changed"
    confirmed_source["course_name"] = str(
        current_source.get("course_name") or confirmed_source.get("course_name") or ""
    )
    return confirmed_source, "confirmed_snapshot_identity_match"


def _teacher_course_publication_candidate(
    tm: TaskManager,
    repository: TeacherLessonAuthoringRepository,
    ...
    current_source = _source_course(tm, course_id)
    source, outline_compatibility = _confirmed_teacher_outline_source(
        current_source,
        repository,
        course_id,
    )
    source["course_name"] = str(raw.get("course_name") or source.get("course_name") or "")
    ...
    if not lessons:
        blockers.append({
            "code": "teacher_course_publication_outline_empty",
            "message": "课程大纲没有可发布的课次。",
        })
    for lesson in lessons:
        lesson_unit_id = str(lesson.get("lesson_unit_id") or "")
        lesson_title = str(lesson.get("title") or lesson_unit_id)
        plan = lesson.get("plan") or {}
        plan_revision_id = str(plan.get("confirmed_revision_id") or "")
        if not plan_revision_id:
            blockers.append({
                "code": "teacher_course_publication_plan_unconfirmed",
                "lesson_unit_id": lesson_unit_id,
                "message": f"{lesson_title}的教案尚未确认或已经过期。",
            })
            continue
        script = lesson.get("script") or {}
        script_revision_id = str(script.get("current_revision_id") or "")
        if not script.get("confirmed") or not script_revision_id:
            blockers.append({
                "code": "teacher_course_publication_script_unconfirmed",
                "lesson_unit_id": lesson_unit_id,
                "message": f"{lesson_title}的讲稿尚未确认或已经过期。",
            })
            continue
        ...
        if str(plan_revision.get("source_outline_revision_id") or "") != _canonical_outline_revision(source):
            blockers.append({
                "code": "teacher_course_publication_outline_changed",
                "lesson_unit_id": lesson_unit_id,
                "message": f"{lesson_title}所依据的大纲已经变化，请重新确认本讲。",
            })
            continue
        script_revision = _script_revision(
            repository,
        ...
        if not teacher_script_revision_is_publishable(script_revision):
            blockers.append({
                "code": "teacher_course_publication_script_quality_blocked",
                "lesson_unit_id": lesson_unit_id,
                "message": f"{lesson_title}的讲稿未通过发布质量检查。",
            })
            continue
    ...
    revision_core = {
        "outline_revision_id": _canonical_outline_revision(source),
        "lessons": revision_items,
    }
    source_digest = stable_hash(revision_core, prefix="tps_")
    source_revisions = {**revision_core, "digest": source_digest}
    publication = raw.get("course_document_publication") or {}
    published_digest = str((publication.get("source_revisions") or {}).get("digest") or "")
    document = None
    if not blockers:
    ...
    question_bank_status = _question_bank_publication_status(course_id)
    current_document = CourseDocument.model_validate(raw["course_document"])
    return {
        "schema_version": "teacher_course_publication_readiness_v1",
        "course_id": course_id,
        "ready": not blockers,
        "published": bool(publication),
        "up_to_date": bool(publication) and published_digest == source_digest,
        "document_revision": current_document.document_revision,
        "lesson_count": len(lessons),
        "confirmed_lesson_count": len(lesson_sources),
        "blockers": blockers,
        "source_revisions": source_revisions,
        "outline_compatibility": outline_compatibility,
        "question_bank_status": question_bank_status,
        "publication": deepcopy(publication),
        "candidate_document": document,
        "command_id": f"publish-teacher-course-v1:{course_id}:{source_digest}",
    }


@router.get("/courses/{course_id}/lesson-authoring/publication-readiness")
async def get_teacher_course_publication_readiness(
    course_id: str,
    request: Request,
    ...
    try:
        raw = course_repository.load_raw(course_id)
        _require_teacher_course_owner(raw, request)
        candidate = _teacher_course_publication_candidate(
            tm,
            repository,
            course_repository,
            course_id,
        )
        candidate.pop("candidate_document", None)
        candidate.pop("command_id", None)
        return candidate
    except TeacherLessonAuthoringError as exc:
        _raise(exc)


@router.post("/courses/{course_id}/lesson-authoring/publish")
async def publish_teacher_course(
    course_id: str,
    body: PublishTeacherCourseRequest,
    request: Request,
    tm: TaskManager = Depends(require_task_manager),
    repository: TeacherLessonAuthoringRepository = Depends(
        get_teacher_lesson_authoring_repository
    ...
    if not body.confirm:
        raise HTTPException(
            status_code=422,
            detail={
                "code": "teacher_course_publication_confirmation_required",
                "message": "请先确认发布整门课程。",
            },
        )
    try:
        raw = course_repository.load_raw(course_id)
        actor = _require_teacher_course_owner(raw, request)
        candidate = _teacher_course_publication_candidate(
            tm,
            repository,
            course_repository,
        ...
        if candidate["blockers"]:
            raise HTTPException(
                status_code=409,
                detail={
                    "code": "teacher_course_publication_blocked",
                    "message": "课程仍有发布阻断项。",
                    "blockers": candidate["blockers"],
                },
            )
        existing_receipt = course_repository.receipt_for_command(
            course_id,
        ...
        if existing_receipt:
            return {
                "status": "published",
                "course_id": course_id,
                "learner_path": f"/course/{course_id}/learn",
                "receipt": existing_receipt,
            }
        if (
            body.expected_document_revision
            and body.expected_document_revision != candidate["document_revision"]
        ):
            raise HTTPException(
                status_code=409,
                detail={
                    "code": "teacher_course_publication_revision_conflict",
                    "message": "课程正文已经变化，请重新检查后发布。",
                },
            )
        quality_report = {
            "schema_version": "teacher_course_publication_quality_v1",
            "passed": True,
            "blocking_issues": [],
            "confirmed_lesson_count": candidate["confirmed_lesson_count"],
        }
        receipt = await course_repository.publish_teacher_authored_course(
            course_id,
            candidate["candidate_document"],
            command_id=candidate["command_id"],
            expected_revision=candidate["document_revision"],
            actor=actor,
            source_revisions=candidate["source_revisions"],
            quality_report=quality_report,
            question_bank_status=candidate["question_bank_status"],
        )
        return {
            "status": "published",
            "course_id": course_id,
        ...
        raise HTTPException(
            status_code=409,
            detail={
                "code": "teacher_course_publication_conflict",
                "message": "课程发布时发生版本冲突，请重新检查后重试。",
            },
        ) from exc
    except TeacherLessonAuthoringError as exc:
        _raise(exc)
...
async def build_teacher_lesson_v6_manuscript(
    ...
    source_plan_revision = str(
        revision.get("revision_id") or lesson.get("confirmed_revision_id") or ""
    )
...
async def build_teacher_lesson_v6(
    ...
    try:
        candidate_repository.clone_checkpoint(
            str(manuscript_state.get("task_id") or ""), task_id
        )
    except (FileNotFoundError, ValueError) as exc:
        raise HTTPException(
            status_code=409,
