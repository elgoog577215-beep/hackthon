...
- **块级公式**：必须使用 `$$` 包裹，且独占一行
  - ✅ 正确：
    ...
    ```
  - ❌ 错误：不要使用 `\\[` 或 `\\]`
- **多行对齐公式（aligned 环境）**：必须完整包裹在 `$$` 中，每行用 `&` 对齐，行末用 `\\\\` 换行
  - ✅ 正确：
    ```
    $$
    \\begin{{aligned}}
    PV_1 &= \\frac{{1800}}{{(1.10)^1}} = 1636.36 \\\\
    PV_2 &= \\frac{{900}}{{(1.10)^2}} = 743.80 \\\\
    PV_3 &= \\frac{{1000}}{{(1.10)^3}} = 751.31
    \\end{{aligned}}
    $$
    ```
  - ❌ 错误：不要在 aligned 内部使用 `$` 符号
  - ❌ 错误：不要使用 `\\$` 作为换行符（应使用 `\\\\`）
  - ❌ 错误：不要漏掉 `\\end{{aligned}}`
- **严禁裸写 LaTeX 命令**，所有数学符号（如 `\\alpha`, `\\frac`, `\\sum` 等）必须在 `$` 或 `$$` 环境中
- **复杂结构**：矩阵、方程组（`cases`）、对齐方程（`align`）等必须包含在 `$$` 中
- **换行符**：LaTeX 中换行使用 `\\\\`（双反斜杠），不是 `\\$` 或 `\\`"""
