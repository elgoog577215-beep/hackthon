...
def get_course_document_repository() -> CourseDocumentRepository:
    """Build the repository from the current injected storage boundary."""
...
def get_task_manager_optional():
    """Return the current task manager without turning read-only routes into hard dependencies."""
    return _task_manager
...
async def get_course_or_404(course_id: str) -> dict:
    ...
    try:
        ...
        data = await run_in_threadpool(repository.load_course_view, course_id)
