...
## 任务
为「{topic}」设计完整的课程蓝图。它不是普通目录，而是后续逐节生成正文时使用的执行契约。
...
6. 为每个小节列出 2-3 个关键点
7. 为每个小节补充学习目标、前置依赖、常见误区、验收标准和范围边界
8. 前置依赖只能引用更早小节的节点 ID，格式为 L2-1-1、L2-2-3；没有依赖则返回空数组
9. 不要伪造论文、链接或书目信息；需要延伸阅读时只给方向，不给未经核验的具体出处
    ...
    {{
      ...
      "sections": [
        {{
          ...
          "scope_boundary": "本节讲到哪里为止，哪些内容留到后续章节"
        }}
      ]
    }}
  ]
...
## 生成边界
1. 只生成当前小节内容，不重写整章，不提前展开后续章节。
2. 对账本中已定义的概念只做必要引用，避免整段重复解释。
3. 对账本标注为前置依赖的内容，要自然衔接并指出本节新增点。
4. 如果需要参考资料但没有真实来源，不要编造论文、年份、链接或机构报告。
5. complex 小节先用 3-5 行写出“写作计划/边界确认”，再进入正文。
...
## 固定教学块
请严格按以下二级标题输出，标题不要改名；如果某一块确实不适合，可以写“本节不需要单独展开”并说明原因。
1. ## 引入问题
2. ## 核心概念
3. ## 推理过程
4. ## 例子讲解
...
## 质量优先原则
- 内容应覆盖学习目标，但不要为了字数重复铺陈
- 只有本节必要的概念才详细展开，前置概念以引用和衔接为主
- 如果超出篇幅范围，优先保留定义、关键推理、例子、误区澄清和验收题
